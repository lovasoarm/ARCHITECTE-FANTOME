...
**SCÈNE CRAZYDEVS : Naruto :** plusieurs jutsus partent en même temps. L’ordre devient important, les messages arrivent tard et deux versions du monde peuvent coexister quelques secondes.
...
```js
// 'ws://' pour HTTP, 'wss://' pour HTTPS:utiliser un transport sécurisé en production lorsque la connexion transporte des données applicatives ou traverse un réseau non maîtrisé ; en pratique, `wss` est la forme attendue lorsque WebSocket est servi via HTTPS
