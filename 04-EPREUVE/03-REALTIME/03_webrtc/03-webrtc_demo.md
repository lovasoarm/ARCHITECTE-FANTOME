...
`RTCDataChannel` expose une API proche de WebSocket, mais ce n'est pas une WebSocket : le canal repose sur le transport WebRTC (SCTP sur DTLS, via ICE). Le trafic peut être direct ou relayé via TURN selon les conditions réseau. Le serveur de signaling n'est généralement pas dans le chemin des données applicatives, mais un relais TURN peut l'être.
...
<html lang="fr">
  ...
  <body>
    ...
    <input id="msg-input" valeur d’exemple="Message à Alfonso..." />
...
Un appel WebRTC comporte au minimum la coordination/signaling (ici via WebSocket) et le transport négocié par ICE. Le chemin de données peut être direct ou passer par TURN ; le signaling transporte ici SDP/candidats ICE mais n'est pas le transport des médias ou données applicatives.
...
Ce qui rate le plus souvent en prod : le TURN manquant pour les NATs symétriques (une fraction des utilisateurs peut voir `failed` lorsque le chemin direct échoue et qu'aucun relais TURN utilisable n'est disponible).
