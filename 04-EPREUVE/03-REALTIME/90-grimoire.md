...
| **WebRTC** (Web Real-Time Communication : communication web en temps réel)                  | API navigateur et ensemble de protocoles pour des communications temps réel entre pairs ; le transport peut être direct ou relayé selon ICE/TURN.                                                                                                                                    | `new RTCPeerConnection(config)`                                   | Scofield et Lincoln qui communiquent directement sans passer par le système téléphonique de Fox River : peer-to-peer, le flux ne traverse pas la prison / deux Chevaliers de Garo qui coordonnent un combat sans repasser par le Conseil : connexion directe, latence minimale                            | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
...
| **TURN** (Traversal Using Relays around NAT)                                                | Serveur relais WebRTC. Quand un chemin direct n’est pas possible, les flux peuvent passer par TURN. Cela améliore fortement la capacité à établir un chemin réseau dans certains environnements, au prix de bande passante et sans constituer une garantie universelle de connexion. | `{ urls: 'turn:srv.com', username, credential }`                  | Le serveur de signaling qui finit par transporter les données / l'intermédiaire qui fait passer les messages quand les deux partis ne peuvent pas communiquer directement                                                                                                                                 | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
...
| **RTCDataChannel**                                                                          | Canal de données bidirectionnel de WebRTC, avec une API proche de WebSocket ; il utilise le transport WebRTC et peut être direct ou relayé par TURN.                                                                                                                                 | `pc.createDataChannel('chat')`                                    | Un téléphone direct entre deux joueurs / une ligne rouge entre deux chefs d'état                                                                                                                                                                                                                          | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
...
| **Bitrate** (débit binaire)                                                                 | Quantité de données transmises par seconde (en bits/s). En WebRTC : plus le bitrate est élevé, meilleure est la qualité vidéo, plus la bande passante consommée est grande.                                                                                                          | `a=fmtp:<payload-type> maxplaybackrate=16000` (exemple codec/SDP) | La vitesse de transit des données / la résolution à laquelle Garo voit ses ennemis                                                                                                                                                                                                                        | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
...
| **Packet loss** (perte de paquets)                                                          | Pourcentage de paquets réseau qui n'arrivent pas à destination. En WebRTC, une perte de paquets croissante peut dégrader sensiblement la qualité perçue ; l'impact exact dépend du codec, du débit, de la correction d'erreurs et du contexte réseau. Visible via `getStats()`.      | `await pc.getStats()`                                             | Des balles qui n'atteignent pas la cible / des messages qui se perdent en chemin                                                                                                                                                                                                                          | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
...
- Ne pas avoir de serveur TURN : une partie des connexions peut échouer lorsque le chemin direct est impossible et qu'aucun relais TURN n'est disponible
...
- TURN souvent requis lorsque les chemins directs échouent, notamment dans certains environnements de NAT symétrique ; la politique ICE/TURN dépend du contexte réseau et des objectifs de disponibilité
