...
## Garder, supprimer, isoler

Face à du code ancien dont tu ne sais pas s'il sert encore, trois critères observables,
mesurables en quelques minutes chacun, permettent de trancher sans deviner :

```text
Critère 1 : date du dernier commit qui touche le fichier
  git log -1 --format="%ad" -- server/billing/legacyDiscount.ts
  --> plus de 18 mois sans modification : signal faible d'usage actif,
      mais pas suffisant seul (du code stable peut rester très utilisé)

Critère 2 : présence dans une trace de production sur 30 jours
  # selon l'outil de tracing/APM en place, chercher les appels de fonction
  # ou les requêtes à l'endpoint sur la fenêtre glissante des 30 derniers jours
  grep "legacyDiscount" /var/log/app/traces-30d.jsonl | wc -l
  --> zéro occurrence sur 30 jours pleins (pas juste un jour creux) :
      signal fort d'absence d'usage réel, au-delà de la présence dans le code

Critère 3 : existence d'un test qui couvre spécifiquement ce code
  rg -n "legacyDiscount" --type ts -g "*.test.ts" -g "*.spec.ts"
  --> aucun test trouvé : personne n'a jugé ce comportement assez important
      pour le protéger explicitement, ou personne n'ose plus y toucher
```

Règle de décision, à appliquer une fois les trois critères mesurés :

```text
Deux critères sur trois au vert (pas de commit récent, absent des traces de
production, sans test dédié) --> supprimer, avec une pull request qui cite
                                  les trois mesures dans sa description.

Un seul critère au vert        --> isoler derrière une frontière explicite
                                  (un module clairement nommé, un commentaire
                                  daté expliquant pourquoi le code reste), et
                                  dater la décision : "revu le 2026-03, gardé
                                  car présent dans 2 % des traces, revoir en
                                  2027 si le chiffre baisse encore."

Zéro critère au vert           --> code actif, ne pas y toucher sans ticket
                                  et sans cartographie complète du rayon
                                  d'impact (étapes 1 à 6 ci-dessus).
```

Dater la décision d'isolement n'est pas cosmétique : sans date, une frontière "temporaire"
devient indiscernable d'un choix d'architecture permanent au bout de deux ans, et personne
ne sait plus si elle mérite d'être réévaluée.
