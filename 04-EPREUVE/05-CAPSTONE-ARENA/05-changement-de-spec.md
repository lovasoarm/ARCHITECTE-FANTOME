...
**N'ouvre pas [`08-SCELLE-MESSAGE-CLIENT-JALON-2.md`](08-SCELLE-MESSAGE-CLIENT-JALON-2.md)
avant d'avoir figé ton schéma de données et horodaté ton ADR du jalon Architecture.** Ce
fichier scellé contient le message du client qui déclenche ce changement de spec. L'ouvrir
trop tôt fausse l'exercice : c'est équivalent à lire la conclusion d'une enquête avant d'avoir
posé une seule hypothèse.

Dès que ton schéma est figé et ton ADR daté, pose toi-même le déclencheur : ajoute dans
`05-TDD-JOURNAL.md` une entrée horodatée avant d'ouvrir l'enveloppe scellée, par exemple :

```text
2026-03-04 09:12 -- Schema fige, ADR-0002 date. Ouverture autorisee de l'enveloppe scellee.
```

Personne ne vérifie que tu as respecté cet ordre : c'est un déclencheur auto-administré, et sa
...
et une contrainte réelle qui n'apparaît qu'une fois qu'une structure concrète (schéma,
architecture) existe pour la révéler. Ce n'est pas une preuve d'incompétence, ni côté client
ni côté développeur : c'est un phénomène normal de tout projet réel, qui se mesure en fichiers
touchés et en heures (voir plus bas), pas une simple impression de "ça a changé".

## La scène

Ton jalon Architecture vient de passer le seuil de 12/20. Ton modèle de données est posé :
une table de créneaux, une table de réservations, un compteur de capacité par créneau. Une
semaine après avoir validé ton schéma de tarification unique, ouvre
[`08-SCELLE-MESSAGE-CLIENT-JALON-2.md`](08-SCELLE-MESSAGE-CLIENT-JALON-2.md) pour
lire le message exact du client fictif (ou de ton formateur).
...
Dans ton `05-TDD-JOURNAL.md` (voir `03-deliverables.md`), ajoute une entrée datée avec :
...
- **Refactorer en silence sans le documenter comme un coût du changement.** Symptôme : ton
  05-TDD-JOURNAL.md ne mentionne jamais ce changement de spec alors qu'il a occupé plusieurs
  heures du jalon 3.
...
Analogie : un changement de spec en cours de route, c'est une commande modifiée quand le plat est déjà au feu, et un déroutement décidé après l'appareillage.
Où l'analogie casse : le plat se refait en dix minutes, une décision d'architecture prise il y a trois jours ne se refait pas gratuitement.
...
`01-01-why-this-level.md` : un brief ambigu se réduit par itération, pas par un questionnaire
