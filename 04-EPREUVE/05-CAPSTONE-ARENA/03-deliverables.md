...
J1 Cadrage        (J0 a J4,   budget 4h)   condition : cahier des charges initial redige
                                            + ADR-001 redige et date, seuil 95-challenge.md >= 12/20
...
J2 Architecture   (J5 a J12,  budget 8h)   condition : schema de donnees fige + contrat d'API
                                            ...
                                            concurrence en place, seuil 95-challenge.md >= 12/20

J3 Livraison      (J13 a J32, budget 20h)  condition : POSTMORTEM.md redige + REVUE-DE-RISQUES.md
                                            signee et datee, seuil 95-challenge.md >= 60/100
```

Un jalon ne s'ouvre jamais avant que la condition du précédent soit remplie **et datée** dans
`05-TDD-JOURNAL.md` : un score atteint sans date de passage écrite ne compte pas comme un jalon
...
- Périmètre de la version 1 : ce qui est dedans, ce qui est explicitement reporté, et
  pourquoi (avec l'argument de coût d'opportunité du Module `01-CADRAGE/05-MVP-SPLIT`).
...
- Un schéma des grands composants (frontend, backend, base de données, services externes
  éventuels) au niveau du Module `02-CONSTRUCTION/15-ARCHI-LAB`.
...
- Un fichier `HYPOTHESES.md` à la racine du dépôt, documentant un vrai bug rencontré pendant
  ...
  [`../02-TOOL-CAVE/03-debugging-toolkit.md`](../02-TOOL-CAVE/03-debugging-toolkit.md) :
...
+-- ADR/
|   +-- ADR-0001-....md
|   +-- ADR-REFUS-<option-ecartee>.md
|   \-- ADR-000N-....md
+-- PREUVES/
|   +-- 06A-BUDGET-CLOUD.md
|   +-- NOTE-DIRECTION.md
|   \-- SECURITY_GATE_FILLED.md
+-- src/
|   \-- (code de la V1)
\-- tests/
...
- `02-CAHIER-DES-CHARGES.md` : reformulation finale du besoin, incluant les hypothèses du Livrable 1
...
- `05-TDD-JOURNAL.md` : journal daté des décisions de conception et des changements de spec, avec
...
- `REVUE-DE-RISQUES.md` : revue de risques du projet (sécurité, coûts, données personnelles),
  produite selon le gabarit du Module `04-EPREUVE/01-BONUS-VAULT` (voir `../01-BONUS-VAULT/95-challenge.md`).
- `HYPOTHESES.md` : le protocole d'enquête d'un vrai bug rencontré pendant le capstone, au
  ...
  200 exécutions (voir [`../02-TOOL-CAVE/03-debugging-toolkit.md`](../02-TOOL-CAVE/03-debugging-toolkit.md)).
...
- `ADR/` : un fichier par décision structurante, au format du Module `04-EPREUVE/01-BONUS-VAULT`
  (`../01-BONUS-VAULT/05-decision-templates.md`).
- `SLO.md` : l'objectif de service chiffré (fenêtre, budget d'erreur en requêtes ratées par
  semaine, alerte qui réveille) et le RTO mesuré au chronomètre lors d'un drill de restauration
  réel, exigés par [`06-addendum-staff-engineer.md`](06-addendum-staff-engineer.md).
- `ADR/ADR-REFUS-<option-ecartee>.md` : la décision refusée par écrit, avec son chiffre, la
  perte acceptée et le seuil de réouverture.
- `PREUVES/06A-BUDGET-CLOUD.md` : le budget cloud à trois paliers (100 / 10 000 / 1 000 000), une
  ligne par catégorie dont IA et egress, chaque prix sourcé et daté.
- `PREUVES/NOTE-DIRECTION.md` : la note d'une page à une direction non technique, dont les
  chiffres reprennent exactement ceux du budget.
- `PREUVES/SECURITY_GATE_FILLED.md` : le gate de sécurité du produit, rempli ligne à ligne
  selon [`09-SECURITY-GATE-FIL-ROUGE.md`](09-SECURITY-GATE-FIL-ROUGE.md) : preuve (fichier:ligne,
  test, capture) ou `N/A` motivé.
...
[`../01-BONUS-VAULT/04-security-cost-privacy.md`](../01-BONUS-VAULT/04-security-cost-privacy.md)
...
- **Traiter l'arborescence de livraison comme un détail de forme.** Symptôme : un fichier à la
  racine qui devrait être dans `ADR/`, ce qui plafonne le score à 0/340 quelle que soit la
  qualité du contenu.

<!-- CAPSTONE-DOUBLE-DERIVE:debut -->

### Semaine à DOUBLE DERIVE

Une semaine du calendrier est une semaine à DOUBLE DERIVE : périmètre et incident en même
temps. Elle rend un seul livrable de décision, décrit dans
[07-semaine-double-derive.md](07-semaine-double-derive.md).

<!-- CAPSTONE-DOUBLE-DERIVE:fin -->
