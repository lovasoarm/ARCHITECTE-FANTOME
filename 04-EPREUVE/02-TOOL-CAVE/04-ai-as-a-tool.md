...
Analogie : une IA, c'est un commis très rapide qui n'a jamais goûté le plat, et un équipier qui répond toujours, même sans savoir.
Où l'analogie casse : le commis se tait quand il ne sait pas, l'IA produit une réponse plausible avec le même aplomb qu'une réponse juste.
...
Une réponse d'IA générative n'est jamais un fait vérifié : c'est une hypothèse plausible,
produite avec la même confiance apparente qu'elle soit juste ou fausse. Le seul mécanisme
qui protège de ce risque est de déplacer volontairement l'effort économisé en production
vers un effort de vérification équivalent, jamais de le supprimer purement et simplement du
budget de la tâche.

## Exemple concret

Un développeur doit calculer la disponibilité de créneaux dans un cabinet vétérinaire avec
plusieurs praticiens et des durées de consultation variables selon le motif. Il demande
d'abord à l'IA, en prompt de génération directe, "écris une fonction qui calcule les
créneaux disponibles" : la réponse arrive en quatre secondes, propre, avec des tests
basiques qui passent, et suppose silencieusement que toutes les consultations durent
quinze minutes. Il livre. Deux semaines plus tard, un vétérinaire signale que les
consultations de chirurgie, qui durent quarante-cinq minutes, se voient proposer des
créneaux qui se chevauchent avec le rendez-vous suivant. En reformulant en prompt de
décision ("liste les hypothèses possibles sur la durée d'une consultation selon son motif,
et le risque de chacune avant d'écrire du code"), l'IA expose l'hypothèse de durée uniforme
comme l'une des options, avec son risque explicite : le développeur choisit alors une durée
paramétrée par motif, en connaissance de cause, avant même d'écrire une ligne.


## Piège technique : le contexte non fourni

L'IA générative ne connaît de ton projet que ce que tu lui montres dans la conversation en
cours. Elle ne sait pas que ton entreprise a une convention de nommage précise, que tel champ
de la base de données est en réalité un identifiant externe qu'il ne faut jamais recalculer,
ou que telle fonction a été volontairement simplifiée pour une raison métier non documentée
dans le code lui-même.

```bash
# Mauvaise pratique : demander une fonction sans contexte du projet
# "Ecris une fonction qui calcule le prix d'une reservation."

# Meilleure pratique : fournir le contexte minimal indispensable avant de demander
cat src/types/reservation.ts    # montre la vraie forme des données
cat src/regles-metier.md        # montre les règles déjà écrites, si elles existent
# puis seulement : "Ecris une fonction qui calcule le prix d'une reservation en
# respectant strictement ces types et ces regles, sans en inventer de nouvelles."
```

Sans ce contexte, l'IA comble les trous avec les patterns les plus courants de son corpus
d'entraînement : souvent raisonnables en général, souvent faux pour ton cas précis.

## Ce qui change réellement avec l'expérience

Un développeur junior qui découvre l'IA générative a tendance à lui déléguer la décision en
plus de l'exécution : il demande "fais-moi une fonction qui gère les réservations" et accepte
le résultat tel quel, faute de savoir quelles questions poser. Un développeur expérimenté
utilise l'IA différemment : il a déjà en tête les trois ou quatre pièges classiques de son
domaine (fuseaux horaires, arrondis monétaires, doublons de clé, cas limites de date), et il
formule sa demande pour que l'IA les adresse explicitement, ou il les vérifie lui-même après
coup point par point.

```text
Usage junior                            Usage expérimenté
-------------------------------------   ------------------------------------------
Délègue la décision ET l'exécution      Délègue l'exécution, garde la décision
Accepte le premier résultat qui         Compare mentalement le résultat à des
"a l'air" correct                       pièges déjà connus de son domaine
Découvre les cas limites en             Anticipe les cas limites avant de lire
production, par un incident             la réponse de l'IA
```

Ceci n'est pas une raison d'attendre d'avoir de l'expérience avant d'utiliser l'IA : c'est
une raison de construire, en parallèle de son usage de l'IA, sa propre liste de pièges
métier observés, pour progressivement basculer d'un usage junior vers un usage expérimenté.

## Ce que tu emportes

L'IA générative n'élimine pas le travail de compréhension du problème, elle le déplace vers
la vérification. Traite chaque réponse comme le travail d'un collaborateur junior très
rapide qui n'ose jamais dire qu'il ne sait pas : vérifie ses hypothèses implicites, force-la
à exposer ses choix avant qu'ils ne soient enterrés dans du code qui a l'air fini, et ne
colle jamais une logique que tu ne pourrais pas expliquer sans la relire. Le gain de vitesse
n'est réel que si l'effort économisé est réellement réinvesti dans cette vérification.
