...
const cleStripe = "<STRIPE_SECRET_KEY_REDACTED>";
...
### La ligne IA, calculée et non estimée

Une brique IA ne se met pas dans le même sac que les ordres de grandeur ci-dessus :
elle est variable par utilisateur, donc elle se calcule. Format complet dans le
contrat de production
04-BIG-APP-SNOOP/90_preuve de vérification dans le livrable staff.md ;
version courte, en prix d'exercice à remplacer par ton relevé daté :

| Élément          |                 Volume |    Prix d'exercice |       Coût |
| ---------------- | ---------------------: | -----------------: | ---------: |
| Entrée modèle    | 1 800 tokens / requête | 0,000003 € / token | 0,005400 € |
| Sortie modèle    |   450 tokens / requête | 0,000012 € / token | 0,005400 € |
| Requête complète |           2 250 tokens |                  : | 0,010800 € |

`20 requêtes / utilisateur actif / mois × 0,010800 = 0,216 € / utilisateur / mois`,
soit **2 160 € / mois pour 10 000 utilisateurs actifs** : deux ordres de grandeur
au-dessus de la base de données managée du tableau précédent. C'est la ligne qui
transforme une facture de 50 € en facture de 2 200 €, et la seule qui grandit
proportionnellement au succès du produit.

Dans `REVUE-DE-RISQUES.md`, la section coûts doit donc porter, pour toute brique
IA : le coût par utilisateur actif, le plafond mensuel vérifié **côté serveur**,
et le comportement produit quand le plafond est atteint (réponse déterministe
annoncée, jamais une erreur muette). Un plafond sans comportement de repli n'est
pas un plafond.
...
[95-challenge.md](95-challenge.md) et consommé par
[../05-CAPSTONE-ARENA/03-deliverables.md](../05-CAPSTONE-ARENA/03-deliverables.md). Il est
...
La colonne "Coût de la mesure" force une décision explicite : une mesure qui coûte plus
...
Analogie : la sécurité, les coûts et les données personnelles, c'est le contrôle des accès en coulisses de régie, et la gestion des stocks périssables en cuisine.
Où l'analogie casse : un stock périmé se jette ; une donnée personnelle mal gardée continue d'exister chez quelqu'un d'autre.
