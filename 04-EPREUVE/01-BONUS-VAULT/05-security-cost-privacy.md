...
app.get("/reservations/:id", requireAuth, async (req, res) => {
  ...
  const estAdminDeCetteSalle = req.user.role === "admin" && req.user.salleId === reservation.salleId;
...
const cleStripe = "sk_live_51H8xxxxxxxxxxxxxxxxxxxxxxxxxx";
...
async function effacerAdherent(db: Db, adherentId: string) {
  ...
  await db.reservations.deleteMany({ adherentId, dateDebut: { gt: new Date() } });
}
```
...
[../12-CAPSTONE-ARENA/03-deliverables.md](../06-CAPSTONE-ARENA/03-deliverables.md). Il est
