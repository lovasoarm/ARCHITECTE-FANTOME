...
## Règle bloquante : le rapport avant le code

Aucun commit, aucune branche, aucune modification locale enregistrée avant que `carte.md` soit
écrit, daté et poussé. La règle n'est pas une intention : elle est **vérifiée par
horodatage**.

```text
Preuve attendue, deux horodatages compares :
  1. date de commit de carte.md            (git log -1 --format=%cI -- carte.md)
  2. date du premier commit de code        (git log --reverse --format=%cI -- <zone touchee> | head -1)
Condition de validation : (1) < (2), ecart >= 45 min
```

Colle les deux sorties de commande dans le rapport. Si l'ordre est inversé, ou si l'écart est
inférieur à 45 minutes, le challenge est invalide même si le patch est correct : tu as codé
pendant que tu lisais, donc tu as validé tes hypothèses sur ce que tu croyais, pas sur ce que
le code fait.

Formulation courte, à retenir : aucune ligne modifiée tant que ton rapport d'exploration
n'est pas écrit. Critère vérifiable : l'horodatage de ton rapport est antérieur à celui de ta
première modification. Si tu ne peux pas le prouver, le challenge vaut 0. C'est la même
mécanique d'antériorité que la règle en tête de [96-boss-fight.md](96-boss-fight.md).

## Ce qui invalide le challenge

- Modifier une seule ligne de code avant que `carte.md` soit écrit et daté, ou fournir des
  horodatages qui ne respectent pas la condition ci-dessus.
...
5 heures au total, dans cet ordre strict.
Partie 1, cartographie : 3 heures maximum, chronomètre lancé.
Partie 2, patch : 2 heures maximum.
Si le patch dépasse 2 h : ta carte était incomplète. Le réflexe correct n'est pas de continuer
à patcher, c'est de retourner cartographier la zone que tu avais jugée secondaire.
