...
**SCÈNE CRAZYDEVS : Prison Break :** tu ne reconstruis pas toute la prison pour déplacer une porte. Tu réduis le risque, un changement vérifié à la fois, sans perdre les garanties existantes.

> **CrazyDevs : clan des ninjas :** une abstraction peut devenir une technique brillante ou une armure impossible à retirer. Le vrai skill est de savoir quand la jeter.
...
class CampManager {
  ...
  removeSurvivor(name) {
    this.survivors = this.survivors.filter((s) => s !== name);
  }
  addItem(item) {
  ...
  detectThreat(zombieCount) {
    this.threatLevel = zombieCount;
  }
  raiseAlarm() {
    /* ... */
  }
...
class PlayerCard {
  renderRating(match) {
    const total =
      match.stats.goals * 4 +
      match.stats.assists * 3 +
      match.stats.passes * 0.1;
...
function validateSuggestion(suggestion) {
  if (!suggestion.file || !suggestion.line || !suggestion.fix)
    return "suggestion incomplète";
  if (typeof suggestion.line !== "number" || suggestion.line < 0)
...
function canModerateVotes(journalist) {
  return (
    journalist.accreditedYears >= 3 &&
    journalist.country !== "banned" &&
