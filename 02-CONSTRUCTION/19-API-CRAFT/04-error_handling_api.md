...
Une API de production doit exposer un contrat d'erreur cohérent et versionnable pour chaque interface/protocole concerné. Le même vocabulaire peut être partagé entre REST, RPC, événements ou GraphQL, mais la forme exacte peut différer selon les contraintes du protocole.
...
Évite les formats ad hoc et documente un contrat stable. Une enveloppe commune est utile lorsqu'elle simplifie le client et l'observabilité, mais ne doit pas être imposée mécaniquement à tous les protocoles ou intégrations.

---

### Contrat minimal à stabiliser

Pour une interface HTTP JSON, stabilise au minimum : `status` HTTP, `code` machine, un `message` non sensible et, si nécessaire, des détails structurés validés par schéma. Pour GraphQL, gRPC, événements ou messages asynchrones, conserve ces invariants conceptuels mais adapte la représentation au protocole.
...
res
  .status(500)
  .json({ error: { code: "INTERNAL_ERROR", message: "erreur serveur" } });
```
...
Un contrat d'erreur stable et documenté est utile ; sa représentation exacte doit rester adaptée au protocole, au client et au niveau d'exposition. La stabilité porte surtout sur la sémantique (`code`, statut/protocole, message sûr, détails structurés), pas sur une enveloppe identique partout.
