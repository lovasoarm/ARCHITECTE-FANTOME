...
router.post("/", (req, res) => {
  ...
  res.status(201).set("Location", `/players/${newPlayer.id}`).json(newPlayer);
});
...
En pratique, PATCH est souvent utilisé pour les mises à jour partielles ; la répartition exacte dépend des conventions de l'API étudiée.
