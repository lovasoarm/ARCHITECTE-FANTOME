...
> **ARRÊTE-TOI ICI.** Ce fichier est un point de passage obligé entre `02-CONSTRUCTION/10_design_patterns` et `02-CONSTRUCTION/11_refactoring`. Ne l'ouvre pas comme "encore un chapitre" : c'est un palier de respiration avant un saut de nature. Si tu n'as pas encore fait la [mi-rétrospective du bloc](../MI-RETRO-BLOC-2-BUILD.md), fais-la avant ce pont : elle porte sur les dix modules que tu viens de terminer, pas sur ceux qui arrivent.
...
Prends une fonction de 40 lignes du curriculum, nomme 3 code smells, propose le premier refacto (le plus petit possible). Ne l'exécute pas : décris juste le mouvement. Si tu ne vois aucun smell : `02-CONSTRUCTION/11_refactoring/01_smells.md`.
