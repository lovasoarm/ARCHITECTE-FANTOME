...
> **ARRÊTE-TOI ICI.** Ce fichier est un point de passage obligé entre `02-CONSTRUCTION/11_refactoring` et `02-CONSTRUCTION/12_typescript`. Ne l'ouvre pas comme "encore un chapitre" : c'est un palier de respiration avant un saut de nature.
...
Prends `function add(a, b) { return a + b }`. Ajoute des types. Sans regarder : que se passe-t-il si tu appelles `add("1", 2)` en JS ? En TS strict ? Réponse : `"12"` en JS, erreur de compilation en TS. `02-CONSTRUCTION/12_typescript/01_type_basics.md`.
