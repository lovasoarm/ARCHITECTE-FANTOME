...
C'est de la partial application (application partielle : fixer une partie des arguments d'une fonction pour créer une version spécialisée). Vu en détail dans `../09-FUNCTIONAL-JS/06-partial_application.md`. `bind` est l'implémentation native en JS, sans bibliothèque.
