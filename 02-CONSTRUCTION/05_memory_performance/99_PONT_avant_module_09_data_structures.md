...
> **ARRÊTE-TOI ICI.** Ce fichier est un point de passage obligé entre `02-CONSTRUCTION/05_memory_performance` et `02-CONSTRUCTION/06_data_structures`. Ne l'ouvre pas comme "encore un chapitre" : c'est un palier de respiration avant un saut de nature.

## POURQUOI CE PONT EXISTE

Tu sais lire un profil mémoire, un flamegraph, une complexité en O(). Reste à choisir la structure qui te donne le bon O() dès le départ. `02-CONSTRUCTION/06_data_structures/` transforme ton intuition perf en décisions de conception.
...
Tu dois vérifier 50 000 IDs contre une whitelist de 20 000. Compare mentalement : `array.includes` vs `set.has`. Nomme la complexité totale de chaque, et le facteur de gain. Réponse : O(n·m) vs O(n+m). Si tu bloques, `02-CONSTRUCTION/05_memory_performance/03_complexity/`.
