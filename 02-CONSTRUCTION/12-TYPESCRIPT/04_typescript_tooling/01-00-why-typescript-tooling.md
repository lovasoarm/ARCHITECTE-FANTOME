...
Un projet TypeScript en production n'est généralement pas composé à 100% de code écrit par toi avec des types parfaits dès le départ. C'est un mélange : tes types, des types de libs externes, parfois du JS legacy pas encore migré. Ce module te donne les outils pour gérer ce mélange sans paniquer.
...
```text
tu écris du TS sur un projet greenfield (parti de zéro)   --> les bases (module `02-CONSTRUCTION/12-TYPESCRIPT`) suffisent
...
02-CONSTRUCTION/12-TYPESCRIPT complet --> types, interfaces, generics, utility types : la base doit être solide
06_modules       --> import/export, ESM vs CJS : indispensable pour comprendre la résolution de modules en TS
02-CONSTRUCTION/13-RUNTIME-ENV      --> comprendre Node et CommonJS aide à comprendre pourquoi certaines libs n'ont pas de types ESM propres
```

Ce qui devient plus simple après ce module :

```text
03-PILOTAGE/04-SECURITY       --> tu lis et comprends les types stricts des libs de sécurité sans paniquer
...
- confondre `type` et `interface` au moment d'écrire un .d.ts (vu dans le module `02-CONSTRUCTION/12-TYPESCRIPT`, mais l'erreur revient ici)
