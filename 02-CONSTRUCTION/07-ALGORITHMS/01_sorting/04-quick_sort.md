...
Merge sort offre une complexité O(n log n) prévisible et peut être stable. Quick sort a un bon comportement moyen et peut être très rapide en pratique, mais son coût dépend du choix du pivot, de la répartition des données et de l'implémentation ; son pire cas reste O(n²). Ne choisis donc pas Quick sort sur la seule intuition qu'il est « presque toujours » plus rapide.
...
Quick sort choisit un pivot, partitionne le tableau autour de lui, puis traite récursivement les sous-tableaux. Après une partition, le pivot atteint sa position définitive pour cette étape. La mémoire de pile dépend de la profondeur de récursion : elle est souvent logarithmique avec un bon choix de pivot, mais peut devenir linéaire dans le pire cas. Le temps d'exécution est en moyenne en O(n log n) et dans le pire cas en O(n²). Un pivot aléatoire réduit le risque pratique de déclencher systématiquement le pire cas, sans l'annuler. La stabilité et l'implémentation exacte d'un tri dans un moteur ou une bibliothèque ne doivent pas être déduites de cette fiche : elles dépendent du contrat de l'API et de l'implémentation concernée.
