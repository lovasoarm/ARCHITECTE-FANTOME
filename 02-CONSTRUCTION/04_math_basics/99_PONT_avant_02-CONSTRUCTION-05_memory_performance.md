...
Avant d'attaquer `02-CONSTRUCTION/05_memory_performance/` (heap snapshots, GC, closures qui gardent la mémoire vivante), il te faut une image nette de **comment un nombre, une chaîne, un objet vivent réellement en RAM**. Sans cette image, "fuite mémoire" reste un mot. Avec, tu vois les octets partir.
...
Si les 3 réponses coulent (8 octets ; 2 car même référence ; tant que la closure elle-même est référencée), tu peux ouvrir `02-CONSTRUCTION/05_memory_performance/`. Sinon, relis ce fichier.

---

## OÙ CE PONT S'INSÈRE

- Vient après : `02-CONSTRUCTION/04_math_basics/` complet (représentation numérique).
- Prépare : `02-CONSTRUCTION/05_memory_performance/00_why_memory_performance.md`, puis `02-CONSTRUCTION/05_memory_performance/01_gc/01_gc_basics.md`.
