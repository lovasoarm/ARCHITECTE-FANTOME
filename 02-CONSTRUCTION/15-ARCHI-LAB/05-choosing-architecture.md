...
Analogie : choisir entre monolithe modulaire et services, c'est décider si l'atelier de
...
## Le jour où ton appel de fonction devient un appel réseau

Tous les critères ci-dessus supposent une chose que personne n'écrit : que l'appel arrive, une
fois, dans l'ordre, ou échoue franchement. C'est vrai dans un processus. Ça cesse de l'être à
la seconde où la frontière devient un réseau. Une extraction de service ne déplace pas du code,
elle change la physique du système.

```text
Un processus                          Deux services
-----------                           -------------
appel de fonction                     requête réseau
echec = exception, immediate           echec = timeout, apres 30 s, sans reponse
0 ou 1 execution                       0, 1 ou N executions (retry)
ordre garanti                          ordre non garanti
transaction ACID locale                pas de transaction commune
etat coherent apres retour             coherence eventuelle, fenetre d'incoherence
cout d'un appel : ~100 ns              cout d'un appel : ~1 ms a 100 ms
```

Ce que ça implique, concrètement, dès la première frontière réseau :

- **Timeout.** Un appel sans timeout n'échoue jamais, il attend. Une attente non bornée
  remonte la pile et bloque des connexions en amont : la panne d'un service secondaire devient
  l'indisponibilité du service principal. Tout appel sortant a un budget de temps explicite.
- **Retry.** Réessayer transforme « échec » en « peut-être exécuté deux fois ». Un retry
  aveugle sur une opération non idempotente crée des doublons : deux commandes, deux
  prélèvements. Le retry n'est acceptable qu'avec une clé d'idempotence côté serveur, plus un
  recul exponentiel et une limite de tentatives, sinon la charge de retry achève le service
  déjà en difficulté.
- **Doublon et ordre.** Deux messages émis dans l'ordre A puis B peuvent arriver B puis A, ou
  A deux fois. Toute logique qui suppose l'ordre doit porter un numéro de version ou un
  horodatage source, et rejeter ce qui est plus vieux que l'état courant.
- **Cohérence éventuelle.** Après une écriture, une lecture sur un autre service peut renvoyer
  l'ancienne valeur pendant un temps. Ce n'est pas un bug à corriger, c'est un choix à assumer :
  soit tu rends la fenêtre visible à l'utilisateur (« en cours de traitement »), soit tu paies
  le coût d'une lecture forte au bon endroit.
- **Panne partielle.** Le mode d'échec dominant n'est plus « tout est mort », c'est « un
  morceau répond lentement ». C'est pire : sans disjoncteur ni cloisonnement des pools, une
  lenteur locale se propage à l'ensemble du système.
- **Coût réel d'une transaction distribuée.** Il n'y a pas de `COMMIT` commun. Tu remplaces une
  transaction par une saga : une suite d'étapes avec, pour chacune, une compensation écrite à
  la main, testée, et déclenchable après coup. Compte le travail de compensation dans le coût
  de l'extraction, pas seulement le déploiement.

Règle de décision : si tu ne sais pas encore écrire le timeout, la clé d'idempotence et la
compensation d'une étape, la frontière n'est pas prête à devenir un réseau. Garde-la comme
