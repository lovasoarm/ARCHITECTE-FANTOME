...
**SCÈNE CRAZYDEVS : Naruto :** choisir une structure de données, c’est choisir comment les ninjas entrent, sortent et se retrouvent dans le village. La mauvaise structure devient un goulot d’étranglement déguisé.
...
// { Rick:0, le sniper:1, Glenn:2, Michonne:3, Maggie:4, Carl:5, Hershel:6 }

const uf = new UnionFind(survivors.length);

// connexions découvertes au fil du temps
uf.union(idx["Rick"], idx["le sniper"]);
...
// questions
uf.connected(idx["le sniper"], idx["Carl"]); // true : même camp Rick
...
uf.connected(idx["le sniper"], idx["Hershel"]); // true : maintenant un seul camp
uf.count; // 1
```
