...
En production, les mutations d'état partagé et les dépendances implicites figurent parmi les sources fréquentes de complexité et de bugs ; la proportion exacte dépend du système étudié. Une fonction pure ne peut pas faire ça par définition.
...
- **composable** : tu peux la combiner avec n'importe quelle autre (→ voir `04-composition.md`)
...
```js
const survivants = [
  { nom: "le sniper", force: 85, estCombattant: false },
