...
const calculerScore = (joueurs) =>
  joueurs.map((j) => ({
    ...j,
    score: j.buts * 0.5 + j.passes * 0.3 + (j.aCL ? 30 : 0),
  }));
const trierParScore = (joueurs) =>
  [...joueurs].sort((a, b) => b.score - a.score);
const prendreTop = (n) => (joueurs) => joueurs.slice(0, n);

// assemblage lisible comme une phrase
const top3 = prendreTop(3)(
  trierParScore(calculerScore(filtrerActifs(joueurs))),
);
```
...
const filtrerParSeuil = (joueurs, seuil) =>
...
Le currying est couvert en détail dans `05-currying.md`. L'idée ici : pour composer, une fonction = un argument.
...
const trierParPriorite = (ms) =>
  [...ms].sort((a, b) => b.priorite - a.priorite);
const formaterDispatch = (ms) =>
  ms.map((m) => ({
    ...m,
