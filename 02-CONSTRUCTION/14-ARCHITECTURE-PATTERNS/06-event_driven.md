...
**SCÈNE CRAZYDEVS : Attack on Titan :** les murs doivent protéger sans enfermer l’organisation. Une frontière architecturale utile paie son coût en autonomie, sécurité ou changement futur.
...
eventBus.emit("mission.started", {
  location: "Osaka",
  threat: "Horror B-class",
});
...
Risque : une promesse rejetée dans un listener async peut devenir une erreur non observée selon la configuration de l’EventEmitter et le mécanisme de capture des rejets.
Solution : définir explicitement la stratégie d’erreur du bus (gestion locale, capture des rejets, ou wrapper centralisé) et tester le chemin d’échec attendu ; le `try/catch` local n’est qu’une option parmi d’autres.
