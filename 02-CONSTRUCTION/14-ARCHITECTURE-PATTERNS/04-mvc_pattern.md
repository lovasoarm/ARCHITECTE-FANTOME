...
Sans MVC ou séparation équivalente, la présentation, l'orchestration et les règles métier peuvent devenir fortement couplées. Cela rend certains changements et tests plus coûteux ; ce n'est pas une impossibilité absolue, mais un risque de couplage qu'une bonne architecture cherche à réduire.

Prérequis : `00-SOCLE/04-FUNDAMENTALS` complet, `02-CONSTRUCTION/10-DESIGN-PATTERNS` (notamment observer et factory).
...
model.add("le sniper de la horde (univers Walking Dead)", "Scout");
```
...
```text
Rick clique "Éliminer" sur le sniper
    |
    v
...
le sniper est éliminé. Alexandria continue. Le cycle se ferme proprement.
...
Au lieu que le Controller appelle `view.render()` manuellement, le Model peut émettre des événements (Observer Pattern de `02-CONSTRUCTION/10-DESIGN-PATTERNS`) :
