...
queue.publish("mission.assigned", {
  missionId: "MSN-007",
  shinobiId: "S-001",
  rank: "B",
});
...
```text
OrderService --|
        |--> [Shared DB]  <-- COUPLAGE À EXPLICITER
UserService  --|
```
...
Heuristique par défaut : chaque service doit posséder clairement ses données et son contrat d’accès. Un stockage partagé n’est pas automatiquement interdit, mais il crée un couplage de données qui doit être explicitement justifié, mesuré et gouverné.
...
```text
SIGNaux qui poussent à ne PAS choisir des microservices par défaut :
- l'équipe ne dispose pas encore du temps ou de l'autonomie opérationnelle pour posséder plusieurs services
- le monolithe n'a pas de problème démontré de frontières, déploiement ou montée en charge
- les domaines et responsabilités ne sont pas encore suffisamment compris
- l'observabilité, la gestion des incidents et les contrats de service ne sont pas maîtrisés

Signaux qui peuvent justifier des microservices :
- plusieurs équipes ont des frontières d'ownership réellement distinctes
- certains domaines ont des besoins de scale ou de déploiement fortement différents
- les contraintes de disponibilité, sécurité ou évolution justifient une isolation
- un monolithe fonctionne déjà mais ses frontières de changement deviennent un goulot mesuré
```
...
Le principe à retenir : chaque service doit avoir un owner explicite de ses données. Une base partagée peut être un choix temporaire ou délibéré dans certains systèmes ; elle doit alors expliciter le couplage, les droits, les migrations et le plan d'évolution.
