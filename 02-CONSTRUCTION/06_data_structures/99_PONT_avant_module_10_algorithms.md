...
> **ARRÊTE-TOI ICI.** Ce fichier est un point de passage obligé entre `02-CONSTRUCTION/06_data_structures` et `02-CONSTRUCTION/07_algorithms`. Ne l'ouvre pas comme "encore un chapitre" : c'est un palier de respiration avant un saut de nature.
...
Écris `factorial(n)` en récursif puis en itératif. Quelle version explose à `n = 100000` ? Pourquoi ? Réponse : la récursive, sans TCO. `02-CONSTRUCTION/07_algorithms/03_recursion.md` détaille.
