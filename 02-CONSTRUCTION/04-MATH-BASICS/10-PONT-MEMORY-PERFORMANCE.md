...
Avant d'attaquer `02-CONSTRUCTION/05-MEMORY-PERFORMANCE/` (heap snapshots, GC, closures qui gardent la mémoire vivante), il te faut une image nette de **comment un nombre, une chaîne, un objet vivent réellement en RAM**. Sans cette image, "fuite mémoire" reste un mot. Avec, tu vois les octets partir.

## POURQUOI CE PONT EXISTE

Le module `02-CONSTRUCTION/04-MATH-BASICS` t'a donné les mathématiques (arithmétique, modulo, virgule flottante). Le module `02-CONSTRUCTION/05-MEMORY-PERFORMANCE` va te demander de raisonner en **octets, références, allocations**. Le saut est réel : passer de "5 + 3" à "cette closure garde vivante une variable de 5 Mo pendant 20 minutes" perd tout le monde s'il n'y a pas de palier.
...
Conséquence directe : `0.1 + 0.2 !== 0.3`. Ce n'est pas un bug, c'est le format. Le module `02-CONSTRUCTION/04-MATH-BASICS` l'a montré ; le module `02-CONSTRUCTION/05-MEMORY-PERFORMANCE` t'expliquera comment ces 8 octets s'empilent quand tu crées un million de `Number`.
...
Pour le modèle JavaScript, raisonne en termes de valeurs, bindings et références partagées. La séparation « primitives = stack, objets = heap » est une intuition de certaines implémentations, pas une garantie du langage.

Cette distinction est **la** clé pour comprendre les fuites mémoire du module `02-CONSTRUCTION/05-MEMORY-PERFORMANCE` : une closure qui garde une référence maintient l'objet vivant dans le heap, même si le code qui l'a créé est terminé.
...
Si les 3 réponses coulent (8 octets ; 2 car même référence ; tant que la closure elle-même est référencée), tu peux ouvrir `02-CONSTRUCTION/05-MEMORY-PERFORMANCE/`. Sinon, relis ce fichier.

---

## OÙ CE PONT S'INSÈRE

- Vient après : `02-CONSTRUCTION/04-MATH-BASICS/` complet (représentation numérique).
- Prépare : `02-CONSTRUCTION/05-MEMORY-PERFORMANCE/01A-00-why-memory-performance.md`, puis `02-CONSTRUCTION/05-MEMORY-PERFORMANCE/01_gc/02-gc_basics.md`.
- N'introduit pas d'API nouvelle. Uniquement le **modèle mental** que le reste du module `02-CONSTRUCTION/05-MEMORY-PERFORMANCE` utilisera.
