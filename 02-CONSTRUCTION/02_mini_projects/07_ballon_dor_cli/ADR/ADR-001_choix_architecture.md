...
Le projet est un outil CLI Node.js. Il couvre `02-CONSTRUCTION/13_runtime_env` (process.argv, filesystem, Worker Threads) et `01-CADRAGE/04_error_handling`. Ce n'est pas une API avec des connexions persistantes : chaque commande est un processus Node qui démarre, lit/écrit, et se termine.
...
- Rejeté parce que : ce projet enseigne le filesystem Node.js (`fs`, `path`, streams) : utiliser SQLite contourne l'apprentissage central du module `02-CONSTRUCTION/13_runtime_env`
...
- `fs.readFileSync` et `fs.writeFileSync` sont les APIs centrales de `02-CONSTRUCTION/13_runtime_env` : leur usage sur de vraies données les ancre dans un contexte concret
