...
| ------------------ | ---------------------------------------------------------------------------- |
| `03-PILOTAGE/05_observability` | `logger.ts` (JSON structuré, correlation ID), `tracer.ts`, `sentryClient.ts` |
| `05-MAITRISE/02_scalability`  | rate limiting sur l'endpoint live, simulation de load horizontal       |
| `02-CONSTRUCTION/12_typescript`  | `Event<T>`, `Pipeline<I,O>`, utility types sur les structs d'events     |
...
Liste generee : tout fichier de `02-CONSTRUCTION/02_mini_projects/06_ultras_dashboard` est joignable depuis ici, aucun document n'est laisse sans porte d'entree.
