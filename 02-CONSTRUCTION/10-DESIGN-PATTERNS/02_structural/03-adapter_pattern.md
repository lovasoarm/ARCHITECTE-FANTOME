...
class OptaAPI {
  ...
  fetchScore() {
    return `${this.data.home_goals}-${this.data.away_goals}`;
  }
  fetchTopPerformer() {
    return this.data.best_player;
  }
  fetchTacticalSetup() {
    return this.data.formation_code;
  }
}
...
class StatsBombAPI {
  ...
  getResult() {
    return { home: this.data.score[0], away: this.data.score[1] };
  }
  getMVP() {
    return this.data.players.find(
      (p) => p.rating === Math.max(...this.data.players.map((p) => p.rating)),
    );
  }
  getTactics() {
    return this.data.tactics.formation;
  }
}
...
const sbMatch = new StatsBombAPI({
  score: [2, 1],
  players: [
    { name: "Messi", rating: 9.2 },
    { name: "Ronaldo", rating: 8.1 },
  ],
...
Parfois les données doivent aller dans les deux sens : lire _et_ écrire vers une source externe.
