...
**SCÈNE CRAZYDEVS : Attack on Titan :** une formation militaire n’est pas un uniforme. Un pattern n’est pas une décoration. Il n’a de valeur que lorsqu’il résout la contrainte pour laquelle il existe.
...
function createGuerrierBuilder(name) {
  ...
  const builder = {
    ...
    powerLevel(lvl) {
      if (lvl < 1)
        throw new Error(
          `${name} ne peut pas avoir un power level négatif. Même Yamcha fait mieux.`,
        );
    ...
    build() {
      ...
      if (config.powerLevel > 9000 && config.transformation === null) {
        console.warn(
          `${config.name} dépasse 9000 sans transformation : c'est suspect.`,
        );
      }
...
const yamcha = createGuerrierBuilder("Yamcha").powerLevel(200).build();
...
class MatchBuilder {
  ...
  vipTickets(count) {
    if (count < 0)
      throw new Error(
        "Nombre de billets VIP négatif : t'es en train d'inventer.",
      );
  ...
  weather(condition) {
    ...
    if (!valid.includes(condition))
      throw new Error(`Météo inconnue : ${condition}`);
  ...
  build() {
    ...
    if (this._homeTeam === this._awayTeam)
      throw new Error(
        "Un club ne joue pas contre lui-même. Sauf en entraînement.",
      );
...
const friendly = new MatchBuilder().homeTeam("PSG").awayTeam("Lyon").build();
```
...
const wcFinal = director.buildWorldCupFinal("France", "Brésil", {
