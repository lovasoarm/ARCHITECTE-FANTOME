...
# Grimoire : Module 02-CONSTRUCTION/20-API-DOJO, API Dojo
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- |
| Contrat d'API             | L'ensemble des garanties sur lesquelles un client a le droit de s'appuyer sans risquer une casse au prochain déploiement. | `openapi: 3.1.0\npaths:\n  /reservations:\n    get: {}`                  | menu affiché qui engage la cuisine sur ce qui est réellement servi / carte marine qui engage sur les hauts-fonds réellement présents             | L'analogie s'arrête au cas décrit : hors de ce cas, reviens à la définition. |
| Compatibilité ascendante  | Un client ancien continue de fonctionner face à un serveur nouveau.                                                       | `// champ ajoute, jamais retire : ancien client ignore le nouveau champ` | un vieux ticket de commande reste lisible même avec la nouvelle carte / un vieux plan de route reste valide même avec les nouvelles balises      | L'analogie s'arrête au cas décrit : hors de ce cas, reviens à la définition. |
| Compatibilité descendante | Un client nouveau continue de fonctionner face à un serveur ancien.                                                       | `// le nouveau client tolere l'absence du nouveau champ cote serveur`    | la nouvelle recrue sait encore lire l'ancien système de tickets / le nouveau matelot sait encore lire le vieux livre de bord                     | L'analogie s'arrête au cas décrit : hors de ce cas, reviens à la définition. |
| Idempotence               | Une opération répétée produit le même résultat que l'exécution unique.                                                    | `if (dejaTraite(cle)) return reponsePrecedente(cle);`                    | un même bon de commande renvoyé deux fois ne fait pas deux plats / une même manœuvre rejouée ne double pas le nœud                               | L'analogie s'arrête au cas décrit : hors de ce cas, reviens à la définition. |
| Idempotency key           | Identifiant généré côté client à l'intention, pas à chaque tentative réseau.                                              | `const cle = crypto.randomUUID(); // genere une fois, avant tout retry`  | numéro de ticket pris une fois en salle, pas à chaque passage en cuisine / numéro de manœuvre décidé une fois par le skipper, pas à chaque essai | L'analogie s'arrête au cas décrit : hors de ce cas, reviens à la définition. |
| Authn / Authz             | Authn vérifie qui appelle ; Authz vérifie ce que cet appelant a le droit de faire.                                        | `if (!user) throw 401; if (!user.peut("annuler")) throw 403;`            | badge du personnel vs droit d'accès à la réserve du chef / identité du matelot vs droit de toucher à la barre                                    | L'analogie s'arrête au cas décrit : hors de ce cas, reviens à la définition. |
| Rate limiting             | Limitation du débit d'appels d'un client.                                                                                 | `if (compteur(clientId) > seuilParSeconde) throw 429;`                   | cuisine qui refuse de prendre plus de commandes que sa cadence / port qui limite le nombre de bateaux entrant en même temps                      | L'analogie s'arrête au cas décrit : hors de ce cas, reviens à la définition. |
| Latence perçue            | Le temps d'attente ressenti, distinct du temps de réponse mesuré côté serveur.                                            | `afficherAccuseReception(); await requeteReelle();`                      | accusé de réception immédiat en salle pendant que la cuisine prépare / retour radio immédiat pendant que la manœuvre s'exécute                   | L'analogie s'arrête au cas décrit : hors de ce cas, reviens à la définition. |
...
l'echec se reproduit sur le meme critère, redescends au 15-ARCHI-LAB relire "frontiere de
