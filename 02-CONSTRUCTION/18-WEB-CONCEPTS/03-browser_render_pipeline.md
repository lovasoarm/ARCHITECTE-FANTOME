...
**SCÈNE CRAZYDEVS : Attack on Titan :** HTTP est une ville avec ses portes, ses règles de passage et ses messages. Comprendre le protocole évite de traiter le réseau comme une boîte noire.
...
Pour de nombreuses animations, `transform` et `opacity` sont de bons candidats car ils permettent souvent d'éviter un nouveau layout. Ce n'est pas une loi universelle : le coût dépend de la propriété, du contexte, du navigateur et de l'effet visuel. Mesure avec les outils de performance avant de conclure.
...
Google mesure trois métriques (module `02-CONSTRUCTION/05-MEMORY-PERFORMANCE` pour le détail, résumé rapide ici) :
