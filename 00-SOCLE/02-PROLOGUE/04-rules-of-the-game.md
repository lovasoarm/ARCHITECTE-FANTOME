...
Exemple chiffré : sur le système de refacturation de Karim, une mesure concrète aurait été
"moins de 2% des factures annuelles génèrent une contestation dans les 60 jours". Sans ce
chiffre écrit avant la mise en production, personne ne peut dire si la version actuelle est
meilleure ou pire que la précédente : chacun se contente d'une impression.
...
### Comment les 7 règles s'appliquent à une seule décision, de bout en bout

Prenons le cas de Karim, rejoué correctement :

```text
1. LIVRER    : une première version de la refacturation avec la clé 2023, en production.
2. MESURER   : "moins de 2% de contestations dans les 60 jours" écrit avant le lancement.
3. ECRIRE    : ADR "clé de répartition externalisée dans une table versionnée par année,
               plutôt que codée en dur, car le syndic vote une nouvelle clé chaque année."
4. DEFENDRE  : face à l'objection "une table de plus complique le schéma", réponse : "le
               coût d'une migration de code chaque année est plus élevé que celui d'une
               ligne de configuration versionnée."
5. JETER     : si le vote annuel de clé s'avère en réalité toujours identique après 3 ans,
               envisager de simplifier (mais avec preuve écrite du signal, pas une intuition).
6. DOCUMENTER: l'ADR liste l'option rejetée (clé en dur) et pourquoi.
7. REVENIR   : condition de réexamen inscrite dans l'ADR : "si le volume de litiges dépasse
               5% une année donnée, revoir le mode de calcul, pas seulement la clé."
```
...
- Vouloir appliquer les 7 règles avec la même rigueur sur une décision triviale que sur une
  décision structurante : le symptôme est un projet paralysé par la procédure sur des choix
  qui ne le méritaient pas.

## Analogie

Analogie : les 7 règles du joueur sont les consignes de sécurité d'un atelier, et les règles de quart en navigation.
Où l'analogie casse : à l'atelier une règle protège un doigt, ici elle protège un jugement, et rien ne saigne quand tu la violes.
