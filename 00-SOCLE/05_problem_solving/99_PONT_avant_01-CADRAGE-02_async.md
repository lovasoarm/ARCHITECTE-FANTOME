...
> **ARRÊTE-TOI ICI.** Ce fichier est un point de passage obligé entre `00-SOCLE/05_problem_solving` et `01-CADRAGE/02_async`. Ne l'ouvre pas comme "encore un chapitre" : c'est un palier de respiration avant un saut de nature.

## POURQUOI CE PONT EXISTE

Tu viens de modéliser des états, invariants, transitions. `01-CADRAGE/02_async/` ajoute une dimension : le temps qui passe entre deux états. Un modèle statique juste devient un modèle faux dès qu'on introduit une attente réseau, un timer, un handler d'événement.
...
console.log('E');
```
Réponse attendue : `A E B D C`. Si tu hésites, ouvre `01-CADRAGE/02_async/04_event_loop/`.
