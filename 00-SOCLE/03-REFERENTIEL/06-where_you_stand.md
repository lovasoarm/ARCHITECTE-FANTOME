...
Ce référentiel sert à te situer avant, pendant et après un bloc de travail. Tu peux le consulter rapidement au démarrage, puis revenir aux axes détaillés après les premières épreuves.
...
ce parcours construit quatre capacités. Pas quatre listes de méthodes. Quatre façons de penser.
...
> **Correspondance des cadres** : le parcours utilise deux niveaux de lecture complémentaires. Les six pierres servent à donner une vue synthétique du socle ; les quatre axes ci-dessous servent à diagnostiquer ta maîtrise en pratique. La pensée transférable traverse les quatre axes au lieu de constituer une case séparée.
...
**Modules qui construisent cet axe :** `01-CADRAGE/02-ASYNC` (event loop, callbacks, promises), `02-CONSTRUCTION/05-MEMORY-PERFORMANCE` (GC, profiling), `02-CONSTRUCTION/13-RUNTIME-ENV` (Node vs navigateur).
...
Lire du code, c'est invisible. Personne te demande "t'as bien lu ?". Et pourtant, lire du code que tu n'as pas écrit occupe une part importante du métier.
...
**Modules qui construisent cet axe :** `00-SOCLE/05-PROBLEM-SOLVING` (décomposition, modélisation), `03-PILOTAGE/10-TEAM-CRAFT` (navigation de codebase).
...
**Modules qui construisent cet axe :** `01-CADRAGE/04-ERROR-HANDLING` (propagation, stratégies), `02-CONSTRUCTION/03-TESTING` (TDD, isolation), `03-PILOTAGE/05-OBSERVABILITY` (debug en prod).
...
**Modules qui construisent cet axe :** `02-CONSTRUCTION/10-DESIGN-PATTERNS`, `02-CONSTRUCTION/11-REFACTORING`, `02-CONSTRUCTION/14-ARCHITECTURE-PATTERNS`.
...
Ces profils sont des repères, pas des objectifs à cocher. Si t'es Runtime N3 dès le module `01-CADRAGE/03-DEBUGGING` parce que t'as déjà fait du Node avant : tant mieux, avance.
...
T'as 54 modules pédagogiques directs devant toi. Si tu regardes la montagne entière, tu vas paniquer. Donc :
...
- **Les mini-projets (`02-CONSTRUCTION/02-MINI-PROJECTS`) ne sont pas un bonus.** C'est là que les concepts
...
- **Stagner sur un concept, c'est normal.** Closures, event loop, prototype chain : tout le
  monde galère dessus la première fois. Le `../04-FUNDAMENTALS/02_scope/05-scope_escape_room.md` et compagnie
...
- **L'event loop, le call stack, le cycle HTTP : tu vas les croiser dans plusieurs modules.**
  `05-MAITRISE/06-ANNEXES/03-ascii_charte.md` regroupe les 8 schémas canoniques. Même schéma partout, pas
...
- **Tu cherches un fichier précis et tu sais pas dans quel module il est ?**
  `../../05-MAITRISE/06-ANNEXES/16A-ARBORESCENCE.md` liste tout, dossier par dossier, fichier par
...
```text
01-CADRAGE/02-ASYNC       => l'event loop tord le cerveau la première fois, c'est normal
02-CONSTRUCTION/05-MEMORY-PERFORMANCE => beaucoup de mécanique invisible (GC, profiling) à visualiser sans la voir tourner
02-CONSTRUCTION/06-DATA-STRUCTURES  => 02-CONSTRUCTION/07-ALGORITHMS en dépend direct, bâcler l'un casse l'autre
02-CONSTRUCTION/12-TYPESCRIPT     => le système de types a son propre raisonnement, différent de JS
02-CONSTRUCTION/14-ARCHITECTURE-PATTERNS => abstrait par nature, les exemples concrets sont indispensables ici
03-PILOTAGE/04-SECURITY      => chaque sous-thème (XSS, CSRF, auth) est dense seul, ne pas les fusionner
02-CONSTRUCTION/17-OOP-JS       => la chaîne de prototype clarifie tout, mais seulement si tu ne la survoles pas
```
