...
Module : **05-MAITRISE/04_ai_agents_and_autonomy** : agents IA autonomes.
...
- **Python** : Python : agents avec `langgraph`/`autogen` ; boucle perception-decision-action identique ; memes garde-fous (budgets, retries, kill-switch).
- **Go** : Go : agents pilotables via `langchaingo` ou custom ; concurrence native aide les boucles perception-action.
- **Rust** : Rust : agents rares mais possibles ; memes garde-fous ; utile quand l'agent doit tourner en embarque ou avec contraintes de latence.
