...
> **CE MODULE RÉUTILISE** : mémoire (02-CONSTRUCTION/05-MEMORY-PERFORMANCE), async (01-CADRAGE/02-ASYNC), DB (05-MAITRISE/01-DATABASES), architecture (02-CONSTRUCTION/14-ARCHITECTURE-PATTERNS). Si un de ces prérequis est flou, retourne le voir avant. Ce module ne les réexplique pas.
...
- une API REST fonctionnelle : voir `02-CONSTRUCTION/19-API-CRAFT/`
- sécurité de base (rate limiting, auth) : voir `03-PILOTAGE/04-SECURITY/`
- comment les responsabilités se découpent entre services : voir `02-CONSTRUCTION/14-ARCHITECTURE-PATTERNS/`
- bases de l'observabilité : voir `03-PILOTAGE/05-OBSERVABILITY/` : ou lire les deux en parallèle
...
Central dans le mini-projet `06_ultras_dashboard`, qui combine `03-PILOTAGE/05-OBSERVABILITY`, `05-MAITRISE/02-SCALABILITY`, et `02-CONSTRUCTION/12-TYPESCRIPT` pour un système qui doit gérer des milliers d'utilisateurs connectés simultanément sans jamais tomber pendant un match en direct, ce qui rend la scalabilité une condition de survie du produit, pas un bonus technique.
...
> Ce module réutilise : la performance du `02-CONSTRUCTION/05-MEMORY-PERFORMANCE`, l'architecture du `02-CONSTRUCTION/14-ARCHITECTURE-PATTERNS`.
