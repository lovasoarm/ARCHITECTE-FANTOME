...
**SCÈNE CRAZYDEVS : Naruto :** le jutsu marche 99 fois. La centième fois, l’horloge change de date et le village entier tombe en erreur. Les edge cases sont les combats que les démos évitent.
...
> **Prérequis :** ce fichier suppose que tu sais ce qu'est un prototype et comment fonctionne l'héritage en JS. Si ce n'est pas encore le cas, le cours complet est dans `../../02-CONSTRUCTION/17-OOP-JS/02-prototype_chain_raw.md`. Tu peux quand même lire ce qui suit avec le rappel express ci-dessous, mais le module `02-CONSTRUCTION/17-OOP-JS` reste la référence pour les bases.

Ce fichier couvre ce qui arrive quand ce mécanisme est exploité, mal utilisé, ou carrément détourné.
`__proto__`, `hasOwnProperty`, et prototype pollution : trois sujets où une compréhension superficielle peut suffire à laisser passer un bug de sécurité ou de maintenabilité.
