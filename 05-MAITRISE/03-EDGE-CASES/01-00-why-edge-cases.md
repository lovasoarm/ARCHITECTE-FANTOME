...
> **CE MODULE RÉUTILISE** : tests (02-CONSTRUCTION/03-TESTING), debugging (01-CADRAGE/03-DEBUGGING), erreurs (01-CADRAGE/04-ERROR-HANDLING), math (02-CONSTRUCTION/04-MATH-BASICS). Si un de ces prérequis est flou, retourne le voir avant. Ce module ne les réexplique pas.
...
Périphérique dans la progression (module `05-MAITRISE/03-EDGE-CASES` sur 32).
Mais les comportements couverts ici sont présents depuis le module `00-SOCLE/04-FUNDAMENTALS`.
...
--------------------------------------
00-SOCLE/04-FUNDAMENTALS/04_types  --> les types primitifs et leur comportement de base
00-SOCLE/04-FUNDAMENTALS/01_variables --> références vs valeurs : base pour comprendre la mutation
02-CONSTRUCTION/09-FUNCTIONAL-JS/02_immutability --> pourquoi la mutation implicite est dangereuse
02-CONSTRUCTION/12-TYPESCRIPT        --> ce que TS attrape et ce qu'il laisse passer
03-PILOTAGE/04-SECURITY         --> prototype pollution comme vecteur d'attaque réel
```
...
- `00-SOCLE/04-FUNDAMENTALS` complet : types primitifs, références, coercions de base
- `../../00-SOCLE/04-FUNDAMENTALS/04_types/03-type_coercion.md` en particulier : base directe de ce module
...
**`03-PILOTAGE/04-SECURITY` (si pas encore fait) :** la pollution de prototype comme vecteur d'attaque est plus claire après avoir compris la chaîne prototype en profondeur.
...
> Ce module réutilise : le debugging du `01-CADRAGE/03-DEBUGGING` et l'asynchrone du `01-CADRAGE/02-ASYNC`.
>
> **Référence heisenbug** : le fichier `07-heisenbug_arena.md` de ce module est LE point de référence complet sur les heisenbugs (5 scénarios, méthode, cas qui casse). Le stub `../../01-CADRAGE/03-DEBUGGING/11-heisenbug_arena.md` renvoie ici : ne le duplique pas, approfondis-le.
