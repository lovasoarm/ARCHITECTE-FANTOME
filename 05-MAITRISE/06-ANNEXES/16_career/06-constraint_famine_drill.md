...
# 05 : solution Famine Drill : simulation de survie technologique
...
> volatilise. Tu recuperes une machine fraichement installee, sans IA,
...
Complementaire, pas redondant. Volontairement hors de la sequence numerotee
01-19 pour ne pas casser l'ordre pedagogique existant, tout en se jouant au
bon endroit du parcours (apres avoir vu les deux styles).
...
- **Interdits stricts** : solution, tout LLM (ChatGPT, Claude, Gemini, Cursor,
  Windsurf), autocompletion basee sur ML (IntelliSense standard TypeScript
  OK, aide contextuell'IA interdite), StackOverflow, Google, MDN, GitHub
...
- **Autorises** : `man`, `--help`, `node --help`, `node --repl`, tes propres
  ...
  ce parcours qui traitent du module choisi.
...
- **Facile** : `01-CADRAGE/02-ASYNC` (une Promise from scratch, un mini scheduler).
- **Moyen** : `02-CONSTRUCTION/05-MEMORY-PERFORMANCE` (un LRU cache + un heap profiler
  minimal).
- **Difficile** : `02-CONSTRUCTION/09-FUNCTIONAL-JS` (une lib fonctionnelle : `pipe`,
...
drill_ia_en_panne/
  ...
  08-POSTMORTEM.md      <- rempli a la fin, franc
```
...
1. La machine etait vierge au démarrage. (Preuve : `SETUP_LOG.md` complet.)
...
- Prerequis pedagogique : avoir termine le module cible **et** au moins un
  `EXO_VERIFICATION.md` du meme module.
- Suite recommandee : ecrire un post retour d'experience dans
  `05-MAITRISE/06-ANNEXES/16_career/02-guide.md` (annexe personnelle) et referencer le
  POSTMORTEM.
- Cousin en aval : ce drill nourrit `05-MAITRISE/06-ANNEXES/16_career/04-plateau_playbook.md`
  si tu decouvres un plateau en le passant.

## RESUME

Six a dix heures, une machine vierge, aucune solution, aucun internet, un module
