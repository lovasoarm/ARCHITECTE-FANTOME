...
- `../19_interview/02-desaccord_cto.md` (mise en scène complète autour de `05_prison_break_api`).
- `../19_interview/03-mock_interview_async.md` (défense sur le raisonnement asynchrone).
- `03-PILOTAGE/10-TEAM-CRAFT/15-argumentaire_technique.md` (structure de l'argument).
- `../28_templates/02-POSTMORTEM.md` (renvoi : chaque décision de POSTMORTEM doit pouvoir être défendue oralement).
...
| #   | Critère                                                                              | Oui / Non |
| --- | ------------------------------------------------------------------------------------ | --------- |
| 1   | IA-je attaqué le fond de l'objection ou juste concédé ?                              |           |
| 2   | IA-je chiffré au moins un point (temps, coût, latence, complexité) ?                 |           |
| 3   | IA-je posé au moins une contre-question pour clarifier le vrai besoin ?              |           |
| 4   | IA-je nommé le trade-off explicite (ce que je sacrifie, pour quoi) ?                 |           |
| 5   | IA-je tenu ma position OU cédé de façon argumentée, sans agressivité ni soumission ? |           |
...
Angle : rappelle `../../../01-CADRAGE/03-DEBUGGING/13-HYPOTHESES_EXEMPLE_REPRO_DETERMINISTE.md`. Reproduis la race de façon déterministe ; expose le protocole de falsification.

### Scénario 4 : "Ton architecture en couches est de la sur-ingénierie pour ce périmètre."

Contexte : voir aussi `../19_interview/02-desaccord_cto.md` objection 4.
...
Contexte : voir aussi `../19_interview/02-desaccord_cto.md` objection 1.
...
Angle : distingue couverture de ligne (fétichisme) et couverture de comportement (utile). Cite `02-CONSTRUCTION/03-TESTING/01-00-why-testing.md` et mutation testing (`04-EPREUVE/04-BIG-APP-SNOOP/*_ai_grimoire.md` "Mutation score").
...
Contexte : revue de code, objection sur `../../03-EDGE-CASES/90-grimoire.md`.
...
Angle : reprends `EXO_VERIFICATION.md` du module concerné, montre le drill vérifiable qui a démenti la suggestion. Ne théorise pas : montre la trace.
