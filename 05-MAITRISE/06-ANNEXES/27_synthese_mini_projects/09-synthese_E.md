...
> Couvre : `04-EPREUVE/04-BIG-APP-SNOOP` + `05-MAITRISE/01-DATABASES` + `05-MAITRISE/02-SCALABILITY` + `03-PILOTAGE/05-OBSERVABILITY` + `03-PILOTAGE/10-TEAM-CRAFT` + `05-MAITRISE/03-EDGE-CASES` + `02-CONSTRUCTION/17-OOP-JS`
...
│  ├── PromptBuilder.js    construit le consigne envoyé à l'Oracle
...
**Du module `02-CONSTRUCTION/17-OOP-JS` (oop js) :**
...
**Du module `04-EPREUVE/04-BIG-APP-SNOOP` (IA native dev) :**
Le streaming de la réponse de l'Oracle doit être géré token par token, pas attendre la réponse complète avant de traiter quoi que ce soit. `PromptBuilder` doit construire des consigne qui réduisent activement le risque d'hallucination (contexte précis, format de sortie imposé), pas juste balancer la question brute.
...
**Du module `05-MAITRISE/03-EDGE-CASES` (edge cases) :**
...
**Du module `05-MAITRISE/01-DATABASES` (databases) :**
...
**Du module `05-MAITRISE/02-SCALABILITY` (scalability) et 26 (observability), ensemble :**
...
**Du module `03-PILOTAGE/10-TEAM-CRAFT` (team craft) :**
