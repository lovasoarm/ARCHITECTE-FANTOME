...
> Couvre : `02-CONSTRUCTION/13_runtime_env` + `02-CONSTRUCTION/14_architecture_patterns` + `02-CONSTRUCTION/18_web_concepts` + `02-CONSTRUCTION/19_api_craft` + `03-PILOTAGE/04_security`
...
Le cycle de vie HTTP de chaque requête doit suivre le schéma canonique de la charte ASCII (`05-MAITRISE/06_annexes/01_ascii_charte.md`, schéma 5) : auth, validation, handler, error handler, dans cet ordre, sans exception.
