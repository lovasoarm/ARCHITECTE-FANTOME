...
**Noyau essentiel** : la seule chose qui doit survivre du module. Si tu ne gardes qu'elle, tu retrouves ou reconstruis le reste en moins de 2 minutes.
...
Chaque Noyau essentiel pointe vers un fichier précis du repo : le grimoire de synthèse quand il existe, sinon le fichier le plus dense. C'est le seul endroit où rouvrir en cas de doute : pas besoin de fouiller le dossier entier.
...
**Noyau essentiel** : l'IA fera la syntaxe. Elle ne choisira pas à ta place, ne sécurisera pas à ta place, n'expliquera pas à ta place. Les six pierres, c'est ce qui te rend irremplaçable.
_Fichier : `05-MAITRISE/06_annexes/22_PONTS_INTER_MODULES.md`_
...
**Noyau essentiel** : Node ≥ 22 LTS (version longue durée, la plus stable), `.nvmrc` = ton fichier source de vérité pour la version utilisée. Vérifie `node -v` et `git --version` avant tout.
_Fichier : `00-SOCLE/01_getting_started/01_install.md`_
...
**Noyau essentiel** : révision espacée : tu revois chaque module à J+1, J+7, J+21, J+60 après l'avoir appris. Sans ce rythme, 70% de ce que tu as appris disparaît en 24h.
_Fichier : `00-SOCLE/03_referentiel/00_why_referentiel.md`_
...
**Noyau essentiel** : une closure, c'est une fonction qui se souvient encore des variables qui l'entouraient au moment où elle a été créée, même après que la fonction autour ait fini de s'exécuter. Si tu ne peux pas l'expliquer à un enfant, à un junior, et à un senior, tu ne l'as pas encore comprise.
_Fichier : `00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md`_
...
**Noyau essentiel** : méthode de Polya, dans cet ordre strict : comprendre le problème → planifier une approche → exécuter → réviser le résultat. La cause n°1 des bugs qu'on croit "techniques" est en réalité un malentendu sur ce qui était demandé.
_Fichier : `00-SOCLE/05_problem_solving/01_polya_method.md`_
...
**Noyau essentiel** : un seul thread (fil d'exécution) en JavaScript. Une seule pile d'appel (call stack, la liste des fonctions en train de s'exécuter, empilées). Une file de microtasks (les Promises, `queueMicrotask`) et une file de macrotasks (`setTimeout`, les entrées/sorties). Les microtasks se vident entièrement avant qu'une seule macrotask ne passe.
_Fichier : `01-CADRAGE/02_async/04_event_loop/03_event_loop_grimoire.md`_
...
**Noyau essentiel** : écris ton hypothèse par écrit avant de tester quoi que ce soit. Pas de reproduction du bug (le refaire arriver exprès, à volonté), pas de correction valable. Une correction sans reproduction, c'est de la superstition, pas de l'ingénierie.
_Fichier : `01-CADRAGE/03_debugging/05_hypothesis_driven_debug.md`_
...
**Noyau essentiel** : une hiérarchie d'erreurs typées (des classes d'erreurs précises, une par type de problème) vaut mieux que cent try/catch génériques. Utilise `extends Error`, garde toujours la stack (la trace qui montre d'où vient l'erreur), et ajoute un `cause` (la raison d'origine, quand une erreur en cache une autre).
_Fichier : `01-CADRAGE/04_error_handling/02_custom_errors.md`_
...
**Noyau essentiel** : un test vérifie un comportement (ce que le code doit garantir), pas une implémentation (comment il le fait en interne). Nomme tes tests par ce qu'ils garantissent, pas par la fonction qu'ils appellent.
_Fichier : `02-CONSTRUCTION/03_testing/01_unit_sniper.md`_
...
**Noyau essentiel** : les valeurs "falsy" (qui deviennent `false` dans un `if`) : `0`, `''`, `null`, `undefined`, `NaN`, `false`. Avec la loi de De Morgan (une règle qui dit comment transformer un ET/OU niés) et le court-circuit (`&&`/`||` qui arrêtent l'évaluation dès que le résultat est connu), tu couvres la moitié des bugs de conditions.
_Fichier : `02-CONSTRUCTION/04_math_basics/01_boolean_logic.md`_
...
**Noyau essentiel** : aucune optimisation sans avoir mesuré d'abord. Utilise l'onglet Performance de Chrome, `node --inspect`, ou un heap snapshot (une photo de tout ce qui est en mémoire à un instant donné). Sinon, tu chasses un fantôme.
_Fichier : `02-CONSTRUCTION/05_memory_performance/04_profiling/01_profiling_basics.md`_
...
**Noyau essentiel** : `Map` dès que tu as des clés dynamiques (qui changent), `Set` pour garantir l'unicité des valeurs. Ces deux réflexes valent 10 optimisations plus compliquées.
_Fichier : `02-CONSTRUCTION/06_data_structures/07_hash_table/01_hash_table_basics.md`_
...
**Noyau essentiel** : recherche binaire (couper l'espace de recherche en deux à chaque fois) sur une liste triée, table de hash sinon. Toute autre variante que tu croises est un cas particulier de l'un des deux.
_Fichier : `02-CONSTRUCTION/07_algorithms/02_searching/01_linear_binary.md`_
...
**Noyau essentiel** : une fonction pure (même entrée = toujours même sortie, sans effet de bord ailleurs) est la brique la plus facile à tester. L'immutabilité par défaut (ne jamais modifier une donnée, en créer une nouvelle à la place) élimine une classe entière de bugs.
_Fichier : `02-CONSTRUCTION/09_functional_js/01_pure_functions.md`_
...
**Noyau essentiel** : un pattern (une solution nommée à un problème récurrent) permet à deux devs de se comprendre en 3 mots au lieu de 3 paragraphes. C'est toute sa valeur.
_Fichier : `02-CONSTRUCTION/10_design_patterns/04_patterns_grimoire.md`_
...
**Noyau essentiel** : un nom qui ment sur ce qu'il fait coûte plus cher qu'une fonction trop longue. Renommer une variable ou une fonction est le refactoring le plus rentable qui existe.
_Fichier : `02-CONSTRUCTION/11_refactoring/01_clean_code_basics.md`_
...
**Noyau essentiel** : côté Node (serveur) : `fs` (fichiers), `process` (infos sur l'environnement d'exécution), les streams (flux de données). Côté navigateur : le DOM (la représentation de la page), les Web APIs. Les deux environnements ont en commun `fetch` et `Promise`.
_Fichier : `02-CONSTRUCTION/13_runtime_env/01_node_vs_browser.md`_
...
**Noyau essentiel** : Clean Architecture, une seule règle à retenir : les dépendances pointent toujours vers le domaine (le cœur métier de l'application), jamais l'inverse.
_Fichier : `02-CONSTRUCTION/14_architecture_patterns/04_clean_architecture.md`_
...
**Noyau essentiel** : `this` est déterminé au moment de l'appel de la fonction, pas au moment où elle est définie. Seule exception : la fonction fléchée, qui capture `this` de son environnement de création. Le reste n'est qu'un cas particulier de cette règle.
_Fichier : `02-CONSTRUCTION/17_oop_js/04_this_keyword_rules.md`_
...
**Noyau essentiel** : l'accessibilité est une contrainte de conception dès le départ, pas une couche qu'on ajoute à la fin. Un composant inaccessible est un composant cassé, pour environ 15% des utilisateurs.
_Fichier : `03-PILOTAGE/02_web_inclusive/01_a11y_why_it_matters.md`_
...
**Noyau essentiel** : WebSockets : bidirectionnel (les deux côtés peuvent parler en même temps), stateful (la connexion garde un état, coûteux à faire grossir à l'échelle) : pour le chat, la collaboration en direct, le jeu en ligne. SSE (Server-Sent Events) : uniquement serveur vers client, mais sur du HTTP simple : pour les notifications, les fils d'actualité. WebRTC : de pair à pair (les appareils se parlent directement), complexe à mettre en place (STUN/TURN/signaling, des mécanismes pour connecter deux appareils derrière des box internet différentes) : pour la voix et la vidéo.
_Fichier : `04-EPREUVE/03_realtime/04_realtime_grimoire.md`_
...
**Noyau essentiel** : des ressources nommées au pluriel, des verbes HTTP porteurs de sens, des statuts de réponse corrects. L'idempotence (`GET`/`PUT`/`DELETE` peuvent être rejoués plusieurs fois sans changer le résultat, contrairement à `POST`) couvre 80% d'une bonne API.
_Fichier : `02-CONSTRUCTION/19_api_craft/08_api_grimoire.md`_
...
**Noyau essentiel** : l'IA propose, c'est toi qui décides. La signature d'un dev qui a compris ce qu'il fait : il peut expliquer ce qu'il a accepté ET ce qu'il a refusé, et pourquoi.
_Fichier : `04-EPREUVE/04_ai_native_dev/12_ai_grimoire.md`_
...
_Fichier : `04-EPREUVE/04_ai_native_dev/03_validate_ai_output.md`_
...
_Fichier : `05-MAITRISE/01_databases/03_data_modeling.md`_
...
**Noyau essentiel** : les 8 croyances fausses classiques sur les systèmes distribués (par exemple : croire que le réseau est fiable, que la latence est nulle, ou que la bande passante est infinie : la liste complète des 8 est dans le fichier source). À relire tous les ans, elles ne prennent pas une ride.
_Fichier : `05-MAITRISE/02_scalability/03_distributed_fallacies.md`_
...
**Noyau essentiel** : des logs structurés (au format JSON, avec des niveaux de gravité et un identifiant de corrélation qui suit une même requête à travers tous les services) + des traces distribuées (le chemin complet d'une requête à travers plusieurs services) + les indicateurs RED (Rate, Errors, Duration : pour les services) ou USE (Utilization, Saturation, Errors : pour les ressources comme le CPU ou la mémoire). Sans ça, un bug dans un système à plusieurs services devient un vrai mystère à résoudre.
_Fichier : `03-PILOTAGE/05_observability/02_distributed_tracing.md`_
...
**Noyau essentiel** : sache expliquer la même chose à un enfant, à un pair développeur, et à un décideur non-technique. C'est ce qui sépare ceux qui ont vraiment compris de ceux qui ont juste lu.
_Fichier : `03-PILOTAGE/10_team_craft/12_three_audiences_intro.md`_
...
**Noyau essentiel** : une race condition (un bug qui dépend de l'ordre d'exécution de deux bouts de code en même temps) ne se corrige jamais en réessayant simplement, mais par de la coordination : un mutex (un verrou qui empêche deux bouts de code de toucher la même donnée en même temps), une transaction, ou un verrou optimiste (on vérifie juste avant d'enregistrer que rien n'a changé entre-temps).
_Fichier : `05-MAITRISE/03_edge_cases/05_race_condition_hunter.md`_
...
**Noyau essentiel** : un copilote suggère → tu valides toi-même chaque étape. Un agent agit sur plusieurs étapes sans validation entre chacune. La différence, c'est un facteur de risque, pas un facteur de productivité.
_Fichier : `05-MAITRISE/04_ai_agents_and_autonomy/01_agents_vs_copilots.md`_
...
_Fichier : `05-MAITRISE/04_ai_agents_and_autonomy/02_verifiable_specifications.md`_
...
**Noyau essentiel** : on ne mémorise pas le code des projets, on retient ce qu'ils prouvent qu'on sait faire. Chaque projet se clôt par un POSTMORTEM (un bilan écrit après coup) signé : sans postmortem, c'est comme si le projet n'avait pas eu lieu.
_Fichier : `02-CONSTRUCTION/02_mini_projects/_templates/01_POSTMORTEM_TEMPLATE.md`_
...
**Noyau essentiel** : la meilleure ligne de code est celle que tu n'as pas eu besoin d'écrire. Une solution sans code, un SaaS existant (un logiciel prêt à l'emploi), ou simplement une conversation avec la bonne personne valent souvent mieux qu'un sprint entier de développement.
_Fichier : `05-MAITRISE/06_annexes/04_when_not_to_code.md`_
...
_Fichiers : `05-MAITRISE/06_annexes/20_PERISSABILITE.md`, `05-MAITRISE/06_annexes/22_PONTS_INTER_MODULES.md`_
...
**Noyau essentiel** : un logger structuré (qui écrit des logs dans un format lisible par une machine, pas juste du texte brut), un kit de benchmark reproductible (pour mesurer une performance de façon fiable et répétable), un scaffolder en ligne de commande (un outil qui génère la structure de départ d'un projet), un kit de debug personnel : 3 scripts, pas 30.
_Fichier : `05-MAITRISE/07_tools/00_why_tools.md`_
...
Pas du rêve, pas du recrutement déguisé : ce que le guide carrière du repo lui-même documente (`05-MAITRISE/06_annexes/16_career/00_guide.md`), condensé à l'essentiel.
...
DEVOPS / CLOUD  : ajoute Docker/Kubernetes/Terraform en profondeur
              (05-MAITRISE/02_scalability + 03-PILOTAGE/05_observability sont ton point de départ,
               pas ton point d'arrivée)

AI ENGINEER   : ajoute Python + un framework de Machine Learning
              (04-EPREUVE/04_ai_native_dev te donne le réflexe de validation,
               pas la stack ML elle-même)

CYBERSÉCURITÉ  : ajoute Kali/Burp Suite/du pentest actif (des tests
              d'intrusion volontaires)
              (03-PILOTAGE/04_security te donne les fondations défensives,
               pas l'attaque)
```
