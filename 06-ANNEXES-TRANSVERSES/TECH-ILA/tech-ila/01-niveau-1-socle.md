...
> **Tu dois déjà savoir** : utiliser un terminal basique, avoir écrit du JavaScript/TypeScript avec MyFunnyJS jusqu'au module `02-CONSTRUCTION/12_typescript`, avoir un projet Node minimal qui tourne en local.
...
**Ce qui restera dans 10 ans.** Les pipes, les flux, les codes de sortie, les permissions, les signaux (`SIGTERM`, `SIGKILL`). Ce sont les mêmes concepts que les streams Node ([02-CONSTRUCTION/13_runtime_env/02_streams_buffers.md](../../../02-CONSTRUCTION/13_runtime_env/02_streams_buffers.md)).
...
- **Ancrage MyFunnyJS** : [00-SOCLE/04_fundamentals/08_git_core.md](../../../00-SOCLE/04_fundamentals/08_git_core.md) : commits, branches, fusion : le graphe de snapshots est déjà en place dans ta tête.
...
- **Preuve que c'est acquis** : tu retrouves, dans un dépôt que tu ne connais pas, le commit qui a introduit un bug via `git bisect`, sans lire une seule ligne de documentation. **Si tu bloques, reviens à** : [00-SOCLE/01_getting_started/03_git_101.md](../../../00-SOCLE/01_getting_started/03_git_101.md).

#### Ce que MyFunnyJS permet déjà de comprendre

L'outil est déjà couvert ; ce qui manque, c'est le réflexe d'aller chercher une intention dans l'historique.

- [00-SOCLE/04_fundamentals/08_git_core.md](../../../00-SOCLE/04_fundamentals/08_git_core.md) : commits, branches, fusion : le graphe de snapshots est déjà en place dans ta tête.
- [00-SOCLE/01_getting_started/03_git_101.md](../../../00-SOCLE/01_getting_started/03_git_101.md) : les gestes quotidiens et l'hygiène de commit.
- [01-CADRAGE/03_debugging/05_hypothesis_driven_debug.md](../../../01-CADRAGE/03_debugging/05_hypothesis_driven_debug.md) : `git bisect` n'est que ta dichotomie d'hypothèses appliquée à l'historique.
- [03-PILOTAGE/10_team_craft/04_navigate_codebase.md](../../../03-PILOTAGE/10_team_craft/04_navigate_codebase.md) : sur du legacy, `git log` est la meilleure documentation disponible.
- [05-MAITRISE/02_scalability/01_distributed_thinking.md](../../../05-MAITRISE/02_scalability/01_distributed_thinking.md) : deux dépôts qui divergent puis réconcilient, c'est un problème distribué déguisé en outil.
...
**Pont MyFunnyJS.** [03-PILOTAGE/10_team_craft/01_code_review.md](../../../03-PILOTAGE/10_team_craft/01_code_review.md) (la PR est le support de la revue), [03-PILOTAGE/10_team_craft/04_navigate_codebase.md](../../../03-PILOTAGE/10_team_craft/04_navigate_codebase.md) (l'historique est ta meilleure doc sur du legacy).
...
> **Réutilise** : [03-PILOTAGE/10_team_craft/04_navigate_codebase.md](../../../03-PILOTAGE/10_team_craft/04_navigate_codebase.md)
...
| Erreur async non catchée              | crash silencieux du process                  | `unhandledRejection` (cf. [01-CADRAGE/04_error_handling/04_async_error_traps.md](../../../01-CADRAGE/04_error_handling/04_async_error_traps.md)) |
...
Vitest ou le runner natif `node:test`. Les tests d'intégration frappent un vrai serveur HTTP éphémère. Base de données : vrai PostgreSQL dans un conteneur, pas un mock. Cf. [02-CONSTRUCTION/03_testing/04_integration_reactor.md](../../../02-CONSTRUCTION/03_testing/04_integration_reactor.md) et [02-CONSTRUCTION/03_testing/03_mocking_madness.md](../../../02-CONSTRUCTION/03_testing/03_mocking_madness.md).

#### Sécurité

[03-PILOTAGE/04_security/09_supply_chain_sbom.md](../../../03-PILOTAGE/04_security/09_supply_chain_sbom.md) prend tout son sens ici : un `npm install` tire des centaines de paquets transitifs. Verrouille le lockfile, active `npm audit` en CI, épingle les versions, méfie-toi des paquets installés hier par un compte inconnu.

#### Performance

Mesure avant d'optimiser ([02-CONSTRUCTION/05_memory_performance/00_measure_first.md](../../../02-CONSTRUCTION/05_memory_performance/00_measure_first.md)). Outils : `node --inspect` + DevTools, `node --cpu-prof`, clinic.js (PROFESSIONNELLE, mécanisme MyFunnyJS : profiling du même event loop), autocannon (CONTEXTUELLE, mécanisme MyFunnyJS : génération de charge pour observer la backpressure). Métrique reine côté serveur : le **lag de l'event loop**. Si elle dépasse 50 ms, quelque chose bloque.

#### Observabilité

Logs structurés JSON (pino : PROFESSIONNELLE, mécanisme MyFunnyJS : sérialisation structurée plutôt que `console.log`), un `request_id` propagé partout, métriques exportées. Cf. [03-PILOTAGE/05_observability/01_structured_logging.md](../../../03-PILOTAGE/05_observability/01_structured_logging.md).
...
> **Réutilise** : [02-CONSTRUCTION/13_runtime_env/02_streams_buffers.md](../../../02-CONSTRUCTION/13_runtime_env/02_streams_buffers.md)
...
- **Ancrage MyFunnyJS** : [00-SOCLE/01_getting_started/04_package_managers.md](../../../00-SOCLE/01_getting_started/04_package_managers.md) : résolution de versions, lockfile, arbre de dépendances : le problème est identique avec pip, Maven ou NuGet.
...
- **Preuve que c'est acquis** : tu peux expliquer pourquoi `npm ci` est la commande de CI et jamais `npm install`. **Si tu bloques, reviens à** : [00-SOCLE/01_getting_started/04_package_managers.md](../../../00-SOCLE/01_getting_started/04_package_managers.md).
...
- [00-SOCLE/01_getting_started/04_package_managers.md](../../../00-SOCLE/01_getting_started/04_package_managers.md) : résolution de versions, lockfile, arbre de dépendances.
- [03-PILOTAGE/04_security/09_supply_chain_sbom.md](../../../03-PILOTAGE/04_security/09_supply_chain_sbom.md) : chaque dépendance transitive est une surface d'attaque, pas une ligne gratuite.
- [03-PILOTAGE/04_security/10_audit_your_supply_chain.md](../../../03-PILOTAGE/04_security/10_audit_your_supply_chain.md) : la méthode d'audit que tu as déjà appliquée à un petit projet.
- [00-SOCLE/04_fundamentals/06_modules/01_import_export.md](../../../00-SOCLE/04_fundamentals/06_modules/01_import_export.md) : ce que tu importes détermine ce qui finit dans ton artefact.
...
**Sécurité.** Typosquatting (`react-dom` vs `raect-dom`), scripts `postinstall` malveillants, compromission de mainteneur. Lis [03-PILOTAGE/04_security/09_supply_chain_sbom.md](../../../03-PILOTAGE/04_security/09_supply_chain_sbom.md) et [03-PILOTAGE/04_security/10_audit_your_supply_chain.md](../../../03-PILOTAGE/04_security/10_audit_your_supply_chain.md).
...
> **Réutilise** : [03-PILOTAGE/04_security/10_audit_your_supply_chain.md](../../../03-PILOTAGE/04_security/10_audit_your_supply_chain.md)
...
- **Pont MyFunnyJS** : [02-CONSTRUCTION/14_architecture_patterns/06_microservices_intro.md](../../../02-CONSTRUCTION/14_architecture_patterns/06_microservices_intro.md) : le même arbitrage entre un ensemble regroupé et des parties indépendantes, appliqué ici au dépôt plutôt qu'au déploiement.
...
- **Ancrage MyFunnyJS** : [02-CONSTRUCTION/18_web_concepts/01_http_rest_basics.md](../../../02-CONSTRUCTION/18_web_concepts/01_http_rest_basics.md) : méthodes, codes, en-têtes : la base est posée.
...
- **Preuve que c'est acquis** : avec `curl -v` seul, tu expliques la différence entre un 4xx et un 5xx sur un cas réel, sans consulter de documentation. **Si tu bloques, reviens à** : [02-CONSTRUCTION/18_web_concepts/01_http_rest_basics.md](../../../02-CONSTRUCTION/18_web_concepts/01_http_rest_basics.md).

#### Ce que MyFunnyJS permet déjà de comprendre

- [02-CONSTRUCTION/18_web_concepts/01_http_rest_basics.md](../../../02-CONSTRUCTION/18_web_concepts/01_http_rest_basics.md) : méthodes, codes, en-têtes.
- [00-SOCLE/04_fundamentals/05_web_basics/02_fetch_adventure.md](../../../00-SOCLE/04_fundamentals/05_web_basics/02_fetch_adventure.md) : timeouts, annulation, réponses inattendues.
- [01-CADRAGE/04_error_handling/03_error_propagation.md](../../../01-CADRAGE/04_error_handling/03_error_propagation.md) : un code HTTP est une propagation d'erreur à travers un réseau, avec perte de contexte.
- [02-CONSTRUCTION/18_web_concepts/04_caching_strategies.md](../../../02-CONSTRUCTION/18_web_concepts/04_caching_strategies.md) : `ETag` et `Cache-Control` sont la version réseau de ce que tu as déjà vu en mémoire.
...
| CORS        | protection navigateur, pas de sécurité serveur ([03-PILOTAGE/04_security/02_csrf_cors.md](../../../03-PILOTAGE/04_security/02_csrf_cors.md)) |
...
> **Réutilise** : [02-CONSTRUCTION/18_web_concepts/01_http_rest_basics.md](../../../02-CONSTRUCTION/18_web_concepts/01_http_rest_basics.md)
...
- **Preuve que c'est acquis** : tu sais lire un `EXPLAIN ANALYZE` et dire, sans deviner, si un index est utilisé et pourquoi. **Si tu bloques, reviens à** : [05-MAITRISE/01_databases/01_sql_basics.md](../../../05-MAITRISE/01_databases/01_sql_basics.md).
...
**Preuve exigée :** `EXPLAIN (ANALYZE, BUFFERS)` avant et après. Sans mesure, ce n'est pas de l'optimisation, c'est de la superstition ([02-CONSTRUCTION/05_memory_performance/00_measure_first.md](../../../02-CONSTRUCTION/05_memory_performance/00_measure_first.md)).
...
| Piège | tu crois lire du JS, tu produis 200 requêtes             | injection SQL si tu concatènes ([03-PILOTAGE/04_security/01_xss_injection.md](../../../03-PILOTAGE/04_security/01_xss_injection.md)) |
...
> **Réutilise** : [05-MAITRISE/01_databases/03_data_modeling.md](../../../05-MAITRISE/01_databases/03_data_modeling.md)
...
MyFunnyJS t'a appris la stratégie ([02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md](../../../02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md)). Voici l'outillage professionnel actuel.

- **Ancrage MyFunnyJS** : [02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md](../../../02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md) : quoi tester, à quelle granularité, et pourquoi le runner est un détail.
...
- **Preuve que c'est acquis** : tu casses volontairement une implémentation sans casser le comportement, et un bon test ne bouge pas. **Si tu bloques, reviens à** : [02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md](../../../02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md).

#### Ce que MyFunnyJS permet déjà de comprendre

- [02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md](../../../02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md) : quoi tester, à quelle granularité.
- [02-CONSTRUCTION/03_testing/03_mocking_madness.md](../../../02-CONSTRUCTION/03_testing/03_mocking_madness.md) : mocker la base, c'est tester ton mock.
- [02-CONSTRUCTION/03_testing/04_integration_reactor.md](../../../02-CONSTRUCTION/03_testing/04_integration_reactor.md) : le test qui traverse plusieurs couches est celui qui attrape les bugs réels.
- [01-CADRAGE/03_debugging/04_repro_before_fix.md](../../../01-CADRAGE/03_debugging/04_repro_before_fix.md) : un test qui échoue d'abord est la preuve qu'un correctif corrige quelque chose.
...
> **Réutilise** : [02-CONSTRUCTION/03_testing/03_mocking_madness.md](../../../02-CONSTRUCTION/03_testing/03_mocking_madness.md)
...
- **Ancrage MyFunnyJS** : [02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md) : configuration par variables d'environnement et signaux : `SIGTERM` et l'arrêt gracieux sont exactement le sujet de `docker stop`.
...
- **Ancrage MyFunnyJS** : [02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md) : la config vient de l'environnement, jamais de l'artefact.
...
- **Preuve que c'est acquis** : tu montres, sur ton propre déploiement, que le même artefact tourne en préprod et en prod avec uniquement la config qui change. **Si tu bloques, reviens à** : [02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md).

#### Ce que MyFunnyJS permet déjà de comprendre

- [02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md) : la config vient de l'environnement, jamais de l'artefact.
- [03-PILOTAGE/05_observability/06_debug_in_prod.md](../../../03-PILOTAGE/05_observability/06_debug_in_prod.md) : un déploiement sans observation est un pari.
- [03-PILOTAGE/04_security/09_supply_chain_sbom.md](../../../03-PILOTAGE/04_security/09_supply_chain_sbom.md) : l'artefact promu contient tout ce que tu as installé, y compris ce que tu n'as pas choisi.
- [01-CADRAGE/04_error_handling/05_error_strategy.md](../../../01-CADRAGE/04_error_handling/05_error_strategy.md) : décider à l'avance ce qui déclenche un rollback est une stratégie d'erreur, pas une improvisation.
...
> **Réutilise** : [02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md) : la config est lue au démarrage, validée, et le service refuse de démarrer si une variable obligatoire manque
