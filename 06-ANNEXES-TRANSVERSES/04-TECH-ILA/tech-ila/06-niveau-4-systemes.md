...
> **Tu viens de** : [05-niveau-3-backend.md](05-niveau-3-backend.md)
> **Tu dois déjà savoir** : Docker de base (`03-niveau-1-socle.md`), un backend NestJS ou Express qui tourne, les bases de la file d'attente et du pool de connexions.
> **Ensuite** : [07-niveau-5-transfert.md](07-niveau-5-transfert.md)
...
**Tag : NOYAU DURABLE** (le concept) / **PÉRISSABLE** (la syntaxe YAML) · Coût : ~15 h avant utilité · Durée de vie : ~10 ans (le modèle) · À apprendre après : `02-CONSTRUCTION/03-TESTING/` complet · Prérequis : `02-CONSTRUCTION/03-TESTING/`, [`03-PILOTAGE/10-TEAM-CRAFT/02-code_review.md`](../../../03-PILOTAGE/10-TEAM-CRAFT/02-code_review.md)
...
- [`02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md`](../../../02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md) : l'ordre des étapes d'un pipeline est une stratégie de test, pas une contrainte d'outil.
- [`../../../01-CADRAGE/03-DEBUGGING/08-flaky_bugs.md`](../../../01-CADRAGE/03-DEBUGGING/08-flaky_bugs.md) : un test instable est un bug de non-déterminisme, jamais une raison de relancer.
- `03-PILOTAGE/10-TEAM-CRAFT/02-code_review.md` : le pipeline automatise le vérifiable pour que la revue parle du reste.
- [`03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md`](../../../03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md) : le CI a accès à tes secrets et installe du code tiers : c'est ta chaîne d'approvisionnement.
- [`../../../00-SOCLE/01-GETTING-STARTED/06-package_managers.md`](../../../00-SOCLE/01-GETTING-STARTED/06-package_managers.md) : le lockfile est ce qui rend un build reproductible d'une machine à l'autre.
...
**Tests flaky (instables) en CI.** Un test qui échoue une fois sur vingt détruit la confiance de l'équipe : les gens relancent au lieu de lire. C'est `../../../01-CADRAGE/03-DEBUGGING/08-flaky_bugs.md` en contexte d'équipe. Traite-les comme des bugs de production : isole, reproduis, corrige ou supprime. Jamais "relance".

**Sécurité du pipeline : épinglage par SHA, pas de folklore.** Le CI a accès à tes secrets de déploiement. `action/checkout@vN` est un tag mouvant : son propriétaire peut repointer `vN` vers un commit malveillant demain matin, et ton pipeline exécutera ce code avec tes secrets sans qu'aucune ligne n'ait changé chez toi. `action/checkout@<sha40>` (le SHA complet) est un commit précis, immuable : c'est la seule garantie que l'action que tu exécutes aujourd'hui est celle que tu as auditée. C'est exactement le scénario `03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md` appliqué à ton CI. En complément : limite les permissions du token au strict nécessaire (`permissions: contents: read` par défaut), et ne donne jamais les secrets de prod aux pipelines déclenchés par une PR venant d'un fork.
...
> **Réutilise** : `02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md`
...
<a id="72-cloud-et-déploiement"></a>
<a id="sec-cloud"></a>
...
| Coût     | la facture est une métrique d'architecture ([`05-MAITRISE/06-ANNEXES/04-finops_greenops.md`](../../../05-MAITRISE/06-ANNEXES/04-finops_greenops.md))                               | calcule le coût mensuel à partir d'une grille tarifaire figée (voir 7.2 bis)                  |
...
- [`02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md`](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md) : config et secrets viennent de l'environnement : c'est la base du 12-factor (douze règles de conception pour une application cloud portable, dont « la config vit dans l'environnement, jamais dans le code ») et de tout déploiement cloud.
- [`05-MAITRISE/02-SCALABILITY/02-distributed_thinking.md`](../../../05-MAITRISE/02-SCALABILITY/02-distributed_thinking.md) : ce que tu gagnes en répliquant et ce que tu perds en cohérence.
- `05-MAITRISE/06-ANNEXES/04-finops_greenops.md` : la facture est une métrique d'architecture, mesurable comme la latence.
- [`02-CONSTRUCTION/05-MEMORY-PERFORMANCE/02A-measure_first.md`](../../../02-CONSTRUCTION/05-MEMORY-PERFORMANCE/02A-measure_first.md) : dimensionner sans mesurer produit soit une facture inutile, soit un incident.
- [`03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md`](../../../03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md) : le moindre privilège s'applique aux rôles cloud comme aux dépendances.
...
> **Réutilise** : `02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md` : le même artefact doit tourner dans les deux cas, seule la configuration change.
...
<a id="73-observabilité"></a>
<a id="sec-observabilite"></a>

### 7.3 : Observabilité

**Tag : NOYAU DURABLE** · Coût : ~12 h avant utilité · Durée de vie : ~10 ans (les trois piliers ne changent pas) · À apprendre après : 7.1 CI/CD · Prérequis : module `03-PILOTAGE/05-OBSERVABILITY/` complet

Le module du parcours t'a donné les trois piliers. Voici l'outillage et les décisions.

#### Ce que ce parcours permet déjà de comprendre

- [`03-PILOTAGE/05-OBSERVABILITY/02-structured_logging.md`](../../../03-PILOTAGE/05-OBSERVABILITY/02-structured_logging.md) : un log est une donnée requêtable, pas une phrase.
- [`03-PILOTAGE/05-OBSERVABILITY/03-distributed_tracing.md`](../../../03-PILOTAGE/05-OBSERVABILITY/03-distributed_tracing.md) : la propagation de contexte à travers les services et les files.
- [`03-PILOTAGE/05-OBSERVABILITY/05-metrics_alerting.md`](../../../03-PILOTAGE/05-OBSERVABILITY/05-metrics_alerting.md) : pourquoi la moyenne cache exactement les utilisateurs qui souffrent.
- [`03-PILOTAGE/05-OBSERVABILITY/08-prod_stack_trace_drill.md`](../../../03-PILOTAGE/05-OBSERVABILITY/08-prod_stack_trace_drill.md) : lire une trace de production sans le code sous les yeux.
- [`02-CONSTRUCTION/05-MEMORY-PERFORMANCE/02A-measure_first.md`](../../../02-CONSTRUCTION/05-MEMORY-PERFORMANCE/02A-measure_first.md) : sans mesure préalable, une optimisation est une croyance.
...
> **Réutilise** : `03-PILOTAGE/05-OBSERVABILITY/08-prod_stack_trace_drill.md`
...
<a id="74-résilience-et-architecture-distribuée"></a>
<a id="sec-resilience"></a>

### 7.4 : Résilience et architecture distribuée

**Tag : NOYAU DURABLE** · Coût : ~15 h avant utilité · Durée de vie : ~15 ans (les patrons sont indépendants du langage) · À apprendre après : 7.3 Observabilité · Prérequis : [`05-MAITRISE/02-SCALABILITY/04-distributed_fallacies.md`](../../../05-MAITRISE/02-SCALABILITY/04-distributed_fallacies.md), `05-MAITRISE/03-EDGE-CASES/`

#### Ce que ce parcours permet déjà de comprendre

- `05-MAITRISE/02-SCALABILITY/04-distributed_fallacies.md` : le réseau n'est pas fiable, la latence n'est pas nulle : tous les patrons ci-dessous en découlent.
- `05-MAITRISE/03-EDGE-CASES/` : les cas limites chassés en local deviennent des pannes partielles en distribué.
- [`01-CADRAGE/02-ASYNC/02_promises/02-promise_race.md`](../../../01-CADRAGE/02-ASYNC/02_promises/02-promise_race.md) : un timeout est une course entre ton appel et une horloge.
- [`../../../02-CONSTRUCTION/04-MATH-BASICS/06-probability_random.md`](../../../02-CONSTRUCTION/04-MATH-BASICS/06-probability_random.md) : le jitter d'un retry est du hasard utile : sans lui, tous tes clients réessaient à la même seconde.
- [`01-CADRAGE/04-ERROR-HANDLING/06-error_strategy.md`](../../../01-CADRAGE/04-ERROR-HANDLING/06-error_strategy.md) : dégrader proprement est une décision d'erreur, prise avant l'incident.
...
**Micro-services : la position honnête.** Ils résolvent un problème **organisationnel** (des équipes qui veulent déployer indépendamment) au prix d'un problème **technique** (le réseau, la cohérence, l'observabilité, la latence). Une équipe de cinq personnes avec sept micro-services a acheté tous les coûts sans aucun bénéfice. Le monolithe modulaire est un choix professionnel respectable et souvent le bon ([`02-CONSTRUCTION/14-ARCHITECTURE-PATTERNS/07-microservices_intro.md`](../../../02-CONSTRUCTION/14-ARCHITECTURE-PATTERNS/07-microservices_intro.md)).
...
> **Réutilise** : [`03-PILOTAGE/05-OBSERVABILITY/09-oncall_drill.md`](../../../03-PILOTAGE/05-OBSERVABILITY/09-oncall_drill.md)
...
**Tag : NOYAU DURABLE** · Coût : ~10 h avant utilité · Durée de vie : ~10 ans · À apprendre après : 7.4 Résilience · Prérequis : module `03-PILOTAGE/04-SECURITY/` complet

Ce que ce parcours t'a appris (XSS, CSRF, injection, pollution de prototype, OWASP, chaîne d'approvisionnement) se traduit en production par une poignée de gestes :
...
- **Journalisation d'audit** sur les actions sensibles : et **jamais** de données personnelles ou de secrets dans les logs (RGPD, [`03-PILOTAGE/04-SECURITY/07-privacy_and_data_regulation.md`](../../../03-PILOTAGE/04-SECURITY/07-privacy_and_data_regulation.md)).
...
- **Ancrage ce parcours** : [`03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md`](../../../03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md) et [`02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md`](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md) : la config et les secrets viennent de l'environnement, jamais du code.
...
> **Réutilise** : `03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md`
...
- [`03-PILOTAGE/04-SECURITY/02-xss_injection.md`](../../../03-PILOTAGE/04-SECURITY/02-xss_injection.md) : la zone protégée et la zone exposée de chaque framework, et pourquoi une CSP est une seconde barrière.
- [`03-PILOTAGE/04-SECURITY/03-csrf_cors.md`](../../../03-PILOTAGE/04-SECURITY/03-csrf_cors.md) : CORS n'est pas une protection serveur ; `SameSite` en est une.
- `03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md` : le scan en CI et le SBOM formalisent ce que tu as déjà audité à la main.
- [`03-PILOTAGE/04-SECURITY/07-privacy_and_data_regulation.md`](../../../03-PILOTAGE/04-SECURITY/07-privacy_and_data_regulation.md) : une donnée personnelle dans un log est une fuite avec rétention.
- [`03-PILOTAGE/05-OBSERVABILITY/02-structured_logging.md`](../../../03-PILOTAGE/05-OBSERVABILITY/02-structured_logging.md) : une journalisation d'audit non structurée n'est pas exploitable en incident.
...
**Ce que l'IA ne fera pas à ta place.** Elle écrira un middleware d'authentification correct dans 80 % des cas. Elle ne saura pas que, dans **ton** métier, un utilisateur du service support a le droit de voir la ressource d'un autre client mais pas de la modifier. Ce contexte n'est nulle part dans le consigne, et c'est exactement là que naissent les failles d'autorisation.
...
> **Mode** : assistant autorisé
> **Contraintes** : rédige un ADR complet suivant [`03-PILOTAGE/10-TEAM-CRAFT/03-adr_writing.md`](../../../03-PILOTAGE/10-TEAM-CRAFT/03-adr_writing.md), à partir des mesures de l'exercice de 7.2. Ajoute la contrainte suivante : réécris la décision en 5 lignes pour un responsable produit, sans un seul nom de techno.
> **Réutilise** : `03-PILOTAGE/10-TEAM-CRAFT/03-adr_writing.md`
...
> **Mode** : assistant autorisé
> **Contraintes** : chronologie, impact, cause racine, action corrective, action préventive : format [`03-PILOTAGE/05-OBSERVABILITY/09-oncall_drill.md`](../../../03-PILOTAGE/05-OBSERVABILITY/09-oncall_drill.md). Ajoute la version 5 lignes sans nom de techno pour un responsable produit.
> **Réutilise** : `03-PILOTAGE/05-OBSERVABILITY/09-oncall_drill.md`
...
Tant que tu es à gauche, la réponse est un index, une requête réécrite, un cache, une réplique de lecture (voir [`05-MAITRISE/01-DATABASES/`](../../../05-MAITRISE/01-DATABASES/01-00-why-databases.md)). Sortir l'artillerie avant ce seuil, c'est la sur-ingénierie décrite au [niveau 3 à propos des files massives](05-niveau-3-backend.md) : trois outils de plus à opérer pour un problème que la base réglait.
...
- Une tâche relancée qui n'est pas idempotente double les lignes. Exactement le problème de [`05-MAITRISE/02-SCALABILITY/08-message_queues.md`](../../../05-MAITRISE/02-SCALABILITY/08-message_queues.md), avec des millions de lignes au lieu d'un message.
...
- La facture. Un calcul distribué mal cadré consomme des heures-machine sans plafond naturel : c'est le sujet de `05-MAITRISE/06-ANNEXES/04-finops_greenops.md`, en plus brutal.
...
- **Ancrage ce parcours** : `05-MAITRISE/06-ANNEXES/04-finops_greenops.md` : décrire l'infra en code rend la facture relisible en PR
...
**Jaeger / Grafana / Datadog / Sentry** : Tag : PÉRISSABLE / CONTEXTUELLE (le backend change, OpenTelemetry reste) · Coût : ~4 h par outil · Durée de vie : ~4 ans · À apprendre après : 7.3 Observabilité : ce que ça change côté mécanisme du parcours : aucun, ce sont des visualisations de ce que `03-PILOTAGE/05-OBSERVABILITY/03-distributed_tracing.md` décrit déjà.

**OpenTelemetry** : Tag : NOYAU DURABLE · Coût : ~8 h avant utilité · Durée de vie : ~10 ans · À apprendre après : module `03-PILOTAGE/05-OBSERVABILITY/`

- **Ancrage ce parcours** : `03-PILOTAGE/05-OBSERVABILITY/03-distributed_tracing.md` : c'est l'implémentation standard de la propagation de contexte
...
**Renovate / Dependabot** : Tag : PROFESSIONNELLE · Coût : ~2 h avant utilité · Durée de vie : ~6 ans · À apprendre après : 7.1 : ce que ça change côté mécanisme du parcours : automatise `../../../00-SOCLE/01-GETTING-STARTED/06-package_managers.md`, ne remplace pas la lecture du changelog avant de merger.

**Testcontainers** : Tag : PROFESSIONNELLE · Coût : ~4 h avant utilité · Durée de vie : ~6 ans · À apprendre après : 7.1

- **Ancrage ce parcours** : `02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md` : tester contre un vrai PostgreSQL en conteneur plutôt qu'un mock
...
[← Niveau 3 : Backend](05-niveau-3-backend.md) · [Sommaire](../README.md) · [Niveau 5 : Transfert →](07-niveau-5-transfert.md)

## Platform Engineering : ne pas faire de chaque dev un admin de cluster

> **SCÈNE CRAZYDEVS : Prison Break :** vingt équipes, vingt pipelines bricolés et vingt portes secrètes. Le Staff ne rajoute pas une vingt-et-unième porte : il construit une route sûre.

Le terrain `03-PILOTAGE/12-PLATFORM-ENGINEERING` applique : GitOps, drift, Internal Developer Platform, golden paths, policy et developer experience. La compétence durable est d’identifier le toil et de standardiser ce qui doit l’être sans retirer l’autonomie nécessaire.

La CNCF décrit en 2026 l’essor des plateformes internes et de GitOps comme marqueurs de maturité cloud-native ; le cursus en garde les invariants plutôt que de figer un outil.
