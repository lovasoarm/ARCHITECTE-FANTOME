...
> **Tu viens de** : [02-orientation.md](02-orientation.md) : la grille d'admission et le test du zéro outil.
> **Tu dois déjà savoir** : utiliser un terminal basique, avoir écrit du JavaScript/TypeScript avec ce parcours jusqu'au module `02-CONSTRUCTION/12-TYPESCRIPT`, avoir un projet Node minimal qui tourne en local.
> **Ensuite** : [04-niveau-2-frontend.md](04-niveau-2-frontend.md) : React, Next.js, formulaires et accessibilité.
...
- **Ancrage ce parcours** : [00-SOCLE/01-GETTING-STARTED/04-shell_survival.md](../../../00-SOCLE/01-GETTING-STARTED/04-shell_survival.md) : navigation, pipes, redirections : c'est exactement la chaîne `tail -f | grep` que tu utiliseras en incident.
...
- **Preuve que c'est acquis** : tu sais lire `ls -ln` et expliquer un `permission denied` sans changer une ligne de code. **Si tu bloques, reviens à** : [00-SOCLE/01-GETTING-STARTED/04-shell_survival.md](../../../00-SOCLE/01-GETTING-STARTED/04-shell_survival.md).

#### Ce que ce parcours permet déjà de comprendre
...
- [00-SOCLE/01-GETTING-STARTED/04-shell_survival.md](../../../00-SOCLE/01-GETTING-STARTED/04-shell_survival.md) : navigation, pipes, redirections.
- [02-CONSTRUCTION/13-RUNTIME-ENV/06_node_cli_scripts](../../../02-CONSTRUCTION/13-RUNTIME-ENV/06_node_cli_scripts/) : tu as déjà lu `process.argv` et le système de fichiers depuis du code ; un script shell et un script Node résolvent le même problème avec deux vocabulaires.
- [02-CONSTRUCTION/13-RUNTIME-ENV/03-streams_buffers.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/03-streams_buffers.md) : un pipe Unix est un stream, et la backpressure que tu connais en Node y est le même mécanisme.
- [02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md) : `export`, `env`, `printenv` : la configuration par l'environnement, déjà pratiquée.
...
| Trouver un fichier         | `find . -name '*.env'`, `rg ""`               | audit codebase                |
...
**Ce qui restera dans 10 ans.** Les pipes, les flux, les codes de sortie, les permissions, les signaux (`SIGTERM`, `SIGKILL`). Ce sont les mêmes concepts que les streams Node ([02-CONSTRUCTION/13-RUNTIME-ENV/03-streams_buffers.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/03-streams_buffers.md)).
...
> **Temps réaliste** : 30 min · **Prérequis matériel / compte** : un terminal, un de tes mini-projets ce parcours · **Coût max** : 0 €
> **Mode** : jeûne d’IA obligatoire
> **Contraintes** : une seule ligne de shell, pas de script Node
> **Réutilise** : [02-CONSTRUCTION/02-MINI-PROJECTS/README.md](../../../02-CONSTRUCTION/02-MINI-PROJECTS/README.md) : un fichier de log existant ou généré par ton projet
...
**Quel problème ça résout.** Pas "sauvegarder du code". Git résout : _plusieurs cerveaux modifient la même vérité en parallèle et doivent réconcilier_. C'est un problème de systèmes distribués déguisé en outil (cf. [05-MAITRISE/02-SCALABILITY/02-distributed_thinking.md](../../../05-MAITRISE/02-SCALABILITY/02-distributed_thinking.md)).
...
- **Ancrage ce parcours** : [00-SOCLE/04-FUNDAMENTALS/02A-git_core.md](../../../00-SOCLE/04-FUNDAMENTALS/02A-git_core.md) : commits, branches, fusion : le graphe de snapshots est déjà en place dans ta tête.
...
- **Preuve que c'est acquis** : tu retrouves, dans un dépôt que tu ne connais pas, le commit qui a introduit un bug via `git bisect`, sans lire une seule ligne de documentation. **Si tu bloques, reviens à** : [00-SOCLE/01-GETTING-STARTED/05-git_101.md](../../../00-SOCLE/01-GETTING-STARTED/05-git_101.md).

#### Ce que ce parcours permet déjà de comprendre

L'outil est déjà couvert ; ce qui manque, c'est le réflexe d'aller chercher une intention dans l'historique.

- [00-SOCLE/04-FUNDAMENTALS/02A-git_core.md](../../../00-SOCLE/04-FUNDAMENTALS/02A-git_core.md) : commits, branches, fusion : le graphe de snapshots est déjà en place dans ta tête.
- [00-SOCLE/01-GETTING-STARTED/05-git_101.md](../../../00-SOCLE/01-GETTING-STARTED/05-git_101.md) : les gestes quotidiens et l'hygiène de commit.
- [01-CADRAGE/03-DEBUGGING/06-hypothesis_driven_debug.md](../../../01-CADRAGE/03-DEBUGGING/06-hypothesis_driven_debug.md) : `git bisect` n'est que ta dichotomie d'hypothèses appliquée à l'historique.
- [03-PILOTAGE/10-TEAM-CRAFT/05-navigate_codebase.md](../../../03-PILOTAGE/10-TEAM-CRAFT/05-navigate_codebase.md) : sur du legacy, `git log` est la meilleure documentation disponible.
- [05-MAITRISE/02-SCALABILITY/02-distributed_thinking.md](../../../05-MAITRISE/02-SCALABILITY/02-distributed_thinking.md) : deux dépôts qui divergent puis réconcilient, c'est un problème distribué déguisé en outil.
...
**Alternatives crédibles.** Mercurial (rare, PÉRISSABLE), Jujutsu (`jj`, émergent, PÉRISSABLE pour l'instant, mécanisme du parcours inchangé : toujours un graphe de snapshots), Perforce (jeu vidéo, gros binaires, mécanisme du parcours inchangé : verrouillage centralisé au lieu d'un graphe distribué).

**Pont ce parcours.** [03-PILOTAGE/10-TEAM-CRAFT/02-code_review.md](../../../03-PILOTAGE/10-TEAM-CRAFT/02-code_review.md) (la PR est le support de la revue), [03-PILOTAGE/10-TEAM-CRAFT/05-navigate_codebase.md](../../../03-PILOTAGE/10-TEAM-CRAFT/05-navigate_codebase.md) (l'historique est ta meilleure doc sur du legacy).
...
> **Réutilise** : [03-PILOTAGE/10-TEAM-CRAFT/05-navigate_codebase.md](../../../03-PILOTAGE/10-TEAM-CRAFT/05-navigate_codebase.md)
...
> **Repli 100 % local et gratuit** : clone un de tes propres mini-projets ce parcours si aucun accès réseau n'est disponible
...
- **Ancrage ce parcours** : [01-CADRAGE/02-ASYNC/04_event_loop](../../../01-CADRAGE/02-ASYNC/04_event_loop/) : tu sais déjà pourquoi une microtâche passe avant un `setTimeout`. C'est exactement ce qui explique pourquoi ton handler HTTP répond avant ton log.
...
- **Preuve que c'est acquis** : tu sais expliquer pourquoi une API Node "lente" n'a souvent aucun problème de base de données : juste un `JSON.parse` de 30 Mo dans le mauvais handler. **Si tu bloques, reviens à** : [01-CADRAGE/02-ASYNC/04_event_loop](../../../01-CADRAGE/02-ASYNC/04_event_loop/).

#### Ce que ce parcours permet déjà de comprendre

- [01-CADRAGE/02-ASYNC/04_event_loop](../../../01-CADRAGE/02-ASYNC/04_event_loop/) : tu sais déjà pourquoi une microtâche passe avant un `setTimeout`.
- [01-CADRAGE/02-ASYNC/04A-backpressure.md](../../../01-CADRAGE/02-ASYNC/04A-backpressure.md) : tu sais que si tu lis plus vite que tu n'écris, la mémoire monte. C'est le bug n°1 des pipelines de fichiers en Node.
- [02-CONSTRUCTION/13-RUNTIME-ENV/03-streams_buffers.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/03-streams_buffers.md) : les streams ne sont pas une API exotique, c'est le modèle central de Node.
- [02-CONSTRUCTION/05-MEMORY-PERFORMANCE/01_gc](../../../02-CONSTRUCTION/05-MEMORY-PERFORMANCE/01_gc/) : un serveur Node qui vit des semaines révèle toutes les fuites qu'un script de 3 secondes cachait.
...
Message obtenu en production sur le fichier de 4 Go : `FATAL ERROR: Reached heap limit Allocation failed - JavaScript heap out of memory`. Le diagnostic vient de [02-CONSTRUCTION/05-MEMORY-PERFORMANCE/04_profiling](../../../02-CONSTRUCTION/05-MEMORY-PERFORMANCE/04_profiling/) : heap snapshot, tu vois une seule string géante. Pas besoin de deviner.
...
| Erreur async non catchée              | crash silencieux du process                  | `unhandledRejection` (cf. [01-CADRAGE/04-ERROR-HANDLING/05-async_error_traps.md](../../../01-CADRAGE/04-ERROR-HANDLING/05-async_error_traps.md)) |
...
Vitest ou le runner natif `node:test`. Les tests d'intégration frappent un vrai serveur HTTP éphémère. Base de données : vrai PostgreSQL dans un conteneur, pas un mock. Cf. [02-CONSTRUCTION/03-TESTING/05-integration_reactor.md](../../../02-CONSTRUCTION/03-TESTING/05-integration_reactor.md) et [02-CONSTRUCTION/03-TESTING/04-mocking_madness.md](../../../02-CONSTRUCTION/03-TESTING/04-mocking_madness.md).

#### Sécurité

[03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md](../../../03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md) prend tout son sens ici : un `npm install` tire des centaines de paquets transitifs. Verrouille le lockfile, active `npm audit` en CI, épingle les versions, méfie-toi des paquets installés hier par un compte inconnu.

#### Performance

Mesure avant d'optimiser ([02-CONSTRUCTION/05-MEMORY-PERFORMANCE/02A-measure_first.md](../../../02-CONSTRUCTION/05-MEMORY-PERFORMANCE/02A-measure_first.md)). Outils : `node --inspect` + DevTools, `node --cpu-prof`, clinic.js (PROFESSIONNELLE, mécanisme du parcours : profiling du même event loop), autocannon (CONTEXTUELLE, mécanisme du parcours : génération de charge pour observer la backpressure). Métrique reine côté serveur : le **lag de l'event loop**. Si elle dépasse 50 ms, quelque chose bloque.

#### Observabilité

Logs structurés JSON (pino : PROFESSIONNELLE, mécanisme du parcours : sérialisation structurée plutôt que `console.log`), un `request_id` propagé partout, métriques exportées. Cf. [03-PILOTAGE/05-OBSERVABILITY/02-structured_logging.md](../../../03-PILOTAGE/05-OBSERVABILITY/02-structured_logging.md).
...
| Alternative                                                           | Ce qu'elle change                                           | Ce qui reste identique (mécanisme du parcours)                                                                    |
| --------------------------------------------------------------------- | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Deno (PROFESSIONNELLE)                                                | TS natif, sécurité par permissions                          | event loop, modèle async ([01-CADRAGE/02-ASYNC/04_event_loop](../../../01-CADRAGE/02-ASYNC/04_event_loop/))       |
...
| Python/FastAPI (tag du langage : cf. 8.1 en 07-niveau-5-transfert.md) | async aussi, écosystème data                                | modèle async/await très proche ([01-CADRAGE/02-ASYNC/04_event_loop](../../../01-CADRAGE/02-ASYNC/04_event_loop/)) |
...
> **Réutilise** : [02-CONSTRUCTION/13-RUNTIME-ENV/03-streams_buffers.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/03-streams_buffers.md)
...
- **Ancrage ce parcours** : [00-SOCLE/01-GETTING-STARTED/06-package_managers.md](../../../00-SOCLE/01-GETTING-STARTED/06-package_managers.md) : résolution de versions, lockfile, arbre de dépendances : le problème est identique avec pip, Maven ou NuGet.
...
- **Preuve que c'est acquis** : tu peux expliquer pourquoi `npm ci` est la commande de CI et jamais `npm install`. **Si tu bloques, reviens à** : [00-SOCLE/01-GETTING-STARTED/06-package_managers.md](../../../00-SOCLE/01-GETTING-STARTED/06-package_managers.md).
...
- [00-SOCLE/01-GETTING-STARTED/06-package_managers.md](../../../00-SOCLE/01-GETTING-STARTED/06-package_managers.md) : résolution de versions, lockfile, arbre de dépendances.
- [03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md](../../../03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md) : chaque dépendance transitive est une surface d'attaque, pas une ligne gratuite.
- [03-PILOTAGE/04-SECURITY/09-audit_your_supply_chain.md](../../../03-PILOTAGE/04-SECURITY/09-audit_your_supply_chain.md) : la méthode d'audit que tu as déjà appliquée à un petit projet.
- [00-SOCLE/04-FUNDAMENTALS/06_modules/02-import_export.md](../../../00-SOCLE/04-FUNDAMENTALS/06_modules/02-import_export.md) : ce que tu importes détermine ce qui finit dans ton artefact.

**npm vs pnpm.** pnpm utilise un store global et des liens durs : installations plus rapides, disque économisé, et surtout **isolation stricte** : un paquet ne peut pas importer ce qu'il n'a pas déclaré. npm est le défaut universel. Yarn (PROFESSIONNELLE, mécanisme du parcours inchangé) existe encore. Choisir : pnpm pour un monorepo ou une grosse équipe, npm pour tout le reste.

**Sécurité.** Typosquatting (`react-dom` vs `raect-dom`), scripts `postinstall` malveillants, compromission de mainteneur. Lis [03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md](../../../03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md) et [03-PILOTAGE/04-SECURITY/09-audit_your_supply_chain.md](../../../03-PILOTAGE/04-SECURITY/09-audit_your_supply_chain.md).
...
> **Réutilise** : [03-PILOTAGE/04-SECURITY/09-audit_your_supply_chain.md](../../../03-PILOTAGE/04-SECURITY/09-audit_your_supply_chain.md)
...
<a id="45-typescript-en-conditions-réelles"></a>### 4.5 : TypeScript en conditions réelles
...
ce parcours t'a donné le langage : génériques, types utilitaires, gardes de type, types conditionnels, `tsconfig`. TECH-ILA ne les réexplique pas. Ce qui change en entreprise, c'est l'endroit où tu acceptes de faire confiance à un type.

- **Ancrage ce parcours** : [02-CONSTRUCTION/12-TYPESCRIPT](../../../02-CONSTRUCTION/12-TYPESCRIPT/) : génériques, gardes de type, unions discriminées, `tsconfig` : rien de tout ça n'est à réapprendre.
...
- **Ce qu'elle ne résout pas** : le comportement. Un type garantit une forme, pas une logique correcte ; les tests restent nécessaires (cf. [02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md](../../../02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md)).
...
- **Preuve que c'est acquis** : tu sais dire, pour n'importe quelle fonction, où s'arrête la garantie du compilateur et où commence la responsabilité de la validation runtime. **Si tu bloques, reviens à** : [02-CONSTRUCTION/12-TYPESCRIPT](../../../02-CONSTRUCTION/12-TYPESCRIPT/).

#### Ce que ce parcours permet déjà de comprendre

- [02-CONSTRUCTION/12-TYPESCRIPT](../../../02-CONSTRUCTION/12-TYPESCRIPT/) : génériques, gardes de type, unions discriminées, `tsconfig`.
- [05-MAITRISE/03-EDGE-CASES/02-nan_undefined_null.md](../../../05-MAITRISE/03-EDGE-CASES/02-nan_undefined_null.md) : pourquoi `strictNullChecks` n'est pas un caprice de configuration.
- [00-SOCLE/04-FUNDAMENTALS/04_types/03-type_coercion.md](../../../00-SOCLE/04-FUNDAMENTALS/04_types/03-type_coercion.md) : tout ce qui arrive d'une query string est une `string` ; le type ne le corrige pas, la validation si.
- [02-CONSTRUCTION/11-REFACTORING/05-code_smells.md](../../../02-CONSTRUCTION/11-REFACTORING/05-code_smells.md) : un type de 40 lignes est un smell, exactement comme une fonction de 400 lignes.
...
> **Réutilise** : [02-CONSTRUCTION/12-TYPESCRIPT](../../../02-CONSTRUCTION/12-TYPESCRIPT/)
...
- **Ancrage ce parcours** : [02-CONSTRUCTION/18-WEB-CONCEPTS/02-http_rest_basics.md](../../../02-CONSTRUCTION/18-WEB-CONCEPTS/02-http_rest_basics.md) : méthodes, codes, en-têtes : la base est posée.
...
- **Preuve que c'est acquis** : avec `curl -v` seul, tu expliques la différence entre un 4xx et un 5xx sur un cas réel, sans consulter de documentation. **Si tu bloques, reviens à** : [02-CONSTRUCTION/18-WEB-CONCEPTS/02-http_rest_basics.md](../../../02-CONSTRUCTION/18-WEB-CONCEPTS/02-http_rest_basics.md).

#### Ce que ce parcours permet déjà de comprendre

- [02-CONSTRUCTION/18-WEB-CONCEPTS/02-http_rest_basics.md](../../../02-CONSTRUCTION/18-WEB-CONCEPTS/02-http_rest_basics.md) : méthodes, codes, en-têtes.
- [00-SOCLE/04-FUNDAMENTALS/05_web_basics/03-fetch_adventure.md](../../../00-SOCLE/04-FUNDAMENTALS/05_web_basics/03-fetch_adventure.md) : timeouts, annulation, réponses inattendues.
- [01-CADRAGE/04-ERROR-HANDLING/04-error_propagation.md](../../../01-CADRAGE/04-ERROR-HANDLING/04-error_propagation.md) : un code HTTP est une propagation d'erreur à travers un réseau, avec perte de contexte.
- [02-CONSTRUCTION/18-WEB-CONCEPTS/05-caching_strategies.md](../../../02-CONSTRUCTION/18-WEB-CONCEPTS/05-caching_strategies.md) : `ETag` et `Cache-Control` sont la version réseau de ce que tu as déjà vu en mémoire.
...
| CORS        | protection navigateur, pas de sécurité serveur ([03-PILOTAGE/04-SECURITY/03-csrf_cors.md](../../../03-PILOTAGE/04-SECURITY/03-csrf_cors.md)) |
...
**Pont ce parcours.** [02-CONSTRUCTION/19-API-CRAFT](../../../02-CONSTRUCTION/19-API-CRAFT/) tout entier, [01-CADRAGE/04-ERROR-HANDLING/04-error_propagation.md](../../../01-CADRAGE/04-ERROR-HANDLING/04-error_propagation.md).
...
> **Réutilise** : [02-CONSTRUCTION/18-WEB-CONCEPTS/02-http_rest_basics.md](../../../02-CONSTRUCTION/18-WEB-CONCEPTS/02-http_rest_basics.md)
...
- **Ancrage ce parcours** : [02-CONSTRUCTION/06-DATA-STRUCTURES/07_hash_table](../../../02-CONSTRUCTION/06-DATA-STRUCTURES/07_hash_table/) et [02-CONSTRUCTION/06-DATA-STRUCTURES/06_bst](../../../02-CONSTRUCTION/06-DATA-STRUCTURES/06_bst/) : un index B-tree, c'est l'arbre que tu as déjà implémenté ; un index de hachage, la table que tu as déjà implémentée.
...
- **Preuve que c'est acquis** : tu sais lire un `EXPLAIN ANALYZE` et dire, sans deviner, si un index est utilisé et pourquoi. **Si tu bloques, reviens à** : [05-MAITRISE/01-DATABASES/02-sql_basics.md](../../../05-MAITRISE/01-DATABASES/02-sql_basics.md).

#### Ce que ce parcours permet déjà de comprendre

- [02-CONSTRUCTION/06-DATA-STRUCTURES/07_hash_table](../../../02-CONSTRUCTION/06-DATA-STRUCTURES/07_hash_table/) et [02-CONSTRUCTION/06-DATA-STRUCTURES/06_bst](../../../02-CONSTRUCTION/06-DATA-STRUCTURES/06_bst/) : un index B-tree, c'est l'arbre que tu as déjà implémenté.
- [02-CONSTRUCTION/05-MEMORY-PERFORMANCE/03_complexity](../../../02-CONSTRUCTION/05-MEMORY-PERFORMANCE/03_complexity/) : un `Seq Scan` sur 10 millions de lignes, c'est O(n), et tu sais ce que ça coûte.
- [05-MAITRISE/03-EDGE-CASES/06-race_condition_hunter.md](../../../05-MAITRISE/03-EDGE-CASES/06-race_condition_hunter.md) : un `if (await exists(x))` suivi d'un `insert` est la race condition que tu as déjà chassée, cette fois entre deux transactions.
- [05-MAITRISE/03-EDGE-CASES/03-floating_point.md](../../../05-MAITRISE/03-EDGE-CASES/03-floating_point.md) : pourquoi un montant se stocke en `NUMERIC` et jamais en flottant.
...
**Preuve exigée :** `EXPLAIN (ANALYZE, BUFFERS)` avant et après. Sans mesure, ce n'est pas de l'optimisation, c'est de la superstition ([02-CONSTRUCTION/05-MEMORY-PERFORMANCE/02A-measure_first.md](../../../02-CONSTRUCTION/05-MEMORY-PERFORMANCE/02A-measure_first.md)).
...
| Piège | tu crois lire du JS, tu produis 200 requêtes             | injection SQL si tu concatènes ([03-PILOTAGE/04-SECURITY/02-xss_injection.md](../../../03-PILOTAGE/04-SECURITY/02-xss_injection.md)) |
...
| Document store (cf. le cas MongoDB de 05-niveau-3-backend.md)                     | CONTEXTUELLE    | tu gagnes un schéma flexible au démarrage, tu paies une migration ultérieure vers un modèle relationnel bien plus coûteuse que prévu si le schéma s'avère finalement structuré |
...
> **Réutilise** : [05-MAITRISE/01-DATABASES/04-data_modeling.md](../../../05-MAITRISE/01-DATABASES/04-data_modeling.md)
...
**Connexion activée.** Arbre binaire, complexité, hash table, race condition, transaction : cinq chapitres ce parcours viennent de fusionner en un seul modèle mental appelé "base de données".

---

<a id="48-testing-en-conditions-réelles"></a>### 4.8 : Testing en conditions réelles
...
ce parcours t'a appris la stratégie ([02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md](../../../02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md)). Voici l'outillage professionnel actuel.

- **Ancrage ce parcours** : [02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md](../../../02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md) : quoi tester, à quelle granularité, et pourquoi le runner est un détail.
...
- **Preuve que c'est acquis** : tu casses volontairement une implémentation sans casser le comportement, et un bon test ne bouge pas. **Si tu bloques, reviens à** : [02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md](../../../02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md).

#### Ce que ce parcours permet déjà de comprendre

- [02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md](../../../02-CONSTRUCTION/03-TESTING/10-test_strategy_not_framework.md) : quoi tester, à quelle granularité.
- [02-CONSTRUCTION/03-TESTING/04-mocking_madness.md](../../../02-CONSTRUCTION/03-TESTING/04-mocking_madness.md) : mocker la base, c'est tester ton mock.
- [02-CONSTRUCTION/03-TESTING/05-integration_reactor.md](../../../02-CONSTRUCTION/03-TESTING/05-integration_reactor.md) : le test qui traverse plusieurs couches est celui qui attrape les bugs réels.
- [01-CADRAGE/03-DEBUGGING/05-repro_before_fix.md](../../../01-CADRAGE/03-DEBUGGING/05-repro_before_fix.md) : un test qui échoue d'abord est la preuve qu'un correctif corrige quelque chose.
...
> **Réutilise** : [02-CONSTRUCTION/03-TESTING/04-mocking_madness.md](../../../02-CONSTRUCTION/03-TESTING/04-mocking_madness.md)
...
- **Ancrage ce parcours** : [02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md) : configuration par variables d'environnement et signaux : `SIGTERM` et l'arrêt gracieux sont exactement le sujet de `docker stop`.
...
- [02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md) : configuration par variables d'environnement et signaux.
- [00-SOCLE/01-GETTING-STARTED/04-shell_survival.md](../../../00-SOCLE/01-GETTING-STARTED/04-shell_survival.md) : uid, permissions, `ls -ln` : la moitié des `permission denied` d'un conteneur se lisent là.
- [02-CONSTRUCTION/05-MEMORY-PERFORMANCE/01_gc](../../../02-CONSTRUCTION/05-MEMORY-PERFORMANCE/01_gc/) : une limite mémoire de conteneur ne crée pas la fuite, elle la révèle plus tôt.
- [02-CONSTRUCTION/13-RUNTIME-ENV/04-commonjs_vs_esm.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/04-commonjs_vs_esm.md) : ce qui casse au build dans l'image casse pour les mêmes raisons qu'en local.
...
> **Réutilise** : un de tes mini-projets ce parcours ([02-CONSTRUCTION/02-MINI-PROJECTS/README.md](../../../02-CONSTRUCTION/02-MINI-PROJECTS/README.md))
...
- **Ancrage ce parcours** : [02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md) : la config vient de l'environnement, jamais de l'artefact.
...
- **Preuve que c'est acquis** : tu montres, sur ton propre déploiement, que le même artefact tourne en préprod et en prod avec uniquement la config qui change. **Si tu bloques, reviens à** : [02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md).

#### Ce que ce parcours permet déjà de comprendre

- [02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md) : la config vient de l'environnement, jamais de l'artefact.
- [03-PILOTAGE/05-OBSERVABILITY/07-debug_in_prod.md](../../../03-PILOTAGE/05-OBSERVABILITY/07-debug_in_prod.md) : un déploiement sans observation est un pari.
- [03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md](../../../03-PILOTAGE/04-SECURITY/08-supply_chain_sbom.md) : l'artefact promu contient tout ce que tu as installé, y compris ce que tu n'as pas choisi.
- [01-CADRAGE/04-ERROR-HANDLING/06-error_strategy.md](../../../01-CADRAGE/04-ERROR-HANDLING/06-error_strategy.md) : décider à l'avance ce qui déclenche un rollback est une stratégie d'erreur, pas une improvisation.
...
> **Réutilise** : [02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md](../../../02-CONSTRUCTION/13-RUNTIME-ENV/05-process_env_argv.md) : la config est lue au démarrage, validée, et le service refuse de démarrer si une variable obligatoire manque
...
[02-orientation.md](02-orientation.md) · [Sommaire](../README.md) · [04-niveau-2-frontend.md →](04-niveau-2-frontend.md)
