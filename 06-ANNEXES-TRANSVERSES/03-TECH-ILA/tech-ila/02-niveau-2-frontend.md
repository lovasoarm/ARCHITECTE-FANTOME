...
> **Tu dois déjà savoir** : closures et portée (`00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md`), fonctions pures (`02-CONSTRUCTION/09_functional_js/01_pure_functions.md`), les patrons d'architecture de base (`02-CONSTRUCTION/14_architecture_patterns/`)
...
- **Ancrage MyFunnyJS** : [`00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md`](../../../00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md), [`02-CONSTRUCTION/09_functional_js/01_pure_functions.md`](../../../02-CONSTRUCTION/09_functional_js/01_pure_functions.md), [`02-CONSTRUCTION/18_web_concepts/03_state_and_dataflow.md`](../../../02-CONSTRUCTION/18_web_concepts/03_state_and_dataflow.md)
...
- **Preuve que c'est acquis** : tu sais dire pourquoi un composant se réexécute et le montrer au profiler · **Si tu bloques, reviens à** : `00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md`
...
- [02_closure_trap.md](../../../00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md) : chaque rendu crée de nouvelles closures ; un effet mal câblé capture une valeur périmée.
- [02_reference_chaos.md](../../../00-SOCLE/04_fundamentals/01_variables/02_reference_chaos.md) : muter un objet d'état ne déclenche pas de rendu, parce que l'identité de la référence n'a pas changé.
- [01_pure_functions.md](../../../02-CONSTRUCTION/09_functional_js/01_pure_functions.md) : un composant est une fonction pure de ses props et de son état ; tout le reste est un effet.
- `01-CADRAGE/02_async/04_event_loop/` : pourquoi lire l'état juste après `setState` renvoie l'ancienne valeur.
- [05_race_condition_hunter.md](../../../05-MAITRISE/03_edge_cases/05_race_condition_hunter.md) : deux fetchs concurrents, le plus lent écrase le plus rapide.
- [06_detached_dom_leak.md](../../../02-CONSTRUCTION/05_memory_performance/01_gc/06_detached_dom_leak.md) : un abonnement non nettoyé au démontage est une fuite, pas un détail.
...
| -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Closures ([02_closure_trap.md](../../../00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md))                       | chaque rendu crée de nouvelles closures ; un handler dans un `useEffect` mal câblé capture une valeur périmée            |
| Références vs copies ([02_reference_chaos.md](../../../00-SOCLE/04_fundamentals/01_variables/02_reference_chaos.md)) | muter un objet d'état ne déclenche pas de rendu : l'identité n'a pas changé                                              |
| Pureté ([01_pure_functions.md](../../../02-CONSTRUCTION/09_functional_js/01_pure_functions.md))                             | un composant doit être une fonction pure de ses props et de son état                                                     |
| Event loop (`01-CADRAGE/02_async/04_event_loop/`)                                                                   | pourquoi les mises à jour d'état sont groupées et pourquoi lire l'état juste après `setState` te donne l'ancienne valeur |
| Fuites mémoire ([06_detached_dom_leak.md](../../../02-CONSTRUCTION/05_memory_performance/01_gc/06_detached_dom_leak.md))    | abonnement non nettoyé au démontage = fuite + `setState` sur composant démonté                                           |
| Race conditions ([05_race_condition_hunter.md](../../../05-MAITRISE/03_edge_cases/05_race_condition_hunter.md))         | deux fetchs concurrents, le plus lent écrase le plus rapide                                                              |
...
- L'accessibilité. `<div onClick>` n'est pas un bouton ([02_aria_basics.md](../../../03-PILOTAGE/02_web_inclusive/02_aria_basics.md)).
...
Le compteur monte à 1 et s'arrête. Aucun message d'erreur, aucune exception : c'est le symptôme le plus trompeur de React. Ce n'est **pas** un bug React. C'est `00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md`, exactement. L'effet s'exécute une fois, sa closure capture `count = 0`, et `0 + 1` vaut toujours 1.
...
Correctif : `AbortController` ([02c_abort_controller.md](../../../01-CADRAGE/02_async/03_async_await/02c_abort_controller.md)) ou un drapeau d'annulation dans le cleanup. Ou, mieux, une bibliothèque d'état serveur qui gère ça pour toi.
...
React échappe le texte par défaut. `dangerouslySetInnerHTML` retire cette protection : c'est du XSS direct si la source n'est pas assainie ([01_xss_injection.md](../../../03-PILOTAGE/04_security/01_xss_injection.md)). Le nom de l'API te prévient ; les gens l'utilisent quand même.
...
- **Source maps.** Sans elles, chaque trace est `a.min.js:1:48219`. Elles doivent être **envoyées au service de suivi** et **non servies publiquement** : sinon tu publies ton code source avec ton bundle. Le drill correspondant est [07_prod_stack_trace_drill.md](../../../03-PILOTAGE/05_observability/07_prod_stack_trace_drill.md).
...
- **Le piège de confidentialité.** Les outils de replay de session enregistrent les champs de formulaire. Sans masquage explicite, tu envoies des données personnelles à un tiers ([08_privacy_and_aiact.md](../../../03-PILOTAGE/04_security/08_privacy_and_aiact.md)).
...
> **Réutilise** : [07_prod_stack_trace_drill.md](../../../03-PILOTAGE/05_observability/07_prod_stack_trace_drill.md)
...
[02_closure_trap.md](../../../00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md) (l'effet qui capture une valeur périmée), [02_reference_chaos.md](../../../00-SOCLE/04_fundamentals/01_variables/02_reference_chaos.md) (muter l'état ne rerend pas), [05_race_condition_hunter.md](../../../05-MAITRISE/03_edge_cases/05_race_condition_hunter.md) (deux fetchs concurrents), [06_detached_dom_leak.md](../../../02-CONSTRUCTION/05_memory_performance/01_gc/06_detached_dom_leak.md) (l'abonnement non nettoyé), [05_sentry_in_prod.md](../../../03-PILOTAGE/05_observability/05_sentry_in_prod.md) et [07_prod_stack_trace_drill.md](../../../03-PILOTAGE/05_observability/07_prod_stack_trace_drill.md) (les deux sections ci-dessus), [02_aria_basics.md](../../../03-PILOTAGE/02_web_inclusive/02_aria_basics.md) (ce que React ne résout pas), [04_process_env_argv.md](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md) (la variable d'environnement inlinée).
...
> **Réutilise** : [05_race_condition_hunter.md](../../../05-MAITRISE/03_edge_cases/05_race_condition_hunter.md)
...
- **Ancrage MyFunnyJS** : [01_import_export.md](../../../00-SOCLE/04_fundamentals/06_modules/01_import_export.md), le graphe d'imports pilote le tree-shaking
...
- **Preuve que c'est acquis** : tu sais lire un rapport de bundle et dire quel import fait grossir quel chunk · **Si tu bloques, reviens à** : [01_import_export.md](../../../00-SOCLE/04_fundamentals/06_modules/01_import_export.md)
...
- [01_import_export.md](../../../00-SOCLE/04_fundamentals/06_modules/01_import_export.md) : ce que tu importes définit le graphe parcouru, donc ce qui survit au tree-shaking.
- [03_commonjs_vs_esm.md](../../../02-CONSTRUCTION/13_runtime_env/03_commonjs_vs_esm.md) : la moitié des erreurs de build sont un mélange CommonJS/ESM, pas un bug de Vite.
- [04_process_env_argv.md](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md) : une variable lue au build est inlinée donc publique ; une variable de runtime ne l'est pas.
- [07_prod_stack_trace_drill.md](../../../03-PILOTAGE/05_observability/07_prod_stack_trace_drill.md) : sans source maps, une stack trace minifiée est illisible.
...
> **Réutilise** : [01_import_export.md](../../../00-SOCLE/04_fundamentals/06_modules/01_import_export.md)
...
**Ce que tu dois comprendre, et qui survivra à Vite :** tree-shaking (élimination du code mort), code splitting (découpage par route), source maps (retrouver ta ligne d'origine dans un stack trace minifié : indispensable pour [07_prod_stack_trace_drill.md](../../../03-PILOTAGE/05_observability/07_prod_stack_trace_drill.md)), budget de bundle, et la différence entre variables d'environnement de build (inlinées, donc **publiques**) et de runtime.
...
- **Ancrage MyFunnyJS** : [03_state_and_dataflow.md](../../../02-CONSTRUCTION/18_web_concepts/03_state_and_dataflow.md), l'URL comme source de vérité
...
- **Preuve que c'est acquis** : un parcours complet au clavier sans souris, une URL qui reproduit exactement l'état visuel ailleurs · **Si tu bloques, reviens à** : [03_state_and_dataflow.md](../../../02-CONSTRUCTION/18_web_concepts/03_state_and_dataflow.md)

#### Ce que MyFunnyJS permet déjà de comprendre

- [03_state_and_dataflow.md](../../../02-CONSTRUCTION/18_web_concepts/03_state_and_dataflow.md) : une route est un état partageable ; l'URL est une source de vérité comme une autre.
- [02_aria_basics.md](../../../03-PILOTAGE/02_web_inclusive/02_aria_basics.md) : HTML sémantique d'abord, ARIA ensuite : la règle ne change pas avec le framework.
- [02_dates_timezones.md](../../../03-PILOTAGE/02_web_inclusive/08_i18n/02_dates_timezones.md) : un champ de date sans fuseau explicite produit des bugs invisibles en recette.
- [02_custom_errors.md](../../../01-CADRAGE/04_error_handling/02_custom_errors.md) : un message de validation est une erreur destinée à un humain, donc à concevoir.
...
> **Réutilise** : [03_state_and_dataflow.md](../../../02-CONSTRUCTION/18_web_concepts/03_state_and_dataflow.md)
...
- **Ancrage MyFunnyJS** : [07_seo_and_rendering.md](../../../02-CONSTRUCTION/18_web_concepts/07_seo_and_rendering.md), les quatre stratégies de rendu
...
- **Preuve que c'est acquis** : tu sais nommer, sans le framework, quelle stratégie de rendu convient à quel écran et pourquoi · **Si tu bloques, reviens à** : [07_seo_and_rendering.md](../../../02-CONSTRUCTION/18_web_concepts/07_seo_and_rendering.md)
...
- [07_seo_and_rendering.md](../../../02-CONSTRUCTION/18_web_concepts/07_seo_and_rendering.md) : pourquoi une page vide envoyée au navigateur est d'abord un problème d'indexation.
- [02_browser_render_pipeline.md](../../../02-CONSTRUCTION/18_web_concepts/02_browser_render_pipeline.md) : ce que « hydrater » veut dire concrètement dans le pipeline de rendu.
- [04_caching_strategies.md](../../../02-CONSTRUCTION/18_web_concepts/04_caching_strategies.md) : les caches empilés de Next sont les stratégies que tu connais, appliquées à trois niveaux.
- [01_import_export.md](../../../00-SOCLE/04_fundamentals/06_modules/01_import_export.md) : la fuite d'un secret côté client est une chaîne d'imports, pas un mauvais placement de fichier.
- [01_node_vs_browser.md](../../../02-CONSTRUCTION/13_runtime_env/01_node_vs_browser.md) : la frontière serveur/client existait avant le framework.
...
- Un code serveur n'est protégé que si aucune chaîne d'imports ne le tire côté client. Le bundler suit les imports, pas ton intention ([01_import_export.md](../../../00-SOCLE/04_fundamentals/06_modules/01_import_export.md)).
...
Le reste vaut comme partout : validation côté serveur ([01_xss_injection.md](../../../03-PILOTAGE/04_security/01_xss_injection.md)), cookies `SameSite`, en-têtes de sécurité.

#### Observabilité

Une application Next produit des erreurs des deux côtés de la frontière, et elles ne se lisent pas au même endroit : côté serveur dans les logs de l'hébergeur, côté client dans le suivi d'erreurs du navigateur. Un seul incident peut apparaître dans les deux, ou dans un seul. L'instrumentation doit donc porter un **identifiant de corrélation partagé** entre le rendu serveur et le navigateur, sinon tu compares deux histoires sans savoir qu'elles n'en font qu'une ([02_distributed_tracing.md](../../../03-PILOTAGE/05_observability/02_distributed_tracing.md)).

Le point spécifique à surveiller : les erreurs d'**hydratation**. Elles n'empêchent pas la page de s'afficher, elles la font diverger silencieusement. Cause n°1 : une valeur qui diffère entre serveur et client, presque toujours une date ou un fuseau ([02_dates_timezones.md](../../../03-PILOTAGE/02_web_inclusive/08_i18n/02_dates_timezones.md)) ou une lecture de `localStorage` pendant le rendu.
...
[07_seo_and_rendering.md](../../../02-CONSTRUCTION/18_web_concepts/07_seo_and_rendering.md) (les quatre stratégies de rendu), [04_caching_strategies.md](../../../02-CONSTRUCTION/18_web_concepts/04_caching_strategies.md) (les caches empilés), [01_node_vs_browser.md](../../../02-CONSTRUCTION/13_runtime_env/01_node_vs_browser.md) (la frontière existait avant le framework), [01_import_export.md](../../../00-SOCLE/04_fundamentals/06_modules/01_import_export.md) (la fuite de secret est une chaîne d'imports), [02_dates_timezones.md](../../../03-PILOTAGE/02_web_inclusive/08_i18n/02_dates_timezones.md) (l'erreur d'hydratation la plus fréquente), [02_distributed_tracing.md](../../../03-PILOTAGE/05_observability/02_distributed_tracing.md) (corréler serveur et client).
...
SSR, hydratation (le HTML rendu côté serveur est repris par le JS côté client pour devenir interactif), streaming, invalidation de cache, budget de performance, Core Web Vitals (`02-CONSTRUCTION/05_memory_performance/05_core_web_vitals/`). **Ce qui bougera** : la structure des dossiers, le nom des directives, la stratégie de cache par défaut. Tag **PÉRISSABLE** assumé sur les conventions du framework ; **NOYAU DURABLE** sur les stratégies de rendu elles-mêmes.
...
> **Réutilise** : [07_seo_and_rendering.md](../../../02-CONSTRUCTION/18_web_concepts/07_seo_and_rendering.md)
