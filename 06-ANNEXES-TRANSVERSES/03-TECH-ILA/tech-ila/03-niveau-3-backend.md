...
> **Tu dois déjà savoir** : `node:http` et les fondamentaux Node (`01-niveau-1-socle.md`), composition de fonctions (`02-CONSTRUCTION/09_functional_js/03_composition.md`), les patrons SOLID de base (`02-CONSTRUCTION/14_architecture_patterns/02_solid_principles.md`)
...
- **Ancrage MyFunnyJS** : [01_express_from_scratch.md](../../../02-CONSTRUCTION/19_api_craft/01_express_from_scratch.md), tu as déjà écrit le routeur et la chaîne de middlewares à la main
...
- **Preuve que c'est acquis** : tu sais écrire un middleware d'erreur unique qui voit toutes tes routes, y compris les async · **Si tu bloques, reviens à** : [01_express_from_scratch.md](../../../02-CONSTRUCTION/19_api_craft/01_express_from_scratch.md)
...
- [01_express_from_scratch.md](../../../02-CONSTRUCTION/19_api_craft/01_express_from_scratch.md) : tu as déjà écrit le routeur et la chaîne de middlewares à la main ; Express n'en fait pas plus.
- [03_composition.md](../../../02-CONSTRUCTION/09_functional_js/03_composition.md) : un middleware est une fonction composée, avec un droit de court-circuit.
- [04_currying.md](../../../02-CONSTRUCTION/09_functional_js/04_currying.md) : un middleware configurable est une factory qui retourne un handler.
- [04_async_error_traps.md](../../../01-CADRAGE/04_error_handling/04_async_error_traps.md) : la promesse rejetée qu'aucun middleware ne voit, et le client qui attend jusqu'au timeout.
- [02_closure_trap.md](../../../00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md) : une variable capturée au niveau module est partagée par **toutes** les requêtes.
...
C'est une **composition de fonctions** ([03_composition.md](../../../02-CONSTRUCTION/09_functional_js/03_composition.md)) où chaque maillon peut court-circuiter ou passer la main. Si tu as compris la composition et le pattern Chain of Responsibility (`02-CONSTRUCTION/10_design_patterns/`), tu as compris Express en dix minutes.

**Ce qu'il masque : la frontière async invisible.** Dans Express 4, une erreur jetée dans un handler `async` n'est **pas** attrapée par le gestionnaire d'erreurs : la promesse rejetée n'est vue par personne. Le client attend jusqu'au timeout, et un `unhandledRejection` non traité au niveau du process peut afficher `ERR_UNHANDLED_REJECTION` avant de faire crasher l'application. C'est [04_async_error_traps.md](../../../01-CADRAGE/04_error_handling/04_async_error_traps.md) en production. Correctif : un wrapper `asyncHandler`, ou Express 5, ou un framework qui gère l'async nativement.
...
> **Réutilise** : [04_async_error_traps.md](../../../01-CADRAGE/04_error_handling/04_async_error_traps.md)
...
- **Ancrage MyFunnyJS** : [02_solid_principles.md](../../../02-CONSTRUCTION/14_architecture_patterns/02_solid_principles.md), l'injection de dépendances est l'inversion de dépendance de SOLID
...
- **Preuve que c'est acquis** : tu sais réciter l'ordre exact middleware → guard → interceptor → pipe → handler → interceptor → filtre, et dire à quelle étape une requête invalide est rejetée · **Si tu bloques, reviens à** : [02_solid_principles.md](../../../02-CONSTRUCTION/14_architecture_patterns/02_solid_principles.md)
...
- **Décorateurs** : c'est le pattern Decorator ([01_decorator_pattern.md](../../../02-CONSTRUCTION/10_design_patterns/02_structural/01_decorator_pattern.md)) avec de la métadonnée. `@Injectable()` ne fait pas de magie : il marque une classe pour le conteneur.
- **Injection de dépendances** : c'est l'inversion de dépendance de SOLID ([02_solid_principles.md](../../../02-CONSTRUCTION/14_architecture_patterns/02_solid_principles.md)). Ta classe ne construit plus ses collaborateurs, on les lui donne. Conséquence pratique : tu peux tester en injectant un faux.
...
Ce que ça donne concrètement : un identifiant de corrélation généré à l'entrée, propagé dans le contexte, présent dans chaque ligne de log et dans chaque span ([01_structured_logging.md](../../../03-PILOTAGE/05_observability/01_structured_logging.md), [02_distributed_tracing.md](../../../03-PILOTAGE/05_observability/02_distributed_tracing.md)). Instrumenter après coup un Express écrit à la main demande de toucher trente fichiers ; ici, un.
...
Sur le reste, c'est un conteneur Node standard : image de base à jour, utilisateur non root, endpoint de santé distinct de l'endpoint de disponibilité ([04_load_balancing.md](../../../05-MAITRISE/02_scalability/04_load_balancing.md)).

##### Pont vers les modules MyFunnyJS

[02_solid_principles.md](../../../02-CONSTRUCTION/14_architecture_patterns/02_solid_principles.md) (la DI et le graphe de modules), [04_clean_architecture.md](../../../02-CONSTRUCTION/14_architecture_patterns/04_clean_architecture.md) (ce que la structure ne remplace pas), [02_singleton_pattern.md](../../../02-CONSTRUCTION/10_design_patterns/01_creational/02_singleton_pattern.md) (l'état dans un provider singleton), [02_closure_trap.md](../../../00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md) (la même fuite, vue côté closure), [01_structured_logging.md](../../../03-PILOTAGE/05_observability/01_structured_logging.md) et [02_distributed_tracing.md](../../../03-PILOTAGE/05_observability/02_distributed_tracing.md) (les interceptors ci-dessus), [02_custom_errors.md](../../../01-CADRAGE/04_error_handling/02_custom_errors.md) (ce qu'un exception filter doit formater).
...
- **Ancrage MyFunnyJS** : [04_auth_flows.md](../../../03-PILOTAGE/04_security/04_auth_flows.md), la différence entre prouver qui tu es et obtenir un droit
...
- **Preuve que c'est acquis** : tu as des tests qui prouvent qu'un utilisateur A ne peut ni lire, ni modifier, ni supprimer une ressource de B en étant authentifié · **Si tu bloques, reviens à** : [05_auth_authz.md](../../../02-CONSTRUCTION/18_web_concepts/05_auth_authz.md)

#### Ce que MyFunnyJS permet déjà de comprendre

- [04_auth_flows.md](../../../03-PILOTAGE/04_security/04_auth_flows.md) : la différence entre prouver qui tu es et obtenir un droit est déjà posée.
- [05_auth_authz.md](../../../02-CONSTRUCTION/18_web_concepts/05_auth_authz.md) : pourquoi vérifier la présence d'un token ne vérifie pas la propriété de la ressource.
- [04_auth_jwt.md](../../../02-CONSTRUCTION/19_api_craft/04_auth_jwt.md) : ce qu'un JWT contient, et pourquoi ce contenu vieillit mal.
- [05_hashing_bcrypt.md](../../../03-PILOTAGE/04_security/05_hashing_bcrypt.md) : sel, coût, algorithme dédié : un hachage rapide est un défaut, pas une optimisation.
- [01_nan_undefined_null.md](../../../05-MAITRISE/03_edge_cases/01_nan_undefined_null.md) : une valeur absente n'est pas une valeur fausse ; beaucoup de contournements d'autorisation viennent de là.
...
**Mots de passe.** Jamais en clair, jamais en SHA-256 nu. Algorithme dédié au hachage de mot de passe (bcrypt, scrypt, argon2), avec sel, coût paramétrable ([05_hashing_bcrypt.md](../../../03-PILOTAGE/04_security/05_hashing_bcrypt.md)). Et si tu peux déléguer à un fournisseur d'identité, délègue.
...
> **Réutilise** : [05_auth_authz.md](../../../02-CONSTRUCTION/18_web_concepts/05_auth_authz.md)
...
- **Ancrage MyFunnyJS** : `02-CONSTRUCTION/06_data_structures/`, les mêmes structures que tu connais, partagées entre processus et persistantes au redémarrage
...
- **Preuve que c'est acquis** : tu sais nommer, sans hésiter, ce que tu ne mets jamais dans Redis, et le compromis fraîcheur/coût de chaque clé que tu y places · **Si tu bloques, reviens à** : [04_redis_caching.md](../../../05-MAITRISE/01_databases/04_redis_caching.md)
...
- `02-CONSTRUCTION/06_data_structures/` : chaînes, hashes, ensembles, listes, ensembles triés : déjà implémentées, ici exposées par le réseau.
- [04_redis_caching.md](../../../05-MAITRISE/01_databases/04_redis_caching.md) : ce qu'on met en cache, pour combien de temps, et ce qu'on n'y met jamais.
- [04_caching_strategies.md](../../../02-CONSTRUCTION/18_web_concepts/04_caching_strategies.md) : TTL, invalidation, `stale-while-revalidate` : le compromis fraîcheur/coût est déjà connu.
- [05_race_condition_hunter.md](../../../05-MAITRISE/03_edge_cases/05_race_condition_hunter.md) : un `get` puis `set` sur un compteur partagé est une race condition, même côté client mono-thread.
- `02-CONSTRUCTION/05_memory_performance/01_gc/` : un cache sans limite de taille est une fuite mémoire avec un joli nom.
...
- **Ancrage MyFunnyJS** : [07_message_queues.md](../../../05-MAITRISE/02_scalability/07_message_queues.md), file, producteur, consommateur, livraison au moins une fois
...
- **Preuve que c'est acquis** : tu sais faire tourner deux workers en parallèle et prouver qu'aucun traitement n'est compté deux fois · **Si tu bloques, reviens à** : [07_message_queues.md](../../../05-MAITRISE/02_scalability/07_message_queues.md)
...
- [07_message_queues.md](../../../05-MAITRISE/02_scalability/07_message_queues.md) : file, producteur, consommateur, livraison au moins une fois : le modèle est déjà posé.
- [05_event_driven.md](../../../02-CONSTRUCTION/14_architecture_patterns/05_event_driven.md) : publier un événement au lieu d'appeler une fonction, et ce que ça coûte en traçabilité.
- [06_backpressure.md](../../../01-CADRAGE/02_async/06_backpressure.md) : un producteur plus rapide que ses workers finit toujours par remplir quelque chose.
- `02-CONSTRUCTION/10_design_patterns/` (patron Command) : un job est une commande sérialisée, c'est pour ça qu'il peut être rejoué.
- [05_error_strategy.md](../../../01-CADRAGE/04_error_handling/05_error_strategy.md) : quelle erreur mérite un retry et laquelle part en dead-letter est une stratégie, pas un réflexe.
...
> **Réutilise** : [07_message_queues.md](../../../05-MAITRISE/02_scalability/07_message_queues.md)
...
- **Ancrage MyFunnyJS** : `04-EPREUVE/03_realtime/`, connexion persistante, flux d'événements, reconnexion
...
- **Preuve que c'est acquis** : tu sais dire, pour ton cas, ce qui arrive aux messages émis pendant une coupure de 20 secondes, et pourquoi tu as choisi cette réponse · **Si tu bloques, reviens à** : [06_backpressure.md](../../../01-CADRAGE/02_async/06_backpressure.md)

#### Ce que MyFunnyJS permet déjà de comprendre

- `04-EPREUVE/03_realtime/` : le module entier : connexion persistante, flux d'événements, reconnexion.
- `02-CONSTRUCTION/10_design_patterns/` (patron Observer) : un abonnement sans désabonnement est une fuite, ici multipliée par le nombre de clients.
- [06_backpressure.md](../../../01-CADRAGE/02_async/06_backpressure.md) : émettre plus vite que le client ne consomme fait monter la mémoire du serveur, pas celle du client.
- [02_closure_trap.md](../../../00-SOCLE/04_fundamentals/02_scope/02_closure_trap.md) : un handler de socket qui capture l'état d'un utilisateur le garde en vie après sa déconnexion.
...
- **La backpressure.** Si tu émets plus vite que le client ne consomme, la mémoire serveur monte. Tu as déjà vu ça : [06_backpressure.md](../../../01-CADRAGE/02_async/06_backpressure.md).
...
> **Réutilise** : `04-EPREUVE/03_realtime/`
...
- **Ancrage MyFunnyJS** : [05_graphql_basics.md](../../../02-CONSTRUCTION/19_api_craft/05_graphql_basics.md), schéma, requête, résolveur
...
- **Preuve que c'est acquis** : tu sais activer le log SQL, compter les requêtes réellement émises, et les ramener à un ordre de grandeur constant avec un DataLoader · **Si tu bloques, reviens à** : [05_db_in_js.md](../../../05-MAITRISE/01_databases/05_db_in_js.md)
...
- [05_graphql_basics.md](../../../02-CONSTRUCTION/19_api_craft/05_graphql_basics.md) : schéma, requête, résolveur : les trois pièces sont déjà connues.
- `02-CONSTRUCTION/06_data_structures/` (graphes, BFS/DFS) : une requête GraphQL est un parcours de graphe, et sa profondeur est ton coût.
- [05_db_in_js.md](../../../05-MAITRISE/01_databases/05_db_in_js.md) : le N+1 vient de ce qu'un résolveur ignore ce que son voisin a déjà chargé.
- [04_caching_strategies.md](../../../02-CONSTRUCTION/18_web_concepts/04_caching_strategies.md) : ce que tu perds en faisant passer tout le trafic par un `POST` unique.
- `03-PILOTAGE/04_security/` : l'autorisation par champ est le même raisonnement que par endpoint, appliqué N fois.
...
> **Réutilise** : [05_db_in_js.md](../../../05-MAITRISE/01_databases/05_db_in_js.md)
