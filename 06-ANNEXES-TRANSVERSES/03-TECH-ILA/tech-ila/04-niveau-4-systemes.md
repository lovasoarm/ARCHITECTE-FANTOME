...
**Tag : NOYAU DURABLE** (le concept) / **PÉRISSABLE** (la syntaxe YAML) · Coût : ~15 h avant utilité · Durée de vie : ~10 ans (le modèle) · À apprendre après : `02-CONSTRUCTION/03_testing/` complet · Prérequis : `02-CONSTRUCTION/03_testing/`, [`03-PILOTAGE/10_team_craft/01_code_review.md`](../../../03-PILOTAGE/10_team_craft/01_code_review.md)
...
- [`02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md`](../../../02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md) : l'ordre des étapes d'un pipeline est une stratégie de test, pas une contrainte d'outil.
- [`01-CADRAGE/03_debugging/07_flaky_bugs.md`](../../../01-CADRAGE/03_debugging/07_flaky_bugs.md) : un test instable est un bug de non-déterminisme, jamais une raison de relancer.
- `03-PILOTAGE/10_team_craft/01_code_review.md` : le pipeline automatise le vérifiable pour que la revue parle du reste.
- [`03-PILOTAGE/04_security/09_supply_chain_sbom.md`](../../../03-PILOTAGE/04_security/09_supply_chain_sbom.md) : le CI a accès à tes secrets et installe du code tiers : c'est ta chaîne d'approvisionnement.
- [`00-SOCLE/01_getting_started/04_package_managers.md`](../../../00-SOCLE/01_getting_started/04_package_managers.md) : le lockfile est ce qui rend un build reproductible d'une machine à l'autre.
...
**Tests flaky (instables) en CI.** Un test qui échoue une fois sur vingt détruit la confiance de l'équipe : les gens relancent au lieu de lire. C'est `01-CADRAGE/03_debugging/07_flaky_bugs.md` en contexte d'équipe. Traite-les comme des bugs de production : isole, reproduis, corrige ou supprime. Jamais "relance".

**Sécurité du pipeline : épinglage par SHA, pas de folklore.** Le CI a accès à tes secrets de déploiement. `action/checkout@vN` est un tag mouvant : son propriétaire peut repointer `vN` vers un commit malveillant demain matin, et ton pipeline exécutera ce code avec tes secrets sans qu'aucune ligne n'ait changé chez toi. `action/checkout@<sha40>` (le SHA complet) est un commit précis, immuable : c'est la seule garantie que l'action que tu exécutes aujourd'hui est celle que tu as auditée. C'est exactement le scénario `03-PILOTAGE/04_security/09_supply_chain_sbom.md` appliqué à ton CI. En complément : limite les permissions du token au strict nécessaire (`permissions: contents: read` par défaut), et ne donne jamais les secrets de prod aux pipelines déclenchés par une PR venant d'un fork.
...
> **Réutilise** : `02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md`
...
| Coût     | la facture est une métrique d'architecture ([`05-MAITRISE/06_annexes/03_finops_greenops.md`](../../../05-MAITRISE/06_annexes/03_finops_greenops.md))                                                                                                    | calcule le coût mensuel à partir d'une grille tarifaire figée (voir 7.2 bis)                  |
...
- [`02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md`](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md) : config et secrets viennent de l'environnement : c'est la base du 12-factor (douze règles de conception pour une application cloud portable, dont « la config vit dans l'environnement, jamais dans le code ») et de tout déploiement cloud.
- [`05-MAITRISE/02_scalability/01_distributed_thinking.md`](../../../05-MAITRISE/02_scalability/01_distributed_thinking.md) : ce que tu gagnes en répliquant et ce que tu perds en cohérence.
- `05-MAITRISE/06_annexes/03_finops_greenops.md` : la facture est une métrique d'architecture, mesurable comme la latence.
- [`02-CONSTRUCTION/05_memory_performance/00_measure_first.md`](../../../02-CONSTRUCTION/05_memory_performance/00_measure_first.md) : dimensionner sans mesurer produit soit une facture inutile, soit un incident.
- [`03-PILOTAGE/04_security/09_supply_chain_sbom.md`](../../../03-PILOTAGE/04_security/09_supply_chain_sbom.md) : le moindre privilège s'applique aux rôles cloud comme aux dépendances.
...
> **Réutilise** : `02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md` : le même artefact doit tourner dans les deux cas, seule la configuration change.
...
**Tag : NOYAU DURABLE** · Coût : ~12 h avant utilité · Durée de vie : ~10 ans (les trois piliers ne changent pas) · À apprendre après : 7.1 CI/CD · Prérequis : module `03-PILOTAGE/05_observability/` complet

Le module MyFunnyJS t'a donné les trois piliers. Voici l'outillage et les décisions.

#### Ce que MyFunnyJS permet déjà de comprendre

- [`03-PILOTAGE/05_observability/01_structured_logging.md`](../../../03-PILOTAGE/05_observability/01_structured_logging.md) : un log est une donnée requêtable, pas une phrase.
- [`03-PILOTAGE/05_observability/02_distributed_tracing.md`](../../../03-PILOTAGE/05_observability/02_distributed_tracing.md) : la propagation de contexte à travers les services et les files.
- [`03-PILOTAGE/05_observability/04_metrics_alerting.md`](../../../03-PILOTAGE/05_observability/04_metrics_alerting.md) : pourquoi la moyenne cache exactement les utilisateurs qui souffrent.
- [`03-PILOTAGE/05_observability/07_prod_stack_trace_drill.md`](../../../03-PILOTAGE/05_observability/07_prod_stack_trace_drill.md) : lire une trace de production sans le code sous les yeux.
- [`02-CONSTRUCTION/05_memory_performance/00_measure_first.md`](../../../02-CONSTRUCTION/05_memory_performance/00_measure_first.md) : sans mesure préalable, une optimisation est une croyance.
...
> **Réutilise** : `03-PILOTAGE/05_observability/07_prod_stack_trace_drill.md`
...
**Tag : NOYAU DURABLE** · Coût : ~15 h avant utilité · Durée de vie : ~15 ans (les patrons sont indépendants du langage) · À apprendre après : 7.3 Observabilité · Prérequis : [`05-MAITRISE/02_scalability/03_distributed_fallacies.md`](../../../05-MAITRISE/02_scalability/03_distributed_fallacies.md), `05-MAITRISE/03_edge_cases/`

#### Ce que MyFunnyJS permet déjà de comprendre

- `05-MAITRISE/02_scalability/03_distributed_fallacies.md` : le réseau n'est pas fiable, la latence n'est pas nulle : tous les patrons ci-dessous en découlent.
- `05-MAITRISE/03_edge_cases/` : les cas limites chassés en local deviennent des pannes partielles en distribué.
- [`01-CADRAGE/02_async/02_promises/01_promise_race.md`](../../../01-CADRAGE/02_async/02_promises/01_promise_race.md) : un timeout est une course entre ton appel et une horloge.
- [`02-CONSTRUCTION/04_math_basics/05_probability_random.md`](../../../02-CONSTRUCTION/04_math_basics/05_probability_random.md) : le jitter d'un retry est du hasard utile : sans lui, tous tes clients réessaient à la même seconde.
- [`01-CADRAGE/04_error_handling/05_error_strategy.md`](../../../01-CADRAGE/04_error_handling/05_error_strategy.md) : dégrader proprement est une décision d'erreur, prise avant l'incident.
...
**Micro-services : la position honnête.** Ils résolvent un problème **organisationnel** (des équipes qui veulent déployer indépendamment) au prix d'un problème **technique** (le réseau, la cohérence, l'observabilité, la latence). Une équipe de cinq personnes avec sept micro-services a acheté tous les coûts sans aucun bénéfice. Le monolithe modulaire est un choix professionnel respectable et souvent le bon ([`02-CONSTRUCTION/14_architecture_patterns/06_microservices_intro.md`](../../../02-CONSTRUCTION/14_architecture_patterns/06_microservices_intro.md)).
...
> **Réutilise** : [`03-PILOTAGE/05_observability/08_oncall_drill.md`](../../../03-PILOTAGE/05_observability/08_oncall_drill.md)
...
**Tag : NOYAU DURABLE** · Coût : ~10 h avant utilité · Durée de vie : ~10 ans · À apprendre après : 7.4 Résilience · Prérequis : module `03-PILOTAGE/04_security/` complet
...
- **Journalisation d'audit** sur les actions sensibles : et **jamais** de données personnelles ou de secrets dans les logs (RGPD, [`03-PILOTAGE/04_security/08_privacy_and_aiact.md`](../../../03-PILOTAGE/04_security/08_privacy_and_aiact.md)).
...
- **Ancrage MyFunnyJS** : [`03-PILOTAGE/04_security/09_supply_chain_sbom.md`](../../../03-PILOTAGE/04_security/09_supply_chain_sbom.md) et [`02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md`](../../../02-CONSTRUCTION/13_runtime_env/04_process_env_argv.md) : la config et les secrets viennent de l'environnement, jamais du code.
...
> **Réutilise** : `03-PILOTAGE/04_security/09_supply_chain_sbom.md`
...
- [`03-PILOTAGE/04_security/01_xss_injection.md`](../../../03-PILOTAGE/04_security/01_xss_injection.md) : la zone protégée et la zone exposée de chaque framework, et pourquoi une CSP est une seconde barrière.
- [`03-PILOTAGE/04_security/02_csrf_cors.md`](../../../03-PILOTAGE/04_security/02_csrf_cors.md) : CORS n'est pas une protection serveur ; `SameSite` en est une.
- `03-PILOTAGE/04_security/09_supply_chain_sbom.md` : le scan en CI et le SBOM formalisent ce que tu as déjà audité à la main.
- [`03-PILOTAGE/04_security/08_privacy_and_aiact.md`](../../../03-PILOTAGE/04_security/08_privacy_and_aiact.md) : une donnée personnelle dans un log est une fuite avec rétention.
- [`03-PILOTAGE/05_observability/01_structured_logging.md`](../../../03-PILOTAGE/05_observability/01_structured_logging.md) : une journalisation d'audit non structurée n'est pas exploitable en incident.
...
> **Mode** : assistant autorisé
> **Contraintes** : rédige un ADR complet suivant [`03-PILOTAGE/10_team_craft/02_adr_writing.md`](../../../03-PILOTAGE/10_team_craft/02_adr_writing.md), à partir des mesures de l'exercice de 7.2. Ajoute la contrainte suivante : réécris la décision en 5 lignes pour un responsable produit, sans un seul nom de techno.
> **Réutilise** : `03-PILOTAGE/10_team_craft/02_adr_writing.md`
...
> **Mode** : assistant autorisé
> **Contraintes** : chronologie, impact, cause racine, action corrective, action préventive : format [`03-PILOTAGE/05_observability/08_oncall_drill.md`](../../../03-PILOTAGE/05_observability/08_oncall_drill.md). Ajoute la version 5 lignes sans nom de techno pour un responsable produit.
> **Réutilise** : `03-PILOTAGE/05_observability/08_oncall_drill.md`
...
Tant que tu es à gauche, la réponse est un index, une requête réécrite, un cache, une réplique de lecture (voir [`05-MAITRISE/01_databases/`](../../../05-MAITRISE/01_databases/00_why_databases.md)). Sortir l'artillerie avant ce seuil, c'est la sur-ingénierie décrite au [niveau 3 à propos des files massives](./03-niveau-3-backend.md) : trois outils de plus à opérer pour un problème que la base réglait.
...
- Une tâche relancée qui n'est pas idempotente double les lignes. Exactement le problème de [`05-MAITRISE/02_scalability/07_message_queues.md`](../../../05-MAITRISE/02_scalability/07_message_queues.md), avec des millions de lignes au lieu d'un message.
...
- La facture. Un calcul distribué mal cadré consomme des heures-machine sans plafond naturel : c'est le sujet de `05-MAITRISE/06_annexes/03_finops_greenops.md`, en plus brutal.
...
- **Ancrage MyFunnyJS** : `05-MAITRISE/06_annexes/03_finops_greenops.md` : décrire l'infra en code rend la facture relisible en PR
...
**Jaeger / Grafana / Datadog / Sentry** : Tag : PÉRISSABLE / CONTEXTUELLE (le backend change, OpenTelemetry reste) · Coût : ~4 h par outil · Durée de vie : ~4 ans · À apprendre après : 7.3 Observabilité : ce que ça change côté mécanisme MyFunnyJS : aucun, ce sont des visualisations de ce que `03-PILOTAGE/05_observability/02_distributed_tracing.md` décrit déjà.

**OpenTelemetry** : Tag : NOYAU DURABLE · Coût : ~8 h avant utilité · Durée de vie : ~10 ans · À apprendre après : module `03-PILOTAGE/05_observability/`

- **Ancrage MyFunnyJS** : `03-PILOTAGE/05_observability/02_distributed_tracing.md` : c'est l'implémentation standard de la propagation de contexte
...
**Renovate / Dependabot** : Tag : PROFESSIONNELLE · Coût : ~2 h avant utilité · Durée de vie : ~6 ans · À apprendre après : 7.1 : ce que ça change côté mécanisme MyFunnyJS : automatise `00-SOCLE/01_getting_started/04_package_managers.md`, ne remplace pas la lecture du changelog avant de merger.

**Testcontainers** : Tag : PROFESSIONNELLE · Coût : ~4 h avant utilité · Durée de vie : ~6 ans · À apprendre après : 7.1

- **Ancrage MyFunnyJS** : `02-CONSTRUCTION/03_testing/09_test_strategy_not_framework.md` : tester contre un vrai PostgreSQL en conteneur plutôt qu'un mock
