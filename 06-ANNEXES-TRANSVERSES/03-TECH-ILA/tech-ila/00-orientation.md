...
**Usage 3 : décision.** Tu dois choisir un outil. Tu ouvres la fiche techno, tu lis "Quand ne pas la choisir" et "Alternatives", tu écris un ADR (`03-PILOTAGE/10_team_craft/02_adr_writing.md`).
...
- **RAPPEL** (backticks, non cliquable) : toute mention ultérieure du même chemin dans la même fiche, ou une mention de dossier générique (`03-PILOTAGE/04_security/`).

Un backtick est donc un rappel volontaire, jamais un oubli. Et un chemin d'annexe s'écrit TOUJOURS préfixé de `05-MAITRISE/06_annexes/` : les sous-dossiers d'annexes ont leur propre numérotation, indépendante de celle des modules d’origine MyFunnyJS (comptage historique).
...
MyFunnyJS a déjà posé ce cadre dans `00-SOCLE/03_referentiel/06_intemporel_vs_perissable.md` et `05-MAITRISE/06_annexes/20_PERISSABILITE.md`. TECH-ILA applique la même grille aux technologies.
