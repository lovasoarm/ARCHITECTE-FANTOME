...
```text
Matrice risque : incertitude x coût si l'hypothèse est fausse
...
## Cas chiffré : le prix de retarder le risque

Sur le projet de bibliothèque d'escalade, retarder la vérification de l'historique des chutes
de six semaines a coûté environ 40 heures d'écrans de catalogue et d'adhérents à retravailler
partiellement, une fois le vrai modèle de données connu (il fallait finalement une table
d'événements par corde, pas un simple compteur). Vérifier l'hypothèse en 2 jours dès le
départ aurait coûté 2 jours et évité ces 40 heures de retouche, soit un ratio d'environ 1 pour
20 entre le coût de vérification anticipée et le coût de la découverte tardive.

## Comment prioriser plusieurs risques élevés à la fois

Un projet réel a rarement un seul risque : il en a souvent trois ou quatre en même temps. La
règle n'est pas de tous les attaquer en parallèle (ça dilue l'attention et retarde toutes les
réponses), mais de les ordonner par ce qui bloque le plus de décisions en aval.

```text
Risque A : le format de donnees terrain est-il exploitable ?      --> bloque le modele de donnees
Risque B : le prestataire de paiement supporte-t-il les avoirs ?  --> bloque le flux de facturation
Risque C : le design d'ecran plait-il aux utilisateurs ?          --> bloque rien de structurel

Ordre d'attaque : A d'abord (bloque tout le reste), puis B (bloque
une partie), puis C (raffinable a tout moment sans casser le reste)
```

### Contre-exemple : attaquer un risque qui n'en était pas un

L'excès inverse existe : traiter comme "risque le plus cher" une hypothèse qui, en fait, était
déjà largement vérifiée ailleurs. Sur un autre projet de bibliothèque de club d'escalade, un
développeur a passé une semaine entière à "vérifier empiriquement" que PostgreSQL pouvait
stocker un historique de plusieurs milliers de lignes sans problème de performance, une
question déjà tranchée par des années d'usage documenté à cette échelle. Cette semaine aurait
dû être investie sur la vraie inconnue du projet (le format des étiquettes de cordes), qui,
elle, n'avait jamais été vérifiée nulle part. Le risk-first ne veut pas dire "vérifier tout ce
qui pourrait théoriquement casser", il veut dire vérifier ce qui est réellement incertain
dans ce contexte précis.

```text
   Fausse alerte : question déjà tranchée par la littérature ou l'expérience
   collective (PostgreSQL et quelques milliers de lignes) --> ne pas vérifier,
   ce temps n'apprend rien de nouveau

   Vraie inconnue : question spécifique à ce projet, jamais vérifiée nulle part
   (format des étiquettes de CE club, avec CES cordes) --> vérifier en premier
```

### Comment distinguer une vraie inconnue d'une fausse alerte

Une question simple permet de trier : "existe-t-il déjà, dans la littérature technique ou
dans l'expérience de l'équipe, une réponse fiable à cette question, ou est-elle spécifique à
ce contexte précis et jamais rencontrée ?" Si la réponse existe déjà ailleurs de façon fiable,
ce n'est pas un risque à attaquer, c'est une connaissance à appliquer. Confondre les deux fait
perdre du temps sur des vérifications rassurantes mais inutiles, pendant que la vraie
inconnue continue d'attendre en silence.
