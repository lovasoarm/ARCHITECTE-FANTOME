...
1. Reprends le tableau de ton `06A-BUDGET-CLOUD.md`, palier 10 000, colonnes de traçabilité comprises ([06A-BUDGET-CLOUD.md](06A-BUDGET-CLOUD.md)).
2. Refais la même facture chez le second fournisseur que tu as déjà relevé et daté à l'exercice 2 de [04-choisir_fournisseur.md](04-choisir_fournisseur.md). Aucun nouveau relevé n'est nécessaire : le protocole de la donnée sourcée t'a déjà fait faire le travail.
...
Une section `## Portage` ajoutée à ton `06A-BUDGET-CLOUD.md` : le second tableau daté et sourcé, la liste "change de nature", la phrase de réversibilité. Mode de vérification : critère binaire du [97-CHECKPOINT-PACK/02-drill_2.md](97-CHECKPOINT-PACK/02-drill_2.md).

## 4. Même raisonnement hors JavaScript

Le portage demandé ici ne se limite pas au fournisseur. Reprends les mêmes
postes de coût et demande-toi comment le langage d’exécution modifie le profil
d'exploitation, sans transformer le langage en variable magique :

- **Python** : le coût reste gouverné par le profil d'usage (CPU, egress, stockage,
  appels managés), mais une charge CPU plus lente peut déplacer le point de
  bascule économique.
- **Go** : une image de conteneur très réduite peut diminuer certains coûts de
  démarrage, de registre et de déploiement ; ce n'est pas une réduction de coût
  universelle, car la charge et l'architecture restent déterminantes.
- **Rust** : une empreinte CPU/mémoire potentiellement plus faible peut déplacer
  le seuil auquel l'auto-hébergement devient intéressant, sans supprimer les
  coûts d'egress, de stockage chaud ou de services managés.

**Test de transfert :** ferme le fichier et réécris de mémoire les trois invariants
qui restent vrais même si JavaScript, Python, Go et Rust disparaissent demain.
...
Le [grimoire](90-grimoire.md) fige le vocabulaire, puis le [challenge](95-challenge.md) produit ton `06A-BUDGET-CLOUD.md` complet, portage inclus.

> Ce module fournit la moitie fournisseur du livrable note du palier 04-EPREUVE. L'autre moitie (second langage) vient des `99-PORTAGE-MENTAL.md`. Le livrable lui-meme et son exemplaire rempli : [06-ANNEXES-TRANSVERSES/17-PREUVES-MODELES/99-PORTAGE-MENTAL.md](../../06-ANNEXES-TRANSVERSES/17-PREUVES-MODELES/99-PORTAGE-MENTAL.md).
