...
L'un cede par lassitude ou la              de [contrainte que je vois] ?"
...
## Exercice : une même décision, trois publics, zéro contradiction

Communiquer sous pression ne consiste pas à raconter une histoire différente à chaque
public : ça consiste à choisir, pour chaque public, ce qu'il a besoin de savoir pour agir,
sans jamais dire à l'un ce qui contredirait ce que sait déjà l'autre. Le test de qualité
est simple : si les trois personnes comparaient leurs versions, aucune ne devrait se sentir
trompée.

Prends une décision d'architecture réelle : l'équipe du cabinet vétérinaire a découvert que
le stockage des créneaux en intervalles ouverts (proposé par Farida en ouverture de leçon)
provoque un ralentissement du calcul de charge journalière, et décide de revenir à des
créneaux fixes de 15 minutes, quitte à gérer les urgences autrement.

**Pour un pair technique (10 lignes, tous les détails utiles à la prochaine décision) :**

```text
On repart sur des créneaux fixes de 15 minutes au lieu des intervalles ouverts
qu'on avait commencés à coder. Le calcul de charge journalière sur intervalles
ouverts obligeait à un balayage O(n) sur tous les rendez-vous à chaque
affichage du planning, alors qu'avec des créneaux fixes on peut précalculer un
compteur par créneau et le maintenir à jour à l'écriture. Sur les 3 cliniques
pilotes, le balayage prenait déjà 400ms de plus par affichage, et ça grandit
linéairement avec le volume de rendez-vous. Les urgences insérées entre deux
créneaux seront gérées par un champ `isUrgent` sur un créneau existant plutôt
que par un intervalle libre : ça perd un peu de flexibilité horaire, mais ça
garde la performance de lecture. Le code des intervalles ouverts est encore
dans la branche `feature/open-slots`, à ne pas merger. Prévoir une migration
des 12 créneaux déjà créés en intervalles ouverts en environnement de test.
```

**Pour un product manager (5 lignes, l'impact sur le planning, pas l'implémentation) :**

```text
On change d'approche technique pour les créneaux de rendez-vous : on garde des
créneaux fixes de 15 minutes plutôt que des horaires totalement libres, pour
éviter un ralentissement du planning déjà mesuré en test. Impact planning :
un jour de retard sur la livraison prévue jeudi, le temps d'adapter la gestion
des urgences à ce nouveau modèle. Aucun risque sur la date de mise en service
avec les cliniques pilotes la semaine suivante.
```

**Pour la direction (3 lignes, coût et risque, rien de plus) :**

```text
Ajustement technique sur le planning des rendez-vous : un jour de retard,
sans impact sur la date de lancement avec les cliniques pilotes. Coût :
une journée de développement supplémentaire, déjà absorbée dans la marge du
sprint. Risque : faible, le changement évite un problème de performance
qu'on aurait sinon découvert après le lancement, avec un coût de correction
plus élevé.
```

Vérifie la cohérence des trois versions avec une question simple : est-ce que le pair
technique, le PM et la direction, s'ils comparaient leurs trois textes, y trouveraient un
seul fait contradictoire ? Ici, non : le retard d'un jour, la cause (performance), et
l'absence d'impact sur le lancement apparaissent dans les trois versions, seul le niveau de
détail change. Une version qui minimiserait le retard à la direction alors que le PM sait
qu'il fait un jour serait le signe d'une communication qui ne survivrait pas à une question
croisée entre les trois publics.
...
Analogie : communiquer sous pression, c'est l'annonce au passe pendant le rush, et le message radio en avarie.
Où l'analogie casse : le message radio a une syntaxe imposée, tes canaux d'équipe n'en ont aucune tant que tu ne l'écris pas.
