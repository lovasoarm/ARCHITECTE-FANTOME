...
## Livrable additionnel : même décision, trois versions

Prends **une** décision technique déjà prise sur ton projet fil rouge et rédige-la trois fois,
pour trois publics, sans jamais changer le fond :

1. **Un pair** : mécanisme, compromis, coût technique assumé. Vocabulaire technique autorisé.
2. **Un PM** : effet sur le périmètre, la date et le risque de livraison. Aucun terme technique
   non traduit ; chaque terme conservé est défini en une demi-ligne.
3. **La direction** : coût, risque, alternative écartée, et ce qu'on obtient en échange. Cinq
   lignes maximum, la recommandation en première phrase.

Validation : donne les trois versions à relire à une personne du public visé quand c'est
possible. Une version est ratée si le lecteur doit poser une question pour comprendre ce qu'on
lui demande de faire, ou s'il n'y a aucun coût mentionné.

Ce qui invalide cette partie : trois textes qui disent des choses différentes, ou une version
« direction » obtenue en supprimant des paragraphes de la version « pair » sans les traduire.
...
6 h, égal à la durée de ce niveau dans [CURRICULUM.md](../../06-ANNEXES-TRANSVERSES/archives/CURRICULUM-projectfunny.md). Si tu dépasses
