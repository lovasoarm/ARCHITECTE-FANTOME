...
La correction : faire de la propagation du contexte de trace une responsabilité du framework HTTP partagé (middleware commun), pas une chose que chaque dev doit se souvenir d'ajouter manuellement à chaque nouveau service. Une checklist d'intégration d'un nouveau service doit inclure "propage le traceId" au même titre que "expose un endpoint /health" (vu dans `05-MAITRISE/02-SCALABILITY/04_load_balancing` section 5).
