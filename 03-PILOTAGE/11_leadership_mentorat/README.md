---
stability: intemporel
acte: restituer
---

# 03-PILOTAGE/11_leadership_mentorat : influence sans autorité

> Mode de vérification des exercices de ce module : défense orale enregistrée contre grille ([../../05-MAITRISE/08_maitrise_staff_engineer/challenge.md](../../05-MAITRISE/08_maitrise_staff_engineer/challenge.md)).

Temps de lecture ~4 min

## Ce que c'est

Le module qui travaille le seul levier réel d'un Staff Engineer : d'autres gens décident mieux après lui avoir parlé. Revue de code qui transmet un critère, mentorat qui laisse une trace, standard qu'un outil vérifie, traduction d'une décision par public, et aveu d'ignorance qui renforce au lieu d'affaiblir.

## Structure du module

- [00_why_leadership_mentorat.md](00_why_leadership_mentorat.md) : pourquoi ce module mérite ton temps.
- [00_prereq_check.md](00_prereq_check.md) : filtre anti-illusion avant d'entrer.
- [01_mentorat_solo.md](01_mentorat_solo.md) : le mode solo, mentoré fictif documenté, trois productions à réviser.
- [02_revue_de_code_trois_niveaux.md](02_revue_de_code_trois_niveaux.md) : bloquant, suggestion, question.
- [03_mentorat_protocole.md](03_mentorat_protocole.md) : le protocole en quatre temps, avec trace écrite.
- [04_standards_equipe.md](04_standards_equipe.md) : un standard sans automatisation est un vœu.
- [05_expliquer_trois_publics.md](05_expliquer_trois_publics.md) : la matrice de traduction.
- [06_aveu_ignorance.md](06_aveu_ignorance.md) : dire "je ne sais pas" avec méthode et date.
- [07_standards_pour_agents.md](07_standards_pour_agents.md) : les mêmes standards, appliqués à une flotte d'agents.
- [08_EXO_JEUNE_IA.md](08_EXO_JEUNE_IA.md) : rédiger un protocole de mentorat sans IA, puis mesurer l'écart.
- [verification_pack/criteres.md](verification_pack/criteres.md) : les trois drills et leurs critères binaires.
- [grimoire.md](grimoire.md) : mémo dense. Malgré son numéro, il se lit **en dernier**, après les leçons et les exercices.
- [challenge.md](challenge.md) : challenge, les deux pièces de la preuve S5.
- [boss-fight.md](boss-fight.md) : boss fight, trois personnes, aucune autorité.

## Comment lire ce module

Dans l'ordre affiché ci-dessus, pas dans l'ordre des numéros de fichiers : le grimoire porte le numéro 01 pour raisons historiques, il se lit à la fin.

## Signal que tu es prêt pour la suite

Les trois drills du [verification_pack](verification_pack/criteres.md) sont REUSSI, et tu as écrit une revue à trois niveaux avec un risque nommé, une note d'une page sans jargon avec des chiffres sourcés, et tu as tenu un standard face à trois oppositions sans t'appuyer sur une autorité hiérarchique.

<!-- CONTENU-DOSSIER:debut (genere par outils/generer_index_dossiers.mjs) -->

## Contenu du dossier

Liste generee : tout fichier de `03-PILOTAGE/11_leadership_mentorat` est joignable depuis ici, aucun document n'est laisse sans porte d'entree.

- [00_prereq_check.md](00_prereq_check.md)
- [00_why_leadership_mentorat.md](00_why_leadership_mentorat.md)
- [01_mentorat_solo.md](01_mentorat_solo.md)
- [02_revue_de_code_trois_niveaux.md](02_revue_de_code_trois_niveaux.md)
- [03_mentorat_protocole.md](03_mentorat_protocole.md)
- [04_standards_equipe.md](04_standards_equipe.md)
- [05_expliquer_trois_publics.md](05_expliquer_trois_publics.md)
- [06_aveu_ignorance.md](06_aveu_ignorance.md)
- [07_standards_pour_agents.md](07_standards_pour_agents.md)
- [08_EXO_JEUNE_IA.md](08_EXO_JEUNE_IA.md)
- [08_EXO_JEUNE_IA.md](08_EXO_JEUNE_IA.md)
- [boss-fight.md](boss-fight.md)
- [challenge.md](challenge.md)
- [defense-orale.md](defense-orale.md)
- [grimoire.md](grimoire.md)
- [verification_pack/](verification_pack/README.md)

<!-- CONTENU-DOSSIER:fin -->
- [defense-orale.md](defense-orale.md) — trois objections a soutenir a voix haute avant de valider le module.
