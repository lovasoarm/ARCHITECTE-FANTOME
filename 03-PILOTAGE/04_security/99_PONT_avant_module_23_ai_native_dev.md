...
> **ARRÊTE-TOI ICI.** Ce fichier est un point de passage obligé entre `03-PILOTAGE/04_security` et `04-EPREUVE/04_ai_native_dev`. Ne l'ouvre pas comme "encore un chapitre" : c'est un palier de respiration avant un saut de nature.

## POURQUOI CE PONT EXISTE

Tu sais reconnaître XSS, CSRF, injection SQL, secrets fuités. `04-EPREUVE/04_ai_native_dev/` te demande de repérer les mêmes failles quand elles arrivent dans un output Copilot qui a l'air propre. La menace n'a pas changé, le canal si.
...
Copilot te propose `const hash = crypto.createHash('md5').update(pass).digest('hex')` pour hasher un mot de passe. Nomme les 3 problèmes. Réponse : MD5 (cassé), pas de salt, pas de coût. `03-PILOTAGE/04_security/05_hashing_bcrypt.md` a la version correcte.
