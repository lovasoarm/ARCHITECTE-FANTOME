...
### Signatures d'une course dans les logs

Une "course" (race condition) laisse des traces reconnaissables dans les logs, même sans
accès au code source. Trois signatures reviennent le plus souvent, chacune détectable par une
requête simple sur des logs structurés.

**Signature 1 : deux écritures à la même milliseconde sur la même clé**

```sql
-- Deux événements d'écriture sur le même identifiant, à moins de 5ms d'écart :
-- signe probable d'une absence de verrou entre lecture et écriture.
SELECT a.entity_id, a.recorded_at, b.recorded_at, a.actor_id, b.actor_id
FROM write_events a
JOIN write_events b
  ON a.entity_id = b.entity_id
  AND a.id < b.id
  AND b.recorded_at - a.recorded_at < INTERVAL '5 milliseconds'
ORDER BY a.recorded_at DESC;
```

**Signature 2 : un compteur qui régresse**

Un compteur métier (stock restant, places disponibles, solde) ne devrait varier que dans un
sens cohérent avec les opérations enregistrées. Une valeur qui remonte sans opération de
crédit correspondante révèle une écriture perdue (deux mises à jour concurrentes qui
s'écrasent l'une l'autre au lieu de s'additionner).

```sql
-- Recherche de régressions de compteur non expliquées par un événement de crédit
SELECT entity_id, recorded_at, value,
       LAG(value) OVER (PARTITION BY entity_id ORDER BY recorded_at) AS previous_value
FROM stock_snapshots
QUALIFY value > previous_value
  AND NOT EXISTS (
    SELECT 1 FROM restock_events r
    WHERE r.entity_id = stock_snapshots.entity_id
      AND r.recorded_at BETWEEN previous_recorded_at AND recorded_at
  );
```

**Signature 3 : deux écritures gagnantes sur la même clé**

Quand deux processus lisent la même valeur, la modifient chacun de leur côté, puis
l'enregistrent, la dernière écriture gagne et l'autre disparaît sans laisser de trace
d'erreur : aucune exception, juste une donnée perdue en silence.

```sql
-- Deux mises à jour sur la même clé primaire dans la même seconde, avec des
-- valeurs finales différentes de celles attendues par leur événement d'origine :
-- signe d'un écrasement silencieux (dernier écrivain gagnant).
SELECT entity_id, COUNT(*) AS writes_in_window, ARRAY_AGG(new_value ORDER BY recorded_at)
FROM update_log
GROUP BY entity_id, DATE_TRUNC('second', recorded_at)
HAVING COUNT(*) > 1;
```

Ces trois requêtes ne remplacent jamais un test de concurrence écrit délibérément (voir la
leçon sur les tests qui rapportent), mais elles permettent de confirmer ou d'écarter
l'hypothèse "course" en quelques minutes, sur des logs déjà produits, sans avoir à reproduire
l'incident en local.
...
## Le coût du volume de logs, chiffré

Logger structuré a un coût direct, facile à sous-estimer avant de le calculer une fois.

```text
Hypothèse : 1 000 requêtes par minute en moyenne sur l'API de tournées,
un log structuré d'environ 2 Ko par requête (champs, contexte, identifiants).

1 000 requêtes/minute x 2 Ko = 2 000 Ko/minute = ~2 Mo/minute
2 Mo/minute x 60 minutes x 24 heures x 30 jours = ~86 400 Mo/mois = ~86 Go/mois

A un tarif courant d'entrepot de logs (indicatif, variable selon le fournisseur),
cela represente une facture mensuelle directement proportionnelle au volume choisi,
avant meme de compter la retention longue duree ou la duplication multi-region.
```

Ce calcul, fait une seule fois sur un ordre de grandeur réaliste, justifie une règle
d'échantillonnage plutôt que "logger tout, tout le temps" :

```text
Règle d'échantillonnage qui découle du calcul ci-dessus :

  100 % des requêtes en erreur (4xx, 5xx)      --> toujours tout garder, le volume
                                                    d'erreurs est faible et leur valeur
                                                    diagnostique est maximale
  1 % du trafic nominal (200 OK, latence normale) --> un échantillon suffit pour
                                                    surveiller une tendance générale,
                                                    inutile de tout stocker

Avec ce ratio, le volume de logs du trafic nominal tombe à ~860 Mo/mois au lieu de
86 Go/mois, soit une division par 100, sans perdre aucune information sur les cas qui
comptent le plus pour le diagnostic (les erreurs).
```

Ordre de grandeur de rétention courant : logs bruts détaillés conservés 15 à 30 jours (assez
pour couvrir un cycle de facturation ou un audit récent), puis agrégats ou échantillons
conservés plusieurs mois si une obligation réglementaire ou un besoin de tendance long terme
le justifie. Conserver indéfiniment tout au niveau de détail maximal n'apporte presque jamais
de valeur proportionnelle au coût de stockage engagé.
