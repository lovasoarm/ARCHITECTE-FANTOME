...
Squelette minimal : reprends-le et écris toi-même les parties manquantes, ne le copie pas tel quel.

```js
const macrotasks = [];
const microtasks = [];

function scheduleMacro(fn) {
  macrotasks.push(fn);
}
...
function run() {
  while (macrotasks.length || microtasks.length) tick();
}
...
Ce module reutilise : les modeles mentaux de `01-CADRAGE/02-ASYNC/01A-00-why-async.md`.
