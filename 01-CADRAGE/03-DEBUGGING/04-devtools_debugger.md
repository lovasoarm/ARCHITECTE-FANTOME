...
async function rondeNocturne(camp) {
  const gardes = ["Rick", "le sniper", "Michonne"];
  await Promise.all(
    gardes.map(async (nom) => {
...
Promise.all([rondeNocturne(camp), patrouille(camp)]).then(() =>
  console.log("Stock final:", camp.stock.munitions),
);
```
