...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- |
| `async`                     | Mot-clé qui transforme une fonction en fonction qui retourne toujours une Promise. La valeur retournée est automatiquement wrappée dans une Promise résolue.                                        | `async function get() { return 42 }` : retourne `Promise<42>`                                         | Leon qui promet de ramener le rapport : qu'il réussisse ou qu'il échoue, il y aura toujours une réponse / le tampon de la poste qui garantit que ton colis aura un statut                        | « Leon qui promet de ramener le rapport : qu'il réussisse ou qu'il... » se corrige toute seule quand elle dérape ; sur async, la file d'attente n'est pas ordonnée par priorité métier mais par ordre d'arrivée technique. Rejoue le cas avec une latence artificielle de 2 s pour voir l'entrelacement. |
| `await`                     | Suspend l'exécution de la fonction `async` courante jusqu'à la résolution d'une Promise. Libère la call stack pendant l'attente : le reste de JS continue de tourner.                               | `const data = await fetch('/api')` : pause ici, reprend quand fetch résout                            | Walter White qui attend que la réaction chimique soit finie avant de passer à l'étape suivante : lui attend, mais le labo continue de tourner / le four qui cuit pendant que tu fais autre chose | « Walter White qui attend que la réaction chimique soit finie avant... » suppose que quelqu'un surveille ; sur await, une erreur levée après le premier `await` ne remonte plus dans le `try` de l'appelant synchrone. Écris le test qui lance deux appels concurrents, pas un seul. |
| `async function`            | Déclaration ou expression de fonction préfixée par `async`. Peut contenir des `await`. Retourne toujours une Promise.                                                                               | `const fn = async () => { ... }` : arrow async                                                        | Un Chevalier qui prend ses missions en promettant de répondre / un coursier qui s'engage à livrer, quoi qu'il arrive                                                                             | « Un Chevalier qui prend ses missions en promettant de répondre » suppose un seul acteur à la fois ; sur async function, une erreur levée après le premier `await` ne remonte plus dans le `try` de l'appelant synchrone. Instrumente avec un horodatage à chaque étape avant d'affirmer un ordre. |
| `try/catch` async           | Structure qui attrape les rejections de Promises dans un contexte `async/await`. Équivalent de `.catch()` en syntaxe synchrone.                                                                     | `try { await risky() } catch (e) { /* gère */ }`                                                      | Le filet de sécurité sous l'acrobate : si la Promise tombe, le catch l'attrape / le contrat d'assurance qui s'active quand ça merde                                                              | « Le filet de sécurité sous l'acrobate : si la Promise tombe, le... » tient tant que rien ne tombe en route ; sur try/catch async, une erreur levée après le premier `await` ne remonte plus dans le `try` de l'appelant synchrone. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| `Promise.all`               | Lance toutes les Promises en parallèle. Résout quand toutes sont résolues. Rejette immédiatement si l'une rejette.                                                                                  | `await Promise.all([p1, p2, p3])`                                                                     | Tous les Chevaliers partent en mission en même temps : tu attends que le dernier rentre / une cuisine où tous les plats cuisent en parallèle                                                     | « Tous les Chevaliers partent en mission en même temps : tu attends... » raconte le cas nominal ; sur Promise.all, les micro-tâches passent avant les macro-tâches, donc deux appels « simultanés » ne le sont jamais. Écris le test qui lance deux appels concurrents, pas un seul. |
| `Promise.allSettled`        | Lance toutes les Promises en parallèle. Attend que toutes soient terminées, résolues ou rejetées. Retourne un tableau de résultats avec leur statut.                                                | `await Promise.allSettled([p1, p2])` : retourne `[{status:'fulfilled',...}, {status:'rejected',...}]` | Le Conseil qui attend les rapports de tous les Chevaliers même si certains sont tombés en mission / l'appel à tous les passagers, même ceux qui n'ont pas répondu                                | « Le Conseil qui attend les rapports de tous les Chevaliers même si... » tient tant que rien ne tombe en route ; sur Promise.allSettled, le temps réseau varie d'un facteur dix entre ta machine et la production. Écris le test qui lance deux appels concurrents, pas un seul. |
| `Promise.race`              | Retourne une Promise qui résout ou rejette avec la première Promise qui se termine, dans les deux sens.                                                                                             | `await Promise.race([fetch(), timeout(3000)])`                                                        | Le premier Chevalier à arriver prend la mission, les autres rentrent bredouilles / le sprint : peu importe qui gagne, le match s'arrête                                                          | « Le premier Chevalier à arriver prend la mission, les autres... » suppose un seul acteur à la fois ; sur Promise.race, deux opérations qui se recouvrent dans le temps entrelacent leurs effets, et l'état intermédiaire devient observable. Écris le test qui lance deux appels concurrents, pas un seul. |
| `Promise.any`               | Résout avec la première Promise qui réussit. Rejette seulement si toutes rejettent.                                                                                                                 | `await Promise.any([p1, p2, p3])` : ignore les rejections individuelles                               | On cherche n'importe quel Chevalier disponible : le premier qui répond est le bon / le premier bus qui arrive, peu importe la ligne                                                              | « On cherche n'importe quel Chevalier disponible : le premier qui... » tient tant que rien ne tombe en route ; sur Promise.any, annuler une opération déjà partie ne défait pas l'effet déjà écrit côté serveur. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| Séquentiel                  | Exécution d'opérations async l'une après l'autre. Chaque `await` attend la fin du précédent. Utiliser quand chaque étape dépend du résultat de la précédente.                                       | `const a = await step1(); const b = await step2(a)`                                                   | Le plan d'évasion de Scofield : chaque étape dépend de la précédente / une recette où chaque étape dépend du résultat de celle d'avant                                                           | « Le plan d'évasion de Scofield : chaque étape dépend de la précédente » décrit un monde où chaque étape se voit ; sur Séquentiel, l'ordre d'exécution est décidé par la boucle d'événements, pas par l'ordre d'écriture des lignes. Instrumente avec un horodatage à chaque étape avant d'affirmer un ordre. |
| Parallèle                   | Exécution simultanée de plusieurs Promises. Utiliser quand les opérations sont indépendantes. Gain de temps = temps du plus lent, pas somme des temps.                                              | `const [a, b] = await Promise.all([step1(), step2()])`                                                | Leon et Rei qui partent en mission en même temps plutôt que l'un après l'autre / deux téléchargements simultanés                                                                                 | « Leon et Rei qui partent en mission en même temps plutôt que l'un... » suppose un seul acteur à la fois ; sur Parallèle, deux opérations qui se recouvrent dans le temps entrelacent leurs effets, et l'état intermédiaire devient observable. Rejoue le cas avec une latence artificielle de 2 s pour voir l'entrelacement. |
| `UnhandledPromiseRejection` | Erreur qui arrive quand une Promise rejette sans que personne ne la catchait. En Node.js : crash. En navigateur : avertissement dans la console.                                                    | `fetchData()` sans `await` ni `.catch()` : si ça rejette : `UnhandledPromiseRejection`                | Un Chevalier qui échoue et dont personne ne traite le rapport d'échec : la mission disparaît dans le vide / une bombe sans démineur                                                              | « Un Chevalier qui échoue et dont personne ne traite le rapport... » suppose que quelqu'un surveille ; sur UnhandledPromiseRejection, le rejet d'une promesse non attrapée ne stoppe pas le programme, il le laisse continuer dans un état faux. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| `top-level await`           | `await` utilisé directement à la racine d'un module ES (pas dans une fonction `async`). Disponible en ESM natif et dans certains environnements Node. Bloque l'import du module jusqu'à résolution. | `const config = await loadConfig()` : directement dans `config.mjs`                                   | La config qui doit être chargée avant que le reste du programme puisse démarrer / le générateur de courant qui doit démarrer avant d'allumer les lumières                                        | « La config qui doit être chargée avant que le reste du programme... » se rejoue à l'identique, le code non ; sur top-level await, une erreur levée après le premier `await` ne remonte plus dans le `try` de l'appelant synchrone. Instrumente avec un horodatage à chaque étape avant d'affirmer un ordre. |
| `async IIFE`                | Pattern pour exécuter du code async immédiatement sans top-level await. Utilisé dans les contextes non-ESM.                                                                                         | `;(async () => { const data = await fetch(); console.log(data) })()`                                  | Le technicien qui arrive, fait son travail, et repart sans attendre d'invitation / un script qui s'exécute seul à l'import                                                                       | « Le technicien qui arrive, fait son travail, et repart sans... » se rejoue à l'identique, le code non ; sur async IIFE, l'ordre d'exécution est décidé par la boucle d'événements, pas par l'ordre d'écriture des lignes. Rejoue le cas avec une latence artificielle de 2 s pour voir l'entrelacement. |
| Retry                       | Pattern qui retente une opération async en cas d'échec, avec un délai progressif. Utile pour les erreurs temporaires (réseau, rate limit, 503).                                                     | `for (let i = 0; i < 3; i++) { try { return await fn() } catch(e) { await sleep(i*1000) } }`          | Leon qui retente sa mission si la météo était mauvaise, pas s'il a perdu son armure / relancer sa demande de visa après un refus temporaire                                                      | « Leon qui retente sa mission si la météo était mauvaise, pas s'il a... » décrit un monde où chaque étape se voit ; sur Retry, annuler une opération déjà partie ne défait pas l'effet déjà écrit côté serveur. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| Timeout                     | Wrapping d'une Promise avec `Promise.race` contre un timer. Si l'opération prend plus longtemps que le seuil : erreur de timeout.                                                                   | `Promise.race([fetch(), new Promise((_, r) => setTimeout(() => r(new Error('Timeout')), 3000))])`     | L'armure de Garo : si le combat dépasse 99.9 secondes, elle se désintègre / la minuterie du four qui coupe même si la cuisson n'est pas finie                                                    | « L'armure de Garo : si le combat dépasse 99.9 secondes, elle se... » se rejoue à l'identique, le code non ; sur Timeout, deux opérations qui se recouvrent dans le temps entrelacent leurs effets, et l'état intermédiaire devient observable. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| Fallback                    | Plan B exécuté si le plan A échoue. Permet de retourner un résultat dégradé plutôt que de propager l'erreur.                                                                                        | `try { return await primary() } catch { return await backup() }`                                      | Si Leon est indispo, Rei prend la mission / si le serveur principal est down, le miroir prend le relais                                                                                          | « Si Leon est indispo, Rei prend la mission » décrit un monde où chaque étape se voit ; sur Fallback, le temps réseau varie d'un facteur dix entre ta machine et la production. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| `async/await` vs `.then()`  | Même comportement, syntaxe différente. `async/await` : plus lisible pour les chaînes longues. `.then()` : parfois plus concis pour les transformations simples. Les deux peuvent être mélangés.     | `const x = await fn()` == `fn().then(x => ...)`                                                       | Deux façons d'écrire la même lettre : à la main ou à la machine / deux routes qui mènent au même point                                                                                           | « Deux façons d'écrire la même lettre : à la main ou à la machine » a une frontière visible à l'oeil ; sur async/await vs .then(), les micro-tâches passent avant les macro-tâches, donc deux appels « simultanés » ne le sont jamais. Écris le test qui lance deux appels concurrents, pas un seul. |
