...
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ / meme mecanique cote football : le staff repete jusqu'a ce que la tactique tienne sans le tableau | « -------------------------------------------------------------------... » s'arrête à la première surprise ; sur ------------------------------------, l'ordre d'exécution est décidé par la boucle d'événements, pas par l'ordre d'écriture des lignes. Écris le test qui lance deux appels concurrents, pas un seul. |
| **Callback** | Fonction passée en argument à une autre fonction, pour être appelée plus tard : souvent quand une opération async est terminée. | `charger(id, function(err, data) { ... })` | Laisser son numéro à un livreur pour qu'il rappelle quand il arrive / Donner une sonnette à un médecin pour te rappeler quand c'est ton tour | « Laisser son numéro à un livreur pour qu'il rappelle quand il arrive » s'arrête à la première surprise ; sur Callback, deux opérations qui se recouvrent dans le temps entrelacent leurs effets, et l'état intermédiaire devient observable. Rejoue le cas avec une latence artificielle de 2 s pour voir l'entrelacement. |
| **Error-first callback** | Convention Node.js : le premier argument du callback est toujours une erreur (`null` si tout va bien). Aucune librairie Node sérieuse déroge à ça. | `fs.readFile("f.txt", function(err, data) { if (err) return; ... })` | Un rapport militaire qui commence toujours par "situation" avant "plan d'action" / Un arbitre qui signale d'abord le hors-jeu avant de laisser le jeu continuer | « Un rapport militaire qui commence toujours par "situation" avant... » n'a ni facture ni horloge ; sur Error-first callback, une erreur levée après le premier `await` ne remonte plus dans le `try` de l'appelant synchrone. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| **Callback hell** | Imbrication de callbacks qui crée une pyramide illisible. Pas un problème esthétique : un problème de maintenance, de gestion d'erreur, et de testabilité. | `a(function() { b(function() { c(function() { /* t'es ici */ }) }) })` | Un plan d'évasion avec 5 portes, chaque porte cachée derrière la précédente / Un arbre généalogique dessiné de droite à gauche | « Un plan d'évasion avec 5 portes, chaque porte cachée derrière la... » a une frontière visible à l'oeil ; sur Callback hell, une erreur levée après le premier `await` ne remonte plus dans le `try` de l'appelant synchrone. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| **Inversion of control** | Quand tu passes ton callback à une fonction tierce, tu lui confies le contrôle : c'est elle qui décide quand appeler ton code, combien de fois, et avec quoi. | `librairieTierce.charger(opts, monCallback)` : `monCallback` peut être appelé 0, 1 ou N fois | Confier sa carte de crédit à quelqu'un d'autre pour qu'il paye à ta place / Donner les clés de son club à un agent et espérer qu'il négocie bien | « Confier sa carte de crédit à quelqu'un d'autre pour qu'il paye à... » tient tant que rien ne tombe en route ; sur Inversion of control, les micro-tâches passent avant les macro-tâches, donc deux appels « simultanés » ne le sont jamais. Instrumente avec un horodatage à chaque étape avant d'affirmer un ordre. |
| **Continuation-passing style (CPS)** | Style de code où chaque fonction reçoit une "continuation" : ce qu'il faut faire ensuite : au lieu de retourner une valeur. Les callbacks error-first sont du CPS. | `function etape(data, suite) { traiter(data); suite(null, resultat) }` | Passer le ballon au lieu de marquer soi-même / Transmettre un dossier à l'équipe suivante au lieu de le garder | « Passer le ballon au lieu de marquer soi-même » se rejoue à l'identique, le code non ; sur Continuation-passing style (CPS), les micro-tâches passent avant les macro-tâches, donc deux appels « simultanés » ne le sont jamais. Instrumente avec un horodatage à chaque étape avant d'affirmer un ordre. |
| **Sequential callbacks** | Callbacks exécutés l'un après l'autre, chaque étape attendant la fin de la précédente. Indispensable quand l'étape N dépend du résultat de N-1. | `a(function(err, r1) { b(r1, function(err, r2) { c(r2, ...) }) })` | Faire les passes dans l'ordre du jeu posé, pas en rush / Monter les chapitres d'un plan dans l'ordre logique | « Faire les passes dans l'ordre du jeu posé, pas en rush » suppose un seul acteur à la fois ; sur Sequential callbacks, une erreur levée après le premier `await` ne remonte plus dans le `try` de l'appelant synchrone. Instrumente avec un horodatage à chaque étape avant d'affirmer un ordre. |
| **Parallel callbacks** | Plusieurs callbacks lancés simultanément, un compteur qui détecte quand tous sont terminés. Utilisé quand les opérations sont indépendantes. | `let count = 3; [a,b,c].forEach(id => charger(id, () => { if(--count === 0) fin() }))` | Trois joueurs qui récupèrent le ballon sur différentes zones en même temps / Trois cuisiniers qui préparent chaque plat d'un repas en parallèle | « Trois joueurs qui récupèrent le ballon sur différentes zones en... » s'arrête à la première surprise ; sur Parallel callbacks, annuler une opération déjà partie ne défait pas l'effet déjà écrit côté serveur. Écris le test qui lance deux appels concurrents, pas un seul. |
| **Named callbacks** | Technique anti-callback-hell : extraire les fonctions anonymes en fonctions nommées au lieu de les imbriquer. Même comportement, code linéaire et lisible. | `function surChargement(err, data) { ... }` puis `charger(id, surChargement)` | Donner un prénom à chaque joueur plutôt que de les appeler "le gars là" / Nommer chaque acte d'une pièce de théâtre au lieu de les numéroter | « Donner un prénom à chaque joueur plutôt que de les appeler "le... » a une frontière visible à l'oeil ; sur Named callbacks, la file d'attente n'est pas ordonnée par priorité métier mais par ordre d'arrivée technique. Rejoue le cas avec une latence artificielle de 2 s pour voir l'entrelacement. |
| **Timeout pattern** | Combiner un callback async avec un `setTimeout` pour garantir qu'une réponse est émise dans un délai donné : succès ou erreur. Un booléen `estTermine` évite les doubles appels. | `const t = setTimeout(() => { if (!done) callback(new Error("Timeout")) }, 3000)` | Un minuteur pendant une épreuve : si le joueur ne répond pas dans le temps imparti, l'arbitre siffle / Un transfert qui expire à minuit si les deux clubs ne signent pas | « Un minuteur pendant une épreuve : si le joueur ne répond pas dans... » s'arrête à la première surprise ; sur Timeout pattern, les micro-tâches passent avant les macro-tâches, donc deux appels « simultanés » ne le sont jamais. Trace le chemin d'erreur, pas seulement le chemin nominal. |
| **Retry pattern** | Réappeler une opération async en cas d'échec, avec un compteur de tentatives et un délai croissant entre chaque essai. | `function essayer(n) { op(function(err) { if (err && n > 0) setTimeout(() => essayer(n-1), 500) }) }` | Tirer un penalty raté une deuxième fois si l'arbitre ordonne un retir / Relancer une négociation de transfert échouée après un délai de réflexion | « Tirer un penalty raté une deuxième fois si l'arbitre ordonne un retir » n'a ni facture ni horloge ; sur Retry pattern, le temps réseau varie d'un facteur dix entre ta machine et la production. Instrumente avec un horodatage à chaque étape avant d'affirmer un ordre. |
| **Concurrency control** | Limiter le nombre de callbacks qui s'exécutent simultanément. Un compteur `actif` empêche de dépasser le seuil, une queue lance les suivants au fur et à mesure. | `while (actif < MAX && index < ids.length) { actif++; charger(ids[index++], onDone) }` | Max 3 joueurs en zone de presse en même temps / Faire entrer les spectateurs par groupes pour ne pas saturer les tourniquets | « Max 3 joueurs en zone de presse en même temps » se corrige toute seule quand elle dérape ; sur Concurrency control, le rejet d'une promesse non attrapée ne stoppe pas le programme, il le laisse continuer dans un état faux. Écris le test qui lance deux appels concurrents, pas un seul. |
| **Thunkification** | Transformer une fonction error-first callback en une fonction qui retourne une fonction sans argument. Étape intermédiaire vers les Promises. | `const thunk = (cb) => charger(id, cb)` | Préparer une action sans la déclencher, puis l'activer quand on est prêt / Préparer un discours et le lire seulement quand on a le micro | « Préparer une action sans la déclencher, puis l'activer quand on... » n'a ni facture ni horloge ; sur Thunkification, deux opérations qui se recouvrent dans le temps entrelacent leurs effets, et l'état intermédiaire devient observable. Instrumente avec un horodatage à chaque étape avant d'affirmer un ordre. |
