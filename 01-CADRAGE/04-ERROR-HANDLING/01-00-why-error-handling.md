...
> **CE MODULE RÉUTILISE** : try/catch (00-SOCLE/04-FUNDAMENTALS), async & promises (01-CADRAGE/02-ASYNC). Si un de ces prérequis est flou, retourne le voir avant. Ce module ne les réexplique pas.
...
Oui. Le curriculum le place explicitement dans le noyau dur : "sans ça, t'es imprudent". `01-CADRAGE/04-ERROR-HANDLING` dépend de `00-SOCLE/04-FUNDAMENTALS` et `01-CADRAGE/02-ASYNC`, et c'est un prérequis direct pour `02-CONSTRUCTION/19-API-CRAFT` et `03-PILOTAGE/04-SECURITY`. Un dev qui ne maîtrise pas ce module construit des APIs fragiles, des systèmes qui crashent en cascade, et des incidents de prod qui auraient pu être évités avec un `try/catch` bien placé.
...
Maintenant, ouvre `02-try_catch_basics.md`. Et apprends ce que `try/catch` attrape vraiment, et ce qu'il laisse filer.
